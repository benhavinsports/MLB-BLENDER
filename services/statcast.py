...
def _blender_pull_profile(
    ...
    """Expose direct Savant Pull-Air fields without manufacturing a blend.

    ``pull`` is reserved for the direct league percentile of Pull AIR%, which is
    computed from the complete batted-ball leaderboard. Raw Pull%, AIR%, Pull
    AIR% and Pull Barrel% remain separate evidence lanes.
    """
    return {
        "pull": pull_air_percentile,
        "pull_percent": pull_percent,
        "pua": pull_air_percent,
        "pull_barrel": pull_barrel_percent,
        "pull_identity_source": (
            "SAVANT_PULL_AIR_PERCENTILE"
            if pull_air_percentile is not None
            else None
        ),
    }
