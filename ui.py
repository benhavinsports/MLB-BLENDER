...
/* v91 team color visual layer */
@keyframes elitePulse {
  from { transform: translateY(0); filter: brightness(1); }
  to { transform: translateY(-2px); filter: brightness(1.18); }
}
.player-card-v91{border-radius:28px;padding:28px;margin:22px 0;color:#FFF4E0;}
.chip-row{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px;}
.team-chip{border:1px solid;border-radius:999px;padding:8px 14px;font-weight:900;letter-spacing:.3px;}
.player-name-v91{font-family:Impact,Haettenschweiler,'Arial Black',sans-serif;font-size:clamp(42px,8vw,82px);line-height:.95;letter-spacing:1px;margin:12px 0;}
.matchup-v91{font-size:20px;color:#E8DFD0;font-weight:700;margin:12px 0;}
.score-v91{font-family:Impact,Haettenschweiler,'Arial Black',sans-serif;font-size:clamp(58px,12vw,112px);line-height:1;margin-top:18px;}
.score-label-v91{font-weight:900;color:#BFB7AA;margin-top:8px;}
.bar-v91{height:14px;background:#242424;border-radius:999px;overflow:hidden;margin:10px 0 20px;}
.bar-v91 div{height:100%;border-radius:999px;}
.metrics-grid-v91{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0;}
.metrics-grid-v91 div{border:1px solid #333;border-radius:16px;padding:14px;background:#0d0d0dcc;}
.metrics-grid-v91 b{display:block;font-size:24px;color:#FFF4E0;}
.metrics-grid-v91 span{display:block;color:#aaa;margin-top:6px;}
.path-v91,.fails-v91,.soft-v91{font-size:16px;line-height:1.55;color:#EFE7D5;font-weight:700;margin-top:12px;}
.fails-v91{color:#ff9a9a;}
.soft-v91{color:#ffe28a;}
...
def card(r):
    ...
    st.markdown(f"""
    <div class="player-card-v91" style="
    ...
    ">
        <div class="chip-row">
            <span class="team-chip" style="background:{primary}; color:{text_color}; border-color:{secondary};">{role}</span>
            <span class="team-chip" style="background:#111; color:{secondary}; border-color:{primary};">{archetype}</span>
        </div>
        <div class="player-name-v91" style="color:{text_color}; text-shadow: 0 0 10px {primary};">{fire}{player}</div>
        <div class="matchup-v91"><span style="color:{secondary}; font-weight:900;">{team}</span> · {game} · vs {opponent_pitcher} · Slot {slot}</div>
        <div class="score-v91" style="color:{secondary};">{score:.1f}</div>
        <div class="score-label-v91">TRUE BLEND SCORE</div>
        <div class="bar-v91"><div style="width:{min(100,max(0,score))}%; background:linear-gradient(90deg,{primary},{secondary});"></div></div>
        <div class="metrics-grid-v91">
        ...
        <div class="path-v91"><b>Blend path:</b> {gate_path}</div>
        {f'<div class="fails-v91"><b>Hard:</b> {hard}</div>' if hard else ''}
        {f'<div class="soft-v91"><b>Soft:</b> {soft}</div>' if soft else ''}
    </div>
    """, unsafe_allow_html=True)
