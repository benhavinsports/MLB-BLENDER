...
def board_card_v108(r):
    ...
    flags = flags if flags else "Clean / pass lane"
    st.markdown(f"""
...
def _card_shell_v109(r, compact=False):
    ...
    flags = str(r.get("hard_fails","")).strip() or str(r.get("soft_fails","")).strip() or "Clean / pass lane"
...
def _v113_board_tile(group):
    ...
    pool = top.get("game","") or f"{top.get('team','')} vs {top.get('pitcher','')}"
