...
def card(r, extra=""):
    name = r.get("player", "")
    slot = r.get("core_slot", r.get("ticket_role", ""))
    actual_role = r.get("official_core_role", r.get("true_role_path", ""))
    role = (str(slot) + " · " if str(slot).strip() else "") + str(actual_role)
    game = r.get("game_key", r.get("game", ""))
    pitcher = r.get("pitcher", "")
    try:
        score = f'{float(r.get("blender_score", r.get("score", 0))):.1f}'
    except Exception:
