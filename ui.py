...
.machine-top{position:absolute;left:16px;right:16px;top:12px;height:58px;display:grid;grid-template-columns:230px 1fr 150px 150px 150px;gap:10px;z-index:8;}
...
.machine-path{position:absolute;left:46px;right:46px;top:92px;bottom:170px;z-index:1}.machine-path svg{width:100%;height:100%;overflow:visible}
...
def hero():
    st.markdown(
        '<div style="padding:12px 16px 0;font-size:42px;font-weight:1000;line-height:.9">'
        'THE <span style="color:#b7ff32">BLENDER</span> MACHINE</div>',
        unsafe_allow_html=True,
    )
...
def _machine_tiles_from_results(results):
    ...
    for idx, r in enumerate(source):
        ...
        tiles.append({"owner": player, "game": game, "bucket": bucket, "glow": glow})
...
# Compatibility aliases for older app imports
