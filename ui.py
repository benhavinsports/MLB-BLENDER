...
# UPLOAD_SYNC_GAME_BOARD_OVERRIDE
def game_board_grid_view(results, key_prefix="gb_grid_v147_sync"):
    st.markdown("## 🧩 GAME BOARD — SYNCED RESULTS")
    if not isinstance(results, dict):
    ...
    if owners is not None and not owners.empty:
        ...
        st.warning("No locked owners yet.")

    if role_board is not None and not role_board.empty:
        st.markdown("### Role Outcome Memory")
        st.dataframe(role_board, use_container_width=True, hide_index=True)

    if game_board is not None and not game_board.empty:
