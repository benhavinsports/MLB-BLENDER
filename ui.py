...
def _feed_game_summary(feed_df):
    df = _df(feed_df).copy()
    if df.empty:
        return pd.DataFrame()
    if "game_key" not in df.columns:
        df["game_key"] = df["game"] if "game" in df.columns else ""
    cols = [c for c in ["game_key","game_time_et","pitcher","team","player"] if c in df.columns]
    if not cols or "game_key" not in df.columns:
        return pd.DataFrame()
    rows = []
    for game, g in df.groupby("game_key", dropna=False):
        ...
        if not game:
            continue
        teams = sorted([str(x) for x in g.get("team", pd.Series(dtype=str)).dropna().astype(str).unique() if str(x).strip()])
        pitchers = sorted([str(x) for x in g.get("pitcher", pd.Series(dtype=str)).dropna().astype(str).unique() if str(x).strip()])
        rows.append({
            "game_key": game,
            "game_time_et": str(g["game_time_et"].dropna().astype(str).iloc[0]) if "game_time_et" in g and not g["game_time_et"].dropna().empty else "",
            "teams_in_feed": " / ".join(teams[:2]),
            "pitchers_seen": " / ".join(pitchers[:2]),
            "feed_players": int(len(g)),
            "unique_players": int(g["player"].nunique()) if "player" in g else int(len(g)),
        })
    return pd.DataFrame(rows)

def game_board_grid_view(results, key_prefix="gb_locked", feed_df=None):
    ...
    feed_games = _feed_game_summary(feed_df)
    if not feed_games.empty:
        st.markdown("### 0️⃣ Full Slate Feed Map — Game Board Only")
        st.caption("This board comes directly from the uploaded feed so the visual board keeps every game. It does not change CORE / ALT / WHO ticket integrity.")
        st.dataframe(feed_games, use_container_width=True, hide_index=True)
    ...
    if owners.empty:
        ...
        for _, r in owners.head(30).iterrows():
            card(r)
    if not feed_games.empty:
        owner_games = set(owners.get("game_key", pd.Series(dtype=str)).dropna().astype(str)) if not owners.empty else set()
        missing_games = feed_games[~feed_games["game_key"].astype(str).isin(owner_games)].copy()
        if not missing_games.empty:
            with st.expander("Games present in feed but not rendered as ticket owners — board audit only", expanded=False):
                st.dataframe(missing_games, use_container_width=True, hide_index=True)
