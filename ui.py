...
def blender_machine(names, state="READY"):
    ...
    st.markdown(f"""<div class="machine"><div class="blender-wrap"><div class="feed-slot">CLICK / FEED DATA HERE</div><div class="jar"><div class="blade"></div><div class="center">{state}</div>{floats}</div><div class="output-slot">UPLOAD → BLADES SPIN → OWNERS LOCK → TICKETS POP OUT</div></div></div>""", unsafe_allow_html=True)
