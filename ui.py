...
# =========================================================
# REAL GAMEBOARD UI IMPLEMENTATION
# =========================================================
...
radial-gradient(circle at bottom right, rgba(249,115,22,.10), transparent 25%),
#04070d;
color:var(--text);
}
...
.tile-blue{background:linear-gradient(160deg,rgba(37,99,235,.88),rgba(29,78,216,.72));}
.tile-red{background:linear-gradient(160deg,rgba(220,38,38,.88),rgba(153,27,27,.72));}
.tile-orange{background:linear-gradient(160deg,rgba(249,115,22,.88),rgba(194,65,12,.72));}
.tile-green{background:linear-gradient(160deg,rgba(34,197,94,.88),rgba(21,128,61,.72));}
.tile-purple{background:linear-gradient(160deg,rgba(147,51,234,.88),rgba(107,33,168,.72));}
...
.core-card{border-top:4px solid var(--core);}
.alt-card{border-top:4px solid var(--alt);}
.who-card{border-top:4px solid var(--who);}
...
50%{box-shadow:0 0 28px rgba(59,130,246,.25);}
...
GAMEBOARD_GATES = [
("Core Matchup","tile-blue"),
("Vegas","tile-red"),
("Starter","tile-orange"),
("Ballpark","tile-purple"),
("Weather","tile-red"),
("Discipline","tile-orange"),
("Hard Hit","tile-blue"),
("Barrel","tile-green"),
("xwOBA","tile-green"),
("ISO","tile-red"),
("Swing","tile-purple"),
("Lineup","tile-purple"),
("Recent Form","tile-orange"),
("Survivor Cut","tile-purple"),
("Owner Lock","tile-green")
]

def render_real_blender_board():
    st.markdown(BLENDER_THEME_CSS, unsafe_allow_html=True)

    tiles = ""
    for label, cls in GAMEBOARD_GATES:
        tiles += f'<div class="road-tile {cls}">{label}</div>'

    st.markdown(f"""
    <div class="blender-board">
        <div class="road-row">
            {tiles}
        </div>

        <div class="owner-grid">
            <div class="owner-card core-card live-pulse">
                <div class="owner-title">◎ CORE</div>
                <div class="archetype-pill">CLEAN OWNER</div>
                <div class="archetype-pill">EVENT LOCK</div>
                <div class="chaos-meter"></div>
            </div>

            <div class="owner-card alt-card">
                <div class="owner-title">⟲ ALT</div>
                <div class="archetype-pill">TRANSFER</div>
                <div class="archetype-pill">ADJACENT</div>
                <div class="chaos-meter"></div>
            </div>

            <div class="owner-card who-card">
                <div class="owner-title">? WHO</div>
                <div class="archetype-pill">CHAOS</div>
                <div class="archetype-pill">SPREAD</div>
                <div class="chaos-meter"></div>
            </div>
        </div>
    </div>
    ...
    st.markdown("### Intelligence Tiles")

    cols = st.columns(3)

    intelligence = [
        ("Spread Detection","Detects multi-owner environments and false clean traps."),
        ("Finisher DNA","Tracks real HR conversion ability vs loud-out profiles."),
        ("Event Ownership","Models pressure inheritance and actual HR ownership."),
        ("Chaos Override","Detects entropy windows and spread activation."),
        ("Elimination Timeline","Shows which gate removed hitters."),
        ("Live Flow","Tracks bullpen inheritance and survivor evolution.")
    ]

    for i, (title, body) in enumerate(intelligence):
        with cols[i % 3]:
            with st.expander(title):
                st.markdown(f'<div class="intel-panel">{body}</div>', unsafe_allow_html=True)
