...
# ============================================================
# Duplicate-key-safe download helper V6
# ============================================================
def safe_download_button(label, data, file_name, mime="text/csv", key_suffix=""):
    import hashlib
    raw = f"{label}-{file_name}-{key_suffix}".encode("utf-8", errors="ignore")
    key = "dl_" + hashlib.md5(raw).hexdigest()
...
def tickets_view(results, key_prefix='tickets_v65'):
    ...
    for label,key in [("CORE 3","core"),("ALT 3","alt"),("CHAOS 3","chaos")]:
        ...
        if df is None or df.empty:
            ...
            st.download_button(f"Download {label}", csv_bytes(df), f"{label.lower().replace(' ','_')}.csv", "text/csv", key=f"{key_prefix}_{key}_download_v66")
...
def game_board_view(results):
    ...
    st.download_button("Download Game Board CSV", csv_bytes(owners), "game_board.csv", "text/csv", key="game_board_download_csv_btn_v66")
