...
# === ENGINE ONLY REPAIR UI HOOK ===
def _repair_df(x):
    return x if isinstance(x, pd.DataFrame) else pd.DataFrame()

def run_ticket_view(results, bucket="core", window="Full slate"):
    df = _repair_df(results.get(bucket) if isinstance(results, dict) else None).copy()
    if df.empty: return df
    # Only apply Early/Late if real labels exist; otherwise show all for testing.
...
def tickets_view(results, key_prefix="tickets_engine_repair"):
    st.markdown("## 🎟️ Tickets")
...
def game_board_grid_view(results, key_prefix="gb_engine_repair"):
    ...
    owners = _repair_df(results.get("owners"))
    if not owners.empty:
        st.markdown("### Locked Owners")
        for _, r in owners.head(20).iterrows():
            card(r)
    role_board = _repair_df(results.get("role_board"))
    game_board = _repair_df(results.get("game_board"))
    survivors = _repair_df(results.get("survivors"))
    with st.expander("Role Outcome Memory", expanded=False):
        st.dataframe(role_board, use_container_width=True, hide_index=True)
    with st.expander("True Gate Trace", expanded=False):
        st.dataframe(game_board, use_container_width=True, hide_index=True)
    with st.expander("Full Survivor Audit", expanded=False):
        cols=[c for c in ["game_key","player","team","pitcher","official_core_role","archetype","blender_eligible","metric_count","blender_score","support_score","final_reason"] if c in survivors.columns]
        st.dataframe(survivors[cols], use_container_width=True, hide_index=True)


# ============================================================
# FINAL TICKET WINDOW FIX
# Overrides all previous ticket renderers.
# If the PDF has no real game times, Early/Late must NOT hide picks.
# If core/alt/chaos are empty but owners exist, rebuild display buckets.
# ============================================================
def _final_df(x):
    return x if isinstance(x, pd.DataFrame) else pd.DataFrame()

def _final_ticket_bucket(results, bucket):
    if not isinstance(results, dict):
        return pd.DataFrame()

    df = _final_df(results.get(bucket)).copy()
    owners = _final_df(results.get("owners")).copy()

    # fallback: rebuild ticket buckets from owners if previous bucket got blanked
    if df.empty and not owners.empty:
        ...
        if bucket == "core":
            ...
            if not df.empty: df["ticket_role"] = "CORE"
        elif bucket == "alt":
            used = set(_final_ticket_bucket(results, "core")["player"].astype(str)) if "player" in owners.columns and not _final_ticket_bucket(results, "core").empty else set()
            df = owners[~owners["player"].astype(str).isin(used)].head(3).copy() if "player" in owners.columns else owners.iloc[3:6].copy()
            if not df.empty: df["ticket_role"] = "ALT"
        elif bucket == "chaos":
            if "official_core_role" in owners.columns:
                df = owners[owners["official_core_role"].astype(str).str.contains("WHO|Chaos", case=False, na=False)].head(3).copy()
            if df.empty:
                df = owners.tail(min(3, len(owners))).copy()
            if not df.empty: df["ticket_role"] = "WHO"
    return df

def run_ticket_view(results, bucket="core", window="Full slate"):
    df = _final_ticket_bucket(results, bucket).copy()
    if df.empty:
        return df

    # Only filter by window if actual Early/Late labels exist in the ticket rows.
    if window and window != "Full slate" and "slate_window" in df.columns:
        labels = set(df["slate_window"].dropna().astype(str).str.lower().unique())
        if "early" in labels or "late" in labels:
            df = df[df["slate_window"].astype(str).str.lower() == window.lower()]
        # else: do not hide rows

    if "official_core_role" in df.columns:
        df = df[~df["official_core_role"].astype(str).str.contains("NO PICK|NO PLAY|AUDIT", case=False, na=False)]
    if "player" in df.columns:
        df = df.drop_duplicates(["player"], keep="first")
    return df

def tickets_view(results, key_prefix="tickets_final_window_fix"):
    st.markdown("## 🎟️ Tickets")
    window = st.selectbox("Slate window", ["Full slate","Early","Late"], key=f"{key_prefix}_window")
    if window != "Full slate":
        st.caption("No parsed game times = window filter disabled so valid picks still show.")
    for label, key in [("CORE 3","core"),("ALT 3","alt"),("CHAOS 3","chaos")]:
        st.markdown(f"### {label}")
        df = run_ticket_view(results, key, window)
        if df.empty:
...
def game_board_grid_view(results, key_prefix="gb_final_window_fix"):
    st.markdown("## 🧩 GAME BOARD — TRUE ENGINE")
    if not isinstance(results, dict):
    ...
    c2.metric("Games", meta.get("games", 0))
    c3.metric("Owners", meta.get("owners_locked", 0))
    c4.metric("Core", meta.get("core_count", len(_final_ticket_bucket(results, "core"))))
    if meta.get("message"): st.info(meta["message"])

    owners = _final_df(results.get("owners"))
    if owners.empty:
        st.warning("No locked owners.")
    else:
        st.markdown("### Locked Owners")
        for _, r in owners.head(20).iterrows():
            card(r)
    with st.expander("Role Outcome Memory", expanded=False):
        st.dataframe(_final_df(results.get("role_board")), use_container_width=True, hide_index=True)
    with st.expander("True Gate Trace", expanded=False):
        st.dataframe(_final_df(results.get("game_board")), use_container_width=True, hide_index=True)
    with st.expander("Full Survivor Audit", expanded=False):
        survivors = _final_df(results.get("survivors"))
        cols=[c for c in ["game_key","player","team","pitcher","official_core_role","archetype","blender_eligible","metric_count","blender_score","support_score","final_reason"] if c in survivors.columns]
        st.dataframe(survivors[cols], use_container_width=True, hide_index=True)
