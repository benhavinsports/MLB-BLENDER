...
.real-run-wrap button{
  ...
  height:70px!important;
  box-shadow:0 0 24px rgba(251,191,36,.85)!important;
}
</style>
...
def _normalize(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "ready": bool(st.session_state.get("ready_to_run", False)),
        "rows": _txt(st.session_state.get("parsed_feed_rows", "")),
        "ready": bool(st.session_state.get("ready_to_run", False)),
        "rows": _txt(st.session_state.get("parsed_feed_rows", "")),
    }
...
<div class="board">
  ...
  <div class="footer"><div>START Upload</div><div>Hidden 23 Gates</div><div>Dynamic Tiles</div><div>Core / Alt / Chaos</div><div>RUN Button</div></div>
...
function renderTop(){
  document.getElementById("uploadName").textContent=data.uploaded?(data.ready?"Loaded — press FINISH":"Loaded: "+data.uploaded):"START Upload";
