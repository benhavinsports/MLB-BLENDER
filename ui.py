...
.gb-track{display:flex;gap:8px;align-items:center;overflow-x:auto;padding:14px 10px;margin:12px 0;border:2px solid #fff;border-radius:999px;background:#11aef4;box-shadow:inset 0 0 0 3px rgba(255,255,255,.25)}
.gb-start,.gb-finish{flex:0 0 auto;background:#ffd21e;color:#fff;border:3px solid #fff;border-radius:14px;padding:12px 10px;font-weight:1000;letter-spacing:.5px;writing-mode:vertical-rl;text-orientation:mixed;text-shadow:0 1px 1px rgba(0,0,0,.25)}
.gb-finish{writing-mode:horizontal-tb;border-radius:40px;background:#ffd21e;color:#fff;padding:14px 20px;clip-path:polygon(50% 0%,61% 28%,91% 18%,72% 45%,100% 55%,69% 62%,82% 92%,52% 75%,22% 96%,31% 64%,0 55%,29% 44%,12% 16%,40% 29%)}
.gb-cell{flex:0 0 74px;height:58px;border:3px solid #fff;border-radius:12px;display:flex;align-items:center;justify-content:center;color:white;font-weight:1000;text-shadow:0 1px 2px rgba(0,0,0,.35);font-size:13px;box-shadow:0 3px 0 rgba(0,0,0,.12)}
.gb-official-list{font-size:12px;line-height:1.35;opacity:.84;margin-top:8px;columns:2;column-gap:20px}
.gb-card-title{font-size:22px;font-weight:900;margin-bottom:4px}
...
# OFFICIAL 18-GATE DISPLAY ONLY.
# Backend helper/debug gate names stay hidden; the Game Board renders only the locked Blender terminology.
OFFICIAL_BLENDER_GATES = [
    "Gate 0 — Slate / Attack Side Lock",
    "Gate 1 — PDF Pool Integrity",
    "Gate 2 — No Empty Bat / Lineup Viability",
    "Gate 3 — Pitcher HR Vulnerability",
    "Gate 4 — Batter HR Type Match",
    "Gate 5 — Pull-Air / Launch Window",
    "Gate 6 — Hard-Hit Support Validation",
    "Gate 7 — Pitch-Type Kill Switch",
    "Gate 8 — Zone / Slot Weakness Match",
    "Gate 9 — Opportunity / Lineup Role",
    "Gate 10 — Event Ownership Isolation",
    "Gate 10.5 — Adjacent / Decoy Transfer",
    "Gate 11 — Bullpen Continuation",
    "Gate 12 — Game Script / Chaos Check",
    "Gate 13 — Numerology Overlay",
    "Gate 14 — WHO / Chaos Risk",
    "Gate 15 — Finisher Gate",
    "Gate 16 — Final Lock / No-Fluke Audit",
]

_GATE_STEP_TO_OFFICIAL = {
    0: OFFICIAL_BLENDER_GATES[1],
    1: OFFICIAL_BLENDER_GATES[0],
    2: OFFICIAL_BLENDER_GATES[2],
    3: OFFICIAL_BLENDER_GATES[3],
    4: OFFICIAL_BLENDER_GATES[5],
    5: OFFICIAL_BLENDER_GATES[15],
    6: OFFICIAL_BLENDER_GATES[16],
    7: OFFICIAL_BLENDER_GATES[8],
    8: OFFICIAL_BLENDER_GATES[5],
    9: OFFICIAL_BLENDER_GATES[10],
    10: OFFICIAL_BLENDER_GATES[9],
    11: OFFICIAL_BLENDER_GATES[10],
    12: OFFICIAL_BLENDER_GATES[17],
}

_GAMEBOARD_COLORS = ["#ffd21e", "#0b84ff", "#ee2737", "#ff7a1a", "#873bc7", "#06a63a"]

def _official_gate_name(row):
    try:
        step = int(float(row.get("step", -1)))
    except Exception:
        step = -1
    return _GATE_STEP_TO_OFFICIAL.get(step, f"Gate {step} — Blender Validation")


def _render_snake_path(rows, compact=False):
    rows = rows.sort_values("step") if "step" in rows.columns else rows
    items = []
    for i, (_, gr) in enumerate(rows.iterrows()):
        official = _official_gate_name(gr)
        result = str(gr.get("result", ""))
        cut = result != "PASS"
        color = "#ee2737" if cut else _GAMEBOARD_COLORS[i % len(_GAMEBOARD_COLORS)]
        label = official.split("—", 1)[0].replace("Gate", "G").strip()
        title = official.replace('"', '&quot;')
        items.append(f'<div class="gb-cell" title="{title}" style="background:{color}">{label}</div>')
    if not items:
        items = ["<div class='gb-cell' style='background:#ffd21e'>START</div>"]
    finish = "<div class='gb-finish'>LOCKED</div>" if not compact else ""
    return f'<div class="gb-track"><div class="gb-start">START</div>{"".join(items)}{finish}</div>'
...
def _path_card(player, rows, owner_row=None):
    ...
    game = r.get("game_key", "")
    pitcher = r.get("pitcher", "")
    status = "LOCKED OWNER" if str(role) not in {"CUT", "NO PICK", "NO PLAY", ""} else "CUT"
    badges = "".join(_gate_badge(gr) for _, gr in rows.iterrows())
    track = _render_snake_path(rows)
    st.markdown(f"""
<div class="ticket-card">
  <div class="score-pill">{status}</div>
  <div class="gb-card-title">{name}</div>
  <div style="opacity:.88;font-weight:800">{role} · {game}</div>
...
def game_board_grid_view(results, key_prefix="gb_locked"):
    ...
    st.markdown('<div class="gb-official-list">' + "".join([f"<div>{g}</div>" for g in OFFICIAL_BLENDER_GATES]) + '</div>', unsafe_allow_html=True)
