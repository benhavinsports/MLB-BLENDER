from engine import run_ticket_view, csv_bytes
...
def board_card_v108(r):
    ...
    flags = flags if flags else "{lane}"
    st.markdown(f"""
...
def _card_shell_v109(r, compact=False):
    ...
    flags = str(r.get("hard_fails","")).strip() or str(r.get("soft_fails","")).strip() or "{lane}"
...
# -------------------- v143 HARD OVERRIDE GAME BOARD RENDERER --------------------
# This overrides older card renderers so board_lane_summary(row) never prints as literal text.
...
def _board_card_html(row):
    lane = board_lane_summary(row)
    role = str(row.get("event_role") or row.get("official_core_role") or "Audit")
    player = str(row.get("player", ""))
    team = str(row.get("team", ""))
    pitcher = str(row.get("pitcher", ""))
    archetype = str(row.get("archetype", row.get("event_note", "")))
    soft = str(row.get("soft_gates", "")) or "—"
    cut = str(row.get("cut_gates", "")) or "—"
    trace = gate_trace_html(row)
    score = _safe_score(row)
    return f"""
    <div class='bh-tile'>
      <div class='bh-top'>
        <span class='bh-role'>{role}</span>
        <span class='bh-score'>{score:.1f}</span>
      </div>
      <div class='bh-name'>{player}</div>
      <div class='bh-game'>{team} vs {pitcher}</div>
      <div class='bh-arch'>{archetype}</div>
      <div class='bh-lane'>{lane}</div>
      {trace}
      <div class='bh-summary'><b>SOFT:</b> {soft}<br><b>CUT:</b> {cut}</div>
    </div>
    """
...
def game_board_grid_view(results, key_prefix="game_board"):
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Rows", meta.get("players", len(board)))
    c2.metric("Games", meta.get("games", meta.get("slate_games", 0)))
    c3.metric("Owners", meta.get("owners", meta.get("owners_locked", 0)))
    c4.metric("Core Legs", meta.get("core", meta.get("core_legs", 0)))
    ...
    <style>
    .bh-board{display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:14px}
    .bh-tile{background:linear-gradient(135deg,#08080a,#171717);border:2px solid #3d8cff;border-radius:18px;padding:14px;color:white;box-shadow:0 0 16px #000;margin-bottom:14px}
    .bh-top{display:flex;justify-content:space-between;align-items:center}
    .bh-role{border:1px solid #fff;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:900}
    .bh-score{font-size:24px;font-weight:950;color:#FFD166}
    .bh-name{font-size:24px;font-weight:950;margin-top:8px}
    .bh-game{font-size:14px;opacity:.9;margin-top:2px}
    .bh-arch{font-size:13px;font-weight:900;color:#8ec5ff;margin:8px 0}
    .bh-lane{font-size:13px;font-weight:900;color:#FFD166;margin-bottom:8px}
    .bh-path{display:flex;flex-wrap:wrap;gap:6px;margin:8px 0}
    .bh-chip{display:inline-flex;gap:5px;border:1px solid;border-radius:999px;padding:4px 7px;font-size:11px;font-weight:800}
    .bh-summary{font-size:12px;line-height:1.35;margin-top:8px;background:#0008;border-radius:10px;padding:8px}
    </style>
    <div class='bh-board'>
    ...
    for _, row in filtered.head(80).iterrows():
        st.markdown(_board_card_html(row), unsafe_allow_html=True)
...
# -------------------- v144 TICKET TIMEFRAME + SGP UI --------------------
def tickets_view(results, key_prefix="tickets_v144"):
    st.markdown("## 🎟️ Tickets — Blender Only")
    if not isinstance(results, dict):
        st.warning("Run the Blender first.")
        return
    c1, c2, c3 = st.columns([1.2, 1, 1])
    timeframe = c1.selectbox("Slate window", ["Full slate", "Early games", "Late games", "Custom window"], key=f"{key_prefix}_timeframe")
    custom_start = custom_end = None
    if timeframe == "Custom window":
        custom_start = c2.text_input("Start", "1:00 PM", key=f"{key_prefix}_start")
        custom_end = c3.text_input("End", "4:00 PM", key=f"{key_prefix}_end")
    else:
        c2.caption("Early = before 4 PM ET")
        c3.caption("Late = 4 PM ET and after")

    filtered = run_ticket_view(results, timeframe, custom_start, custom_end)
    core, alt, chaos, sgp = filtered.get("core", pd.DataFrame()), filtered.get("alt", pd.DataFrame()), filtered.get("chaos", pd.DataFrame()), filtered.get("sgp", pd.DataFrame())
    meta = filtered.get("meta", {})
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Window", meta.get("ticket_timeframe", timeframe))
    m2.metric("Rows", meta.get("ticket_rows", 0))
    m3.metric("Core Legs", len(core) if core is not None else 0)
    m4.metric("SGP Spots", len(sgp) if sgp is not None else 0)

    def _ticket_table(title, df):
        st.markdown(f"### {title}")
        if df is None or df.empty:
            st.info("No Blender-qualified legs in this window.")
            return
        cols = [c for c in ["official_core_role","event_role","player","team","pitcher","game","score","archetype","board_lane_label","soft_gates","cut_gates"] if c in df.columns]
        st.dataframe(df[cols], use_container_width=True, hide_index=True)
        st.download_button(f"Download {title} CSV", csv_bytes(df), f"{title.lower().replace(' ','_')}.csv", "text/csv", key=f"{key_prefix}_{title}_download")

    _ticket_table("Core 3", core)
    _ticket_table("Alt 3", alt)
    _ticket_table("Chaos 3", chaos)

    st.markdown("### SGP Environment Finder")
    if sgp is None or sgp.empty:
        st.info("No Blender SGP environment identified in this window.")
    else:
        st.dataframe(sgp, use_container_width=True, hide_index=True)
        st.download_button("Download SGP CSV", csv_bytes(sgp), "blender_sgp.csv", "text/csv", key=f"{key_prefix}_sgp_download")
...
def _v149_safe(row, key, default=""):
    try:
...
def board_lane_summary(row):
    lane = str(_v149_safe(row, "board_lane_label", "") or _v149_safe(row, "lane_note", ""))
    return lane if lane and lane.lower() != "nan" else "Gate memory active"

def _v149_gate_chips(row):
    raw = str(_v149_safe(row, "gate_trace_full", ""))
    if not raw or raw.lower() == "nan":
        return "<div class='v149-empty'>No gate trace available</div>"
    chips = []
    for piece in raw.split("|"):
        piece = piece.strip()
        if ":" not in piece:
            continue
        label, status = piece.rsplit(":", 1)
        status = status.strip().upper()
        color = "#7CFF6B" if status == "PASS" else "#FFD166" if status == "SOFT" else "#FF6B6B"
        chips.append(f"<span class='v149-chip' style='border-color:{color};color:{color}'>{label.strip()} <b>{status}</b></span>")
    return "<div class='v149-path'>" + "".join(chips) + "</div>"

def _v149_tile(row):
    role = str(_v149_safe(row, "event_role", "") or _v149_safe(row, "official_core_role", "") or "Audit")
    color = _v149_role_color(role)
    try:
        score = float(_v149_safe(row, "score", 0) or 0)
    except Exception:
        score = 0.0
    player = str(_v149_safe(row, "player", ""))
    team = str(_v149_safe(row, "team", ""))
    pitcher = str(_v149_safe(row, "pitcher", ""))
    game = str(_v149_safe(row, "game", "")) or f"{team} vs {pitcher}"
    arch = str(_v149_safe(row, "archetype", _v149_safe(row, "event_note", "")))
    lane = board_lane_summary(row)
    soft = str(_v149_safe(row, "soft_gates", "")) or "—"
    cut = str(_v149_safe(row, "cut_gates", "")) or "—"
    passed = str(_v149_safe(row, "pass_gates", "")) or "—"
    return f"""
    <div class="v149-tile" style="--role:{color};">
      <div class="v149-head"><span class="v149-role">{role}</span><span class="v149-score">{score:.1f}</span></div>
      <div class="v149-player">{player}</div>
      <div class="v149-game">{game}</div>
      <div class="v149-arch">{arch}</div>
      <div class="v149-lane">{lane}</div>
      <div class="v149-prog"><span>START</span><em>→</em><span>POOL</span><em>→</em><span>WEAK SLOT</span><em>→</em><span>ADJ</span><em>→</em><span>WHO</span><em>→</em><span>FINISHER</span><em>→</em><span>OWNER</span></div>
      {_v149_gate_chips(row)}
      <div class="v149-audit"><b>PASS:</b> {passed}<br><b>SOFT:</b> {soft}<br><b>CUT:</b> {cut}</div>
    </div>
    """
...
def _v149_style():
    st.markdown("""
    <style>
    .v149-hero{border:2px solid #3434ff;border-radius:22px;padding:18px;margin-bottom:16px;background:radial-gradient(circle at top left,#1a1a4e,#050507 55%);color:white;box-shadow:0 0 22px #000}
    .v149-hero h2{margin:0;font-size:28px;font-weight:950}
    .v149-hero p{margin:6px 0 0;opacity:.85}
    .v149-board{display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:16px}
    .v149-tile{background:linear-gradient(135deg,#07070a,#171719 58%,#111);border:2px solid var(--role);border-radius:20px;padding:15px;color:white;box-shadow:0 0 18px #000}
    .v149-head{display:flex;justify-content:space-between;align-items:center}
    .v149-role{border:1px solid var(--role);color:var(--role);border-radius:999px;padding:5px 11px;font-size:12px;font-weight:950;text-transform:uppercase}
    .v149-score{font-size:27px;font-weight:950;color:var(--role)}
    .v149-player{font-size:24px;font-weight:950;margin-top:9px}
    .v149-game{font-size:13px;opacity:.88;margin-top:4px}
    .v149-arch{font-size:12px;color:#8ec5ff;font-weight:900;margin-top:7px}
    .v149-lane{font-size:13px;color:#FFD166;font-weight:950;margin-top:8px}
    .v149-prog{display:flex;flex-wrap:wrap;gap:5px;align-items:center;margin-top:10px;font-size:10px;font-weight:950;color:#e6e6e6}
    .v149-prog span{border:1px solid #555;border-radius:999px;padding:3px 6px;background:#0008}
    .v149-prog em{font-style:normal;color:var(--role)}
    .v149-path{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}
    .v149-chip{display:inline-flex;gap:6px;align-items:center;border:1px solid;border-radius:999px;padding:4px 8px;font-size:11px;font-weight:850;background:#0009}
    .v149-audit{font-size:12px;line-height:1.35;margin-top:10px;background:#0009;border:1px solid #333;border-radius:12px;padding:9px}
    .v149-audit b{color:#FFD166}
    </style>
    """, unsafe_allow_html=True)

def game_board_grid_view(results, key_prefix="game_board_v149"):
    st.markdown("<div class='v149-hero'><h2>🧩 Blender Game Board</h2><p>START → Pool → Weak Slot → Adjacent → WHO → Finisher → Event Isolation → OWNER.</p></div>", unsafe_allow_html=True)
    _v149_style()
    if not isinstance(results, dict):
        st.warning("Run the Blender first.")
        return
    ...
    if board is None or board.empty:
        ...
        return
    m1,m2,m3,m4 = st.columns(4)
    m1.metric("Players", meta.get("players", len(board)))
    m2.metric("Games", meta.get("games", 0))
    m3.metric("Owners", meta.get("owners", len(results.get("owners", pd.DataFrame())) if isinstance(results.get("owners"), pd.DataFrame) else 0))
    m4.metric("Core", meta.get("core", len(results.get("core", pd.DataFrame())) if isinstance(results.get("core"), pd.DataFrame) else 0))
    c1,c2 = st.columns([2,1])
    q = c1.text_input("Search player/team/pitcher", "", key=f"{key_prefix}_search")
    role_col = "event_role" if "event_role" in board.columns else "official_core_role"
    roles = ["All"] + (sorted([str(x) for x in board[role_col].dropna().unique()]) if role_col in board.columns else [])
    role = c2.selectbox("Role lane", roles, key=f"{key_prefix}_role")
    filtered = board.copy()
    if q:
        filtered = filtered[filtered.astype(str).apply(lambda c: c.str.contains(q, case=False, na=False)).any(axis=1)]
    if role != "All" and role_col in filtered.columns:
        filtered = filtered[filtered[role_col].astype(str).eq(role)]
    if "score" in filtered.columns:
        filtered = filtered.sort_values("score", ascending=False)
    st.markdown("<div class='v149-board'>", unsafe_allow_html=True)
    for _, row in filtered.head(120).iterrows():
        st.markdown(_v149_tile(row), unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    with st.expander("Full PASS / SOFT / CUT audit table", expanded=False):
        cols = [c for c in ["game","game_time","player","team","pitcher","event_role","official_core_role","archetype","score","board_lane_label","pass_gates","soft_gates","cut_gates","gate_trace_full","tags","hr_pa","dmg","hpi","line_drive_pct","cond_pct","hr_edge_pct","pitch_edge_pct"] if c in filtered.columns]
        st.dataframe(filtered[cols], use_container_width=True, height=540, hide_index=True)
    st.download_button("Download Game Board CSV", csv_bytes(filtered), "game_board.csv", "text/csv", key=f"{key_prefix}_download")
...
def tickets_view(results, key_prefix="tickets_v149"):
    ...
    if not isinstance(results, dict):
        ...
        return
    c1,c2,c3 = st.columns([1.2,1,1])
    timeframe = c1.selectbox("Slate window", ["Full slate","Early games","Late games","Custom window"], key=f"{key_prefix}_tf")
    custom_start = custom_end = None
    if timeframe == "Custom window":
        custom_start = c2.text_input("Start", "1:00 PM", key=f"{key_prefix}_start")
        custom_end = c3.text_input("End", "4:00 PM", key=f"{key_prefix}_end")
        ...
        c3.caption("Late = 4 PM ET and after")
    filtered = run_ticket_view(results, timeframe, custom_start, custom_end)
    core = filtered.get("core", pd.DataFrame()); alt = filtered.get("alt", pd.DataFrame()); chaos = filtered.get("chaos", pd.DataFrame()); sgp = filtered.get("sgp", pd.DataFrame())
    a,b,c,d = st.columns(4)
    a.metric("Window", timeframe); b.metric("Core Legs", len(core)); c.metric("Alt Legs", len(alt)); d.metric("SGP Spots", len(sgp))
    def table(title, df):
        st.markdown(f"### {title}")
        if df is None or df.empty:
            st.info("No Blender-qualified legs in this window.")
            return
        cols = [x for x in ["official_core_role","event_role","player","team","pitcher","game","game_time","score","archetype","board_lane_label","soft_gates","cut_gates"] if x in df.columns]
        st.dataframe(df[cols], use_container_width=True, hide_index=True)
        st.download_button(f"Download {title} CSV", csv_bytes(df), f"{title.lower().replace(' ','_')}.csv", "text/csv", key=f"{key_prefix}_{title}_download")
    table("Core 3", core); table("Alt 3", alt); table("Chaos 3", chaos)
    st.markdown("### SGP Environment Finder")
    if sgp is None or sgp.empty:
        st.info("No Blender SGP environment identified in this window.")
    else:
