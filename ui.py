...
# ============================================================
# RUNTIME COMPAT FIX
# Older Tickets UI calls run_ticket_view(), but V147 shipped without it.
# This prevents NameError and keeps tickets rendering from the actual results.
# ============================================================
def run_ticket_view(results, bucket="core", window="Full slate"):
    if not isinstance(results, dict):
        return pd.DataFrame()
    df = results.get(bucket, pd.DataFrame())
    if df is None or not hasattr(df, "empty"):
        return pd.DataFrame()
    df = df.copy()
    # Window filter only if slate_window column exists.
    if window and window != "Full slate" and "slate_window" in df.columns:
        df = df[df["slate_window"].astype(str).str.lower() == str(window).lower()]
    # Only display true ticket rows.
...
def game_board_view(results, key_prefix="gb"):
    ...
    if owners is not None and not owners.empty:
        ...
        st.download_button("Download Game Board CSV", csv_bytes(owners), "game_board.csv", "text/csv", key=str(f"{unique}_owners")+"_"+str(len(owners)))
    if survivors is not None and not survivors.empty:
        ...
        st.download_button("Download Survivors CSV", csv_bytes(survivors), "survivors.csv", "text/csv", key=str(f"{unique}_survivors")+"_"+str(len(survivors)))
...
def compact_game_board_view(results, key_prefix="gb_compact"):
    ...
    st.download_button("Download Compact Game Board CSV", csv_bytes(compact), "compact_game_board.csv", "text/csv", key=str(f"{unique}_compact")+"_"+str(len(compact)))
    st.download_button("Download Full Game Board CSV", csv_bytes(board), "full_game_board.csv", "text/csv", key=str(f"{unique}_full")+"_"+str(len(board)))
...
def game_board_grid_view(results, key_prefix="gb_grid_v108"):
    ...
    st.download_button("Download Game Board CSV", csv_bytes(filtered), "game_board.csv", "text/csv", key=str(f"{key_prefix}_download")+"_"+str(len(filtered)))
...
def game_board_grid_view(results, key_prefix="gb_v109"):
    ...
    st.download_button("Download Game Board CSV", csv_bytes(board), "game_board.csv", "text/csv", key=str(f"{key_prefix}_download")+"_"+str(len(board)))
...
def tickets_view(results, key_prefix="tickets_v144"):
    ...
    if sgp is None or sgp.empty:
        ...
        st.download_button("Download SGP CSV", csv_bytes(sgp), "blender_sgp.csv", "text/csv", key=str(f"{key_prefix}_sgp_download")+"_"+str(len(sgp)))


# ============================================================
# V147_SAFE_TICKETS_OVERRIDE_FINAL
# Fixes NameError from missing run_ticket_view and renders tickets safely.
# ============================================================
def run_ticket_view(results, bucket="core", window="Full slate"):
    if not isinstance(results, dict):
        return pd.DataFrame()
...
def tickets_view(results, key_prefix="tickets_v147_safe"):
    st.markdown("## 🎟️ Tickets")
    ...
    for label, key in [("CORE 3","core"),("ALT 3","alt"),("CHAOS 3","chaos")]:
        ...
        if df.empty:
            ...
            for _, r in df.iterrows():
                card(r)
            st.download_button(f"Download {label}", csv_bytes(df), f"{label.lower().replace(' ','_')}.csv", "text/csv", key=f"{key_prefix}_{key}_{window}_download_{len(df)}")

def game_board_grid_view(results, key_prefix="gb_v147_safe"):
    st.markdown("## 🧩 GAME BOARD — SYNCED")
    if not isinstance(results, dict):
        st.info("Run the Blender first.")
        return
    meta = results.get("meta", {}) or {}
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Rows", meta.get("players", meta.get("input_rows", 0)))
    c2.metric("Games", meta.get("games", 0))
    c3.metric("Owners", meta.get("owners_locked", 0))
    c4.metric("Survivors", meta.get("survivor_rows", 0))
    if meta.get("message"):
        st.info(meta.get("message"))
    owners = results.get("owners", pd.DataFrame())
    role_board = results.get("role_board", pd.DataFrame())
    game_board = results.get("game_board", pd.DataFrame())
    survivors = results.get("survivors", pd.DataFrame())
    if isinstance(owners, pd.DataFrame) and not owners.empty:
        st.markdown("### Locked Owners")
        cols=[c for c in ["game","game_key","player","team","pitcher","official_core_role","archetype","score","gate_grade","hard_fails","soft_fails"] if c in owners.columns]
        st.dataframe(owners[cols], use_container_width=True, hide_index=True)
    else:
        st.warning("No locked owners.")
    if isinstance(role_board, pd.DataFrame) and not role_board.empty:
        st.markdown("### Role Outcome Memory")
        st.dataframe(role_board, use_container_width=True, hide_index=True)
    if isinstance(game_board, pd.DataFrame) and not game_board.empty:
        st.markdown("### Gate Trace Memory")
        st.dataframe(game_board, use_container_width=True, hide_index=True)
    if isinstance(survivors, pd.DataFrame) and not survivors.empty:
        with st.expander("Full Survivor Audit", expanded=False):
            cols=[c for c in ["game","game_key","player","team","pitcher","official_core_role","archetype","score","gate_grade","hard_fails","soft_fails","gate_path"] if c in survivors.columns]
