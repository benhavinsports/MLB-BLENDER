...
def blender_machine(names):
    ...
    st.markdown(f"""<div class="machine"><div class="blender-wrap"><div class="feed-slot">CLICK / FEED DATA HERE</div><div class="jar"><div class="blade"></div><div class="center">READY</div>{floats}</div><div class="output-slot">BLADES SPIN → OWNERS LOCK → TICKETS POP OUT</div></div></div>""", unsafe_allow_html=True)
...
def recap_view(results):
    st.markdown("<div class='ticket'><h3>2AM EASTERN AUTO RECAP</h3><p>Run this automatically from Streamlit scheduler/cron at 2:00 AM America/New_York. It checks Game Board owners against MLB home run results and logs calibration.</p></div>", unsafe_allow_html=True)
    if st.button("RUN RECAP NOW"):
        recap, meta = run_recap_check(results)
        st.write(meta)
        if recap is not None and not recap.empty:
            st.dataframe(recap[["player","team","pitcher","score","official_core_role","archetype","hit_hr"]], use_container_width=True)
            st.download_button("Download Recap CSV", csv_bytes(recap), "recap.csv", "text/csv")
        else:
            st.info("No owners to recap yet.")
