...
/* v95 visual-only team card layer */
@keyframes v95ElitePulse {
  from { transform: translateY(0); filter: brightness(1); }
  to { transform: translateY(-2px); filter: brightness(1.18); }
}
.v95-player-card{border-radius:28px;padding:28px;margin:22px 0;color:#FFF4E0;}
.v95-chip-row{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:18px;}
.v95-chip{border:1px solid;border-radius:999px;padding:8px 14px;font-weight:900;letter-spacing:.3px;}
.v95-player-name{font-family:Impact,Haettenschweiler,'Arial Black',sans-serif;font-size:clamp(42px,8vw,82px);line-height:.95;letter-spacing:1px;margin:12px 0;}
.v95-matchup{font-size:20px;color:#E8DFD0;font-weight:700;margin:12px 0;}
.v95-score{font-family:Impact,Haettenschweiler,'Arial Black',sans-serif;font-size:clamp(58px,12vw,112px);line-height:1;margin-top:18px;}
.v95-score-label{font-weight:900;color:#BFB7AA;margin-top:8px;}
.v95-bar{height:14px;background:#242424;border-radius:999px;overflow:hidden;margin:10px 0 20px;}
.v95-bar div{height:100%;border-radius:999px;}
.v95-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0;}
.v95-metrics div{border:1px solid #333;border-radius:16px;padding:14px;background:#0d0d0dcc;}
.v95-metrics b{display:block;font-size:24px;color:#FFF4E0;}
.v95-metrics span{display:block;color:#aaa;margin-top:6px;}
.v95-path,.v95-hard,.v95-soft{font-size:16px;line-height:1.55;color:#EFE7D5;font-weight:700;margin-top:12px;}
.v95-hard{color:#ff9a9a;}
.v95-soft{color:#ffe28a;}
...
# -------------------- v95 SAFE TEAM COLOR VISUALS --------------------
TEAM_COLOR_PAIRS_V95 = {
    ...
    "Chicago White Sox": ("#C4CED4", "#27251F"),
    "Chicago Cubs": ("#0E3386", "#CC3433"),
    "Cincinnati Reds": ("#C6011F", "#000000"),
    "Cleveland Guardians": ("#E31937", "#00385D"),
    ...
    "Milwaukee Brewers": ("#FFC52F", "#12284B"),
...
def _v95_team_colors(team):
    team = str(team or "").strip()
    if team in TEAM_COLOR_PAIRS_V95:
        return TEAM_COLOR_PAIRS_V95[team]
    low = team.lower()
    for name, pair in TEAM_COLOR_PAIRS_V95.items():
        if low and (low in name.lower() or name.lower() in low):
            return pair
    return ("#B7FF4A", "#F4A326")

def _v95_contrast(hex_color):
    try:
...
def _v95_elite(score):
    try:
        return float(score) >= 78
    except Exception:
        return False
...
def card(r):
    team = r.get("team", "")
    primary, secondary = _v95_team_colors(team)
    try:
        score = float(r.get("score", 0) or 0)
    except Exception:
        score = 0.0
    elite = _v95_elite(score)
    text_color = _v95_contrast(primary)
    glow = f"0 0 26px {secondary}, 0 0 10px {primary}" if elite else "none"
    anim = "v95ElitePulse 1.25s infinite alternate" if elite else "none"
    fire = "🔥 " if elite else ""
    ...
    st.markdown(f"""
    <div class="v95-player-card" style="
        border: 2px solid {secondary};
        box-shadow: {glow};
        animation: {anim};
        background:
            radial-gradient(circle at top left, {primary}55 0%, transparent 36%),
            linear-gradient(135deg, #101010 0%, #141414 48%, {primary}30 100%);
    ">
        <div class="v95-chip-row">
            <span class="v95-chip" style="background:{primary}; color:{text_color}; border-color:{secondary};">{role}</span>
            <span class="v95-chip" style="background:#111; color:{secondary}; border-color:{primary};">{archetype}</span>
        </div>
        <div class="v95-player-name" style="color:{text_color}; text-shadow:0 0 10px {primary};">{fire}{player}</div>
        <div class="v95-matchup"><span style="color:{secondary}; font-weight:900;">{team}</span> · {game} · vs {pitcher} · Slot {slot}</div>
        <div class="v95-score" style="color:{secondary};">{score:.1f}</div>
        <div class="v95-score-label">TRUE BLEND SCORE</div>
        <div class="v95-bar"><div style="width:{min(100,max(0,score))}%; background:linear-gradient(90deg,{primary},{secondary});"></div></div>
        <div class="v95-metrics">
        ...
        <div class="v95-path"><b>Blend path:</b> {gate_path}</div>
        {f'<div class="v95-hard"><b>Hard:</b> {hard}</div>' if hard else ''}
        {f'<div class="v95-soft"><b>Soft:</b> {soft}</div>' if soft else ''}
    </div>
    """, unsafe_allow_html=True)
