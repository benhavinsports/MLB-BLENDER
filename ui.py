...
def inject_css():
    st.markdown("""
<style>
:root{--cream:#f5eadc;--lime:#b7ff32;--line:rgba(255,255,255,.10);}
.stApp{background:#020202;color:var(--cream);}
.block-container{max-width:1120px!important;padding:1rem!important;}
.big-title{font-size:64px;font-weight:1000;line-height:.9;letter-spacing:2px;margin:14px 0 18px;}
.big-title span{color:var(--lime);}
[data-testid="stTabs"] button{font-size:18px!important;font-weight:900!important;color:var(--cream)!important;}
[data-testid="stTabs"] [aria-selected="true"]{color:#ff6b6b!important;border-bottom-color:#ff6b6b!important;}
.stButton button{border-radius:18px!important;min-height:56px!important;font-weight:950!important;border:1px solid rgba(255,255,255,.12)!important;white-space:normal!important;}
...
.start-upload-card [data-testid="stFileUploader"]{margin-top:12px;background:rgba(0,0,0,.08)!important;padding:8px!important;border-radius:18px!important;}
.start-upload-card [data-testid="stFileUploader"] section{background:rgba(255,255,255,.22)!important;border:1px solid rgba(0,0,0,.18)!important;border-radius:16px!important;}
.start-upload-card [data-testid="stFileUploader"] button{background:#050505!important;color:#fff!important;border-radius:12px!important;font-weight:950!important;}
.public-board{background:linear-gradient(180deg,#080c14,#030406);border:1px solid var(--line);border-radius:22px;padding:16px;box-shadow:0 24px 60px rgba(0,0,0,.56);margin:12px 0 18px;}
.board-head{display:flex;align-items:center;gap:10px;margin-bottom:12px;}
.board-mark{width:36px;height:36px;border-radius:12px;border:1px solid var(--line);display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.05);}
...
.board-stat{background:rgba(7,11,18,.82);border:1px solid var(--line);border-radius:12px;padding:8px 10px;font-weight:900;text-align:center;}
.board-stat b{color:var(--lime);font-size:20px;}
.board-note{color:#aab3c1;font-size:13px;margin-bottom:10px;}
.game-tile-wrap button{width:100%!important;min-height:70px!important;border-radius:16px!important;color:#fff!important;font-size:11px!important;line-height:1.08!important;box-shadow:0 10px 22px rgba(0,0,0,.36)!important;}
.tile-blue button{background:linear-gradient(160deg,#2563eb,#1d4ed8)!important;}
.tile-red button{background:linear-gradient(160deg,#dc2626,#991b1b)!important;}
.tile-orange button{background:linear-gradient(160deg,#f97316,#c2410c)!important;}
.tile-purple button{background:linear-gradient(160deg,#9333ea,#6b21a8)!important;}
.tile-green button{background:linear-gradient(160deg,#22c55e,#15803d)!important;}
.selected-game-panel{background:rgba(7,11,18,.88);border:1px solid var(--line);border-radius:16px;padding:14px;margin-top:12px;}
.owner-row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:12px;}
.owner-card{background:rgba(8,13,22,.88);border:1px solid var(--line);border-radius:16px;padding:14px;min-height:126px;}
...
.owner-role{font-size:12px;font-weight:900;color:var(--cream);}
...
.big-title{font-size:50px;margin-bottom:18px;}
.start-upload-title{font-size:32px;}
.board-title{font-size:20px;}
.board-stats{grid-template-columns:repeat(3,1fr);}
.game-tile-wrap button{min-height:58px!important;font-size:10px!important;border-radius:14px!important;}
    ...
    if isinstance(feed, pd.DataFrame) and not feed.empty:
        for key_col in ["game_pk","official_game_pk","game_id"]:
            if key_col in feed.columns:
                temp = feed.copy()
                temp[key_col] = temp[key_col].astype(str).replace({"nan":"","None":""})
                vals = [v for v in temp[key_col].dropna().unique().tolist() if str(v).strip()]
                if vals and len(vals) <= 30:
                    for v in vals:
                        row = temp[temp[key_col].astype(str)==str(v)].iloc[0].to_dict()
...
def _pick_row(results, key, fallback_index=0, selected_game=""):
    df = _df(results.get(key)) if isinstance(results, dict) else pd.DataFrame()
    if not df.empty:
        if selected_game and "game_key" in df.columns:
            sub = df[df["game_key"].astype(str).str.contains(str(selected_game), case=False, regex=False, na=False)]
            if not sub.empty:
...
""", unsafe_allow_html=True)
    ...
    color_classes = ["tile-blue","tile-red","tile-orange","tile-purple","tile-green"]
    for start in range(0, len(games), cols_per_row):
        ...
        for j in range(cols_per_row):
            ...
            g = games[i]
            with cols[j]:
                st.markdown(f'<div class="game-tile-wrap {color_classes[i % len(color_classes)]}">', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    core_row = _pick_row(results, "core", 0, selected)
    alt_row = _pick_row(results, "alt", 1, selected)
    who_row = _pick_row(results, "chaos", 2, selected)
...
def game_board_grid_view(results, key_prefix="gb_locked"):
    ...
    if board.empty or "player" not in board.columns:
        st.info("No private gate trace loaded.")
    else:
