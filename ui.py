...
def card(r, extra=""):
    ...
    gate_power = r.get("confidence_label", f"Gate Power {r.get('blender_score', r.get('score', 0))}")
    decision = r.get("decision_state", "LOCKED OWNER" if bool(r.get("blender_eligible", True)) else "CUT")
    arch = r.get("archetype", "")
    time = r.get("game_time_et", "")
    st.markdown(f"""
<div class="ticket-card">
  <div class="score-pill">{decision}</div>
  ...
  <div style="opacity:.65;font-size:13px;margin-top:4px">{gate_power}</div>
...
def _gate_badge(row):
    ...
    score = row.get("score", "")
    return f'<span class="role-pill {cls}"><b>{gate}</b><br>{result} · {score}<br><span style="opacity:.72">{reason[:70]}</span></span>'
...
def _path_card(player, rows, owner_row=None):
    ...
    game = r.get("game_key", "")
    pitcher = r.get("pitcher", "")
    gate_power = r.get("confidence_label", f"Gate Power {r.get('blender_score', '')}")
    elig = bool(r.get("blender_eligible", False)) if "blender_eligible" in getattr(r, "index", []) else False
    status = "LOCKED" if elig else "CUT"
