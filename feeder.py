...
# ============================================================
# V147_GAMECOUNT_METRIC_REPAIR
# Repairs Star Tool parsed output after original parser:
# - derives true slate_game_key by matchup, not teams/pitchers count
# - maps metric synonyms into engine names
# - removes non-player/header fallback rows
# ============================================================
def repair_feed_after_parse_v147(df):
    ...
    alias_map = {
        "pull_pct": ["pull_pct","pull%","pull","Pull","PULL"],
        "hard_hit_pct": ["hard_hit_pct","hardhit%","hard_hit%","hard hit%","hh%","HH%","HardHit"],
        "barrel_pct": ["barrel_pct","barrel%","brl%","barrel","Barrel"],
        "sweet_spot_pct": ["sweet_spot_pct","sweet%","sweet spot","launch","launch_angle","la"],
        "dmg": ["dmg","damage","ult","ultimate","adj","adjusted","DMG","ULT","Adj"],
        "hpi": ["hpi","HPI","model","rating","score"],
        "hr_pa": ["hr_pa","hr/pa","hr_rate","hr rate","hr9","hr/9","pitcher_hr9","hr_lane"],
        "pitch_edge": ["pitch_edge","pitch edge","edge","Pitch Edge"],
    ...
    for target, names in alias_map.items():
        if target not in df.columns or pd.to_numeric(df.get(target), errors="coerce").notna().sum() == 0:
            c = find_col(names)
            if c is not None:
                df[target] = df[c].apply(to_num)

    # Remove obvious non-player rows.
    if "player" in df.columns:
        p = df["player"].astype(str).str.strip()
        bad = (
            p.eq("") |
            p.str.lower().isin(["player","hitter","batter","name","pitcher","team","game"]) |
            p.str.contains("download|page|copyright|disclaimer|pull%|hard hit|barrel", case=False, na=False)
        )
        df = df[~bad].copy()

    # Derive a true game key.
    if "game_key" not in df.columns:
        df["game_key"] = ""
    if "game" in df.columns:
        g = df["game"].astype(str).str.strip()
        df.loc[g.ne("") & g.ne("nan"), "game_key"] = g[g.ne("") & g.ne("nan")]
    # If original game_key is weak, build from teams or team vs pitcher.
    weak = df["game_key"].astype(str).str.strip().isin(["", "nan", "TEAM vs OPP", " vs "])
    if "team" in df.columns and "opponent" in df.columns:
        df.loc[weak, "game_key"] = df.loc[weak, "team"].astype(str).str.strip() + " vs " + df.loc[weak, "opponent"].astype(str).str.strip()
    weak = df["game_key"].astype(str).str.strip().isin(["", "nan", " vs "])
    if "team" in df.columns and "pitcher" in df.columns:
        df.loc[weak, "game_key"] = df.loc[weak, "team"].astype(str).str.strip() + " vs " + df.loc[weak, "pitcher"].astype(str).str.strip()

    metric_cols = ["pull_pct","hard_hit_pct","barrel_pct","sweet_spot_pct","dmg","hpi","hr_pa","pitch_edge","lineup_slot"]
    for c in metric_cols:
        if c not in df.columns:
            df[c] = None
        df[c] = pd.to_numeric(df[c], errors="coerce")
    df["metric_count"] = df[metric_cols].notna().sum(axis=1)
    df["parser_status"] = df["metric_count"].apply(lambda x: "METRIC_READY" if x >= 3 else "WEAK_PARSE")

    return df.reset_index(drop=True)

_ORIGINAL_READ_FEED_V147_FIX = read_feed
def read_feed(filename, data):
    df, audit = _ORIGINAL_READ_FEED_V147_FIX(filename, data)
    df = repair_feed_after_parse_v147(df)
    audit = audit if isinstance(audit, dict) else {}
    audit["repair_layer"] = "V147_GAMECOUNT_METRIC_REPAIR"
    audit["rows_after_repair"] = int(len(df))
    audit["metric_ready_rows"] = int((df["metric_count"].fillna(0) >= 3).sum()) if "metric_count" in df else 0
    audit["game_count_after_repair"] = int(df["game_key"].nunique()) if "game_key" in df else 0
    return df, audit
