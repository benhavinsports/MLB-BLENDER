...
CANON = [
"page","game","team","pitcher","player","lineup_slot","pull_pct","barrel_pct","sweet_spot_pct",
...
def normalize_structured(df, source):
    ...
    for c in ["game","team","pitcher","player"]:
        df[c] = df[c].fillna("").astype(str).str.strip()
    df["team"] = df["team"].apply(lambda x: normalize_team(x) or x)
    df = df[df["player"].apply(is_player_name)].copy()
    fake_player_re = r"^(Low Effort|Medium Effort|High Effort|Effort|Fresh|Moderate|Elevated|Hot|Cold|Today|Home|Away)$"
