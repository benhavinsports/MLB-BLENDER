...
# ============================================================
# SMART PARSER V5 — strongest stable parser layer
# Goal: never let PDF name-only rows pretend to be full data.
# Extracts tables, text rows, key/value metrics, and validates coverage.
# ============================================================
...
_METRIC_ALIASES_V5 = {
    ...
    "game": ["game","matchup"],
    "lineup_slot": ["lineup","slot","order","bo"],
    "pull_pct": ["pull%","pull pct","pull_pct","pull"],
    "hard_hit_pct": ["hardhit%","hard hit%","hard_hit%","hh%","hh"],
    "barrel_pct": ["barrel%","barrel pct","brl%","barrel"],
    "sweet_spot_pct": ["sweet%","sweet spot","launch","la"],
    "dmg": ["dmg","damage","ult","ultimate","adj","adjusted"],
    "hpi": ["hpi","hr score","model","rating"],
    "hr_lane": ["hr/pa","hr_pa","hr9","hr/9","pitcher hr9","hr lane"],
...
def _sp_num(x):
    try:
        if x is None or _pd.isna(x): return _np.nan
        s = str(x).replace("%","").replace("+","").replace(",","").strip()
        if s.lower() in {"","nan","none","null","-","—"}: return _np.nan
        return float(s)
    except Exception:
        return _np.nan
...
def _sp_txt(x):
    try:
        if x is None or _pd.isna(x): return ""
    except Exception:
        pass
    return str(x).strip()

def _sp_find_col(df, aliases):
    compact = {str(c).lower().replace(" ","").replace("_","").replace("%","").replace("/",""): c for c in df.columns}
    for a in aliases:
        aa = a.lower().replace(" ","").replace("_","").replace("%","").replace("/","")
        if aa in compact: return compact[aa]
    for a in aliases:
        aa = a.lower().replace(" ","").replace("_","").replace("%","").replace("/","")
        for cc, orig in compact.items():
            if aa in cc or cc in aa:
                return orig
    return None

def normalize_blender_feed_v5(df):
    if df is None or len(df) == 0:
        return _pd.DataFrame()
    raw = df.copy()
    raw.columns = [str(c).strip() for c in raw.columns]
    out = _pd.DataFrame()
    for target, aliases in _METRIC_ALIASES_V5.items():
        col = _sp_find_col(raw, aliases)
        out[target] = raw[col] if col is not None else ""
    for c in ["player","team","opponent","pitcher","game","notes"]:
        out[c] = out[c].apply(_sp_txt)
    for c in ["lineup_slot","pull_pct","hard_hit_pct","barrel_pct","sweet_spot_pct","dmg","hpi","hr_lane","pitch_edge"]:
        out[c] = out[c].apply(_sp_num)
    ...
    # Build game key
    blank_game = out["game"].astype(str).str.strip().eq("")
    out.loc[blank_game, "game"] = (
        out.loc[blank_game, "team"].replace("", "TEAM").astype(str) + " vs " +
        out.loc[blank_game, "opponent"].replace("", "OPP").astype(str)
    )
    out["game_key"] = out["game"]
    weak = out["game_key"].astype(str).str.strip().isin(["", "TEAM vs OPP", " vs "])
    out.loc[weak, "game_key"] = (
        out.loc[weak, "team"].replace("", "TEAM").astype(str) + " vs " +
        out.loc[weak, "pitcher"].replace("", "PITCHER").astype(str)
    )

    metric_cols = ["pull_pct","hard_hit_pct","barrel_pct","sweet_spot_pct","dmg","hpi","hr_lane","pitch_edge","lineup_slot"]
    out["metric_count"] = out[metric_cols].notna().sum(axis=1)
    out["parser_status"] = out["metric_count"].apply(lambda x: "FULL_METRICS" if x >= 5 else ("PARTIAL_METRICS" if x >= 3 else "NAME_ONLY_OR_WEAK"))
    return out.reset_index(drop=True)

def _parse_metric_from_line(line, key_patterns):
    for pat in key_patterns:
        m = _re.search(pat, line, _re.I)
        if m:
            return _sp_num(m.group(1))
    return _np.nan

def text_to_rows_v5(text):
    lines = [x.strip() for x in str(text).splitlines() if x.strip()]
    rows = []
    current_game, current_pitcher, current_team = "", "", ""
    for line in lines:
        low = line.lower()
        if (_re.search(r"\bvs\b|@", line, _re.I) and len(line) < 100):
            current_game = line
            continue
        if "pitcher" in low or "probable" in low or "starter" in low:
            if len(line) < 120: current_pitcher = line
        # Try structured row with metrics
        pull = _parse_metric_from_line(line, [r"pull[:\s]+([+-]?\d+\.?\d*)", r"pull%[:\s]+([+-]?\d+\.?\d*)"])
        hard = _parse_metric_from_line(line, [r"hh[:\s]+([+-]?\d+\.?\d*)", r"hard.?hit[:\s]+([+-]?\d+\.?\d*)"])
        barrel = _parse_metric_from_line(line, [r"barrel[:\s]+([+-]?\d+\.?\d*)", r"brl[:\s]+([+-]?\d+\.?\d*)"])
        dmg = _parse_metric_from_line(line, [r"dmg[:\s]+([+-]?\d+\.?\d*)", r"ult[:\s]+([+-]?\d+\.?\d*)"])
        hpi = _parse_metric_from_line(line, [r"hpi[:\s]+([+-]?\d+\.?\d*)"])
        edge = _parse_metric_from_line(line, [r"edge[:\s]+([+-]?\d+\.?\d*)"])
        hr = _parse_metric_from_line(line, [r"hr/?9[:\s]+([+-]?\d+\.?\d*)", r"hr_lane[:\s]+([+-]?\d+\.?\d*)"])

        # Extract probable name: first 2-3 words before metric labels
        if len(line.split()) >= 2 and not _re.search(r"copyright|page \d+|disclaimer|pull%|hard hit%|barrel%", low):
            name_part = _re.split(r"\s+(pull|hh|hard|barrel|brl|dmg|ult|hpi|edge|hr/9|hr9)\b", line, flags=_re.I)[0].strip()
    ...
    return normalize_blender_feed_v5(_pd.DataFrame(rows))
...
def read_pdf_v5(raw):
        ...
        with pdfplumber.open(path) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    header = [_sp_txt(x) for x in table[0]]
                    for row in table[1:]:
                        rec = {}
    ...
    try:
        ...
        for page in doc:
            text += "\n" + page.get_text("text")
        return text_to_rows_v5(text)
    except Exception as e:
        raise ValueError(f"PDF parser failed: {e}")

# Override original read_feed with stronger version.
def read_feed(name, raw):
    lname = str(name).lower()
    import io
    if lname.endswith(".csv"):
        return normalize_blender_feed_v5(_pd.read_csv(io.BytesIO(raw))), {"source":"csv_v5"}
    if lname.endswith((".xlsx",".xls")):
        return normalize_blender_feed_v5(_pd.read_excel(io.BytesIO(raw))), {"source":"excel_v5"}
    if lname.endswith(".pdf"):
        return read_pdf_v5(raw), {"source":"pdf_v5"}
    if lname.endswith((".txt",".md")):
        return text_to_rows_v5(raw.decode("utf-8", errors="ignore")), {"source":"text_v5"}
