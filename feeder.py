...
# === ENGINE ONLY REPAIR FEEDER HOOK ===
from engine_only_repair import normalize_feed_engine_only, true_game_count as _repair_game_count
_PRE_REPAIR_READ_FEED = read_feed
def read_feed(filename, data):
    df, audit = _PRE_REPAIR_READ_FEED(filename, data)
    df = normalize_feed_engine_only(df)
    audit = audit if isinstance(audit, dict) else {}
    audit["engine_only_repair"] = True
    audit["rows_after_repair"] = int(len(df))
    audit["metric_ready_rows"] = int((df["metric_count"].fillna(0) >= 3).sum()) if "metric_count" in df.columns else 0
    audit["true_game_count"] = int(_repair_game_count(df))
    return df, audit


# ============================================================
# FINAL COUNTER FIX
# App metrics import these helpers from feeder.py in some builds.
# Force them to use repaired true game count, not stale/raw counters.
# ============================================================
try:
    from engine_only_repair import true_game_count as _final_true_game_count
except Exception:
    _final_true_game_count = None

def actual_game_count(df):
    if _final_true_game_count:
        return _final_true_game_count(df)
    if isinstance(df, pd.DataFrame) and "game_key" in df.columns:
        s = df["game_key"].astype(str).str.strip()
        s = s[(s!="") & (s!="nan")]
