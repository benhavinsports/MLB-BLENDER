...
def game_participants(game_key: str) -> set[str]:
    g = _txt(game_key)
    if not g:
        return set()
    teams = TEAM_RE.findall(g)
    if len(teams) >= 2:
        return {teams[0].lower(), teams[1].lower()}
    if " vs " in g:
        return {x.strip().lower() for x in g.split(" vs ")[:2] if x.strip()}
    return set()


def team_fits_game(team: str, game_key: str) -> bool:
    parts = game_participants(game_key)
    if not parts:
        return True
    t = _txt(team).lower()
    return bool(t and t in parts)
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d{1,2}:\d{2}\s*[AP]M\s*ET$", line, re.I):
            ...
            if prev_abbr and next_abbr:
                ...
                new_game = f"{current_away} vs {current_home}"
                if new_game != current_game:
                    # Hard reset row context so a player name cannot inherit the previous game's team/pitcher.
                    current_team = ""
                    current_pitcher = ""
                    current_side_team_abbr = ""
...
def parse_player_chunk(player, hand, rank, chunk, team, pitcher, game_key, game_time_et) -> dict:
    ...
    if game_key and team and not team_fits_game(team, game_key):
        notes = (notes + " DATA_BINDING_TEAM_GAME_MISMATCH").strip()
        team = ""
    return {
...
def clean_feed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "binding_status" not in out.columns: out["binding_status"] = "PDF_BOUND"
    mismatch = []
    for idx, r in out.iterrows():
        if _txt(r.get("team")) and _txt(r.get("game_key")) and not team_fits_game(r.get("team"), r.get("game_key")):
            out.at[idx, "binding_status"] = "TEAM_GAME_MISMATCH_TEAM_BLANKED"
            out.at[idx, "notes"] = (_txt(r.get("notes")) + " DATA_BINDING_TEAM_GAME_MISMATCH").strip()
            out.at[idx, "team"] = ""
            mismatch.append(True)
        else:
            mismatch.append(False)
    out.loc[out["metric_count"] == 0, "parser_status"] = "NAME_ONLY"
    out.loc[pd.Series(mismatch, index=out.index), "parser_status"] = "BINDING_MISMATCH_NEEDS_REVIEW"
...
def read_feed(filename, data):
    ...
    audit = {"filename": str(filename), "parser_version": "V149_FAST_PDF_CACHE"}
    if name.endswith(".csv"):
        ...
        # SPEED RULE: Star Tool PDFs are text-first. pdfplumber table extraction is the slowest path.
        # Only fall back to table extraction when text parsing finds almost nothing.
        if len(df_text) >= 5:
            audit["table_rows"] = "skipped_fast_text_parser"
            df = df_text
        else:
