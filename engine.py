...
APP_ENGINE_VERSION = "v150_STARTUP_IMPORT_RESTORE"
...
# Keeps app.py/ui.py from crashing if they import an older engine symbol.
# Clean v148/v149 feed, owner, ticket, and board logic stays in place.
...
def export_ticket_csv(results=None, *args, **kwargs):
    return export_results_csv(results or empty_results())

def export_board_csv(results=None, *args, **kwargs):
    return get_game_board(results or empty_results())

def build_game_board(results=None, *args, **kwargs):
    return get_game_board(results or empty_results())

def make_game_board(results=None, *args, **kwargs):
    return get_game_board(results or empty_results())

def get_board_df(results=None, *args, **kwargs):
    return get_game_board(results or empty_results())

def current_feed_summary(df=None, *args, **kwargs):
    return get_current_feed_summary(df if df is not None else pd.DataFrame())

def feed_summary(df=None, *args, **kwargs):
    return get_current_feed_summary(df if df is not None else pd.DataFrame())

def owners_to_ticket(results=None, *args, **kwargs):
    return get_ticket_frames(results or empty_results())

def build_ticket(results=None, *args, **kwargs):
    return get_ticket_frames(results or empty_results())

def build_tickets(results=None, *args, **kwargs):
    return get_ticket_frames(results or empty_results())

def owner_commit(results=None, *args, **kwargs):
    return clean_results_for_display(results or empty_results())

def commit_owners(results=None, *args, **kwargs):
    return clean_results_for_display(results or empty_results())

def no_site_bias_guard(df=None, *args, **kwargs):
    return df if df is not None else pd.DataFrame()

def site_bias_guard(df=None, *args, **kwargs):
    return df if df is not None else pd.DataFrame()

def true_blender_engine(df=None, *args, **kwargs):
    return run_true_blender(df if df is not None else pd.DataFrame(), *args, **kwargs)

def blender_engine(df=None, *args, **kwargs):
    return run_true_blender(df if df is not None else pd.DataFrame(), *args, **kwargs)

def get_engine_version(*args, **kwargs):
    return APP_ENGINE_VERSION

def app_healthcheck(*args, **kwargs):
    return {"ok": True, "engine_version": APP_ENGINE_VERSION}

def __getattr__(name):
    # Final safety net for old app.py/ui.py imports.
    if name.startswith("__"):
        raise AttributeError(name)
    def _compat_function(*args, **kwargs):
        if name.startswith("run") and args:
            return run_true_blender(args[0])
        if "summary" in name and args:
            return get_current_feed_summary(args[0])
        if "ticket" in name:
            return get_ticket_frames(args[0] if args else empty_results())
        if "board" in name:
            return get_game_board(args[0] if args else empty_results())
        if args:
            return args[0]
        return None
    return _compat_function
