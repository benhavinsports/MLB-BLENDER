...
def run_true_blender(df):
    ...
    if owners.empty:
        # FALLBACK OWNER RECOVERY ENGINE
        if not survivors.empty:
            ...
            for g, sg in survivors.groupby(group_field, dropna=False):
                ...
                if "killed_by_gate" in sg.columns:
                    sg["gate_penalty"] = sg["killed_by_gate"].astype(str).apply(lambda x: 15 if x and x != "nan" else 0)
                else:
                    sg["gate_penalty"] = 0

                sg["fallback_score"] = sg["score"] - sg["gate_penalty"]

                pick = sg.sort_values("fallback_score", ascending=False).iloc[0].to_dict()
                pick["official_core_role"] = "FALLBACK_OWNER"
                fallback_rows.append(pick)

            owners = pd.DataFrame(fallback_rows)

        meta["message"] = "Fallback owner recovery activated. Hard gates were too strict, so Blender preserved the strongest remaining owner from each game."

    owners=owners.sort_values("score",ascending=False).drop_duplicates(subset=["player","team","pitcher"],keep="first").reset_index(drop=True)
    core,alt,chaos=build_tickets_from_owners(owners,survivors)
    meta["message"] = f"Blender complete: {len(owners)} owners locked from {meta['games']} games. Tickets + Game Board synced successfully."
