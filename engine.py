...
def _extract_team_name(x):
    if isinstance(x, dict):
        for key in ["name", "teamName", "clubName", "fullName", "abbreviation"]:
            v = x.get(key)
            if v:
                return str(v)
        if "team" in x:
            return _extract_team_name(x.get("team"))
        return ""
    if x is None:
        return ""
    return str(x)
...
def team_abbr(team):
    raw = _extract_team_name(team).strip()
    if not raw:
        return "UNK"
    u = raw.upper().strip()
    if u in ABBR_TEAM:
        return u
    # handle full names regardless of case
    for name, abbr in TEAM_ABBR.items():
        if raw.lower() == name.lower():
            return abbr
    # handle common city/name strings
    for name, abbr in TEAM_ABBR.items():
        if name.lower() in raw.lower() or raw.lower() in name.lower():
            return abbr
    cleaned = re.sub(r"[^A-Z]", "", u)[:3]
    return cleaned or "UNK"

def canonical_game_key(team, opponent=None, pitcher=None, game=None):
    """Robust one true game id. Never crashes on dict/API objects."""
    t1 = team_abbr(team)
    text = _extract_team_name(game) if isinstance(game, dict) else str(game or "")
    opp_raw = _extract_team_name(opponent).strip()

    if not opp_raw and " vs " in text:
        left, right = text.split(" vs ", 1)
        if team_abbr(right) != "UNK":
            opp_raw = right
    if not opp_raw and " @ " in text:
        left, right = text.split(" @ ", 1)
        if team_abbr(right) != "UNK":
            opp_raw = right

    t2 = team_abbr(opp_raw) if opp_raw else ""
    if t2 and t2 != "UNK" and t2 != t1:
        a,b = sorted([t1,t2])
        return f"{a}_{b}"
    p = re.sub(r"[^A-Za-z]", "", str(pitcher or ""))[:10].upper()
...
def fetch_live_public_slate(date_iso=None):
    """Load public MLB schedule/probable pitchers without crashing the app."""
    if date_iso is None:
    ...
    try:
        ...
        for d in data.get("dates", []):
            for g in d.get("games", []):
                ...
                away = _extract_team_name(away_obj)
                home = _extract_team_name(home_obj)
                away_p = _extract_team_name((teams.get("away") or {}).get("probablePitcher") or {})
                home_p = _extract_team_name((teams.get("home") or {}).get("probablePitcher") or {})
                game_pk = g.get("gamePk", "")
                game_txt = f"{away} vs {home}"
                game_key = canonical_game_key(away, home, home_p, game_txt)
                rows.append({"game_pk":game_pk,"game_key":game_key,"game":game_txt,"team":away,"opponent":home,"pitcher":home_p,"player":"","source":"PUBLIC_SLATE"})
                rows.append({"game_pk":game_pk,"game_key":game_key,"game":game_txt,"team":home,"opponent":away,"pitcher":away_p,"player":"","source":"PUBLIC_SLATE"})
        df = pd.DataFrame(rows)
        if not df.empty:
            df = normalize_game_frame(df)
        return df, {"source":"MLB Stats API", "date":date_iso, "games": df["game_key"].nunique() if not df.empty and "game_key" in df else 0}
    except Exception as e:
        return pd.DataFrame(), {"source":"MLB Stats API", "date":date_iso, "error":str(e)}

def merge_public_context(feed_df, public_df):
    # Public slate is context only; never create fake player rows from it.
