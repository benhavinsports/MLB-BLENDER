...
def empty_results(message):
    return {'owners':pd.DataFrame(),'core':pd.DataFrame(),'alt':pd.DataFrame(),'chaos':pd.DataFrame(),'survivors':pd.DataFrame(),'cuts':pd.DataFrame(),'game_board':pd.DataFrame(),'role_board':pd.DataFrame(),'environment_board':pd.DataFrame(),'state_log':pd.DataFrame(),'meta':{'engine_version':'V164_TRUE_EVENT_OWNER_ENGINE','message':message,'owners_locked':0,'core_count':0,'passed_rows':0,'cut_rows':0,'core_rule':'CORE_3_TOP_TRUE_EVENT_OWNERS_NO_SCORE_CLUSTER'}}
...
def lock_attack_sides(df):
    ...
    for gid,gdf in df.groupby('_gid',dropna=False):
        ...
        rows.append({'game_id':str(gid),'game_key':_txt(gdf.iloc[0].get('game_key') or gdf.iloc[0].get('game')),'locked_attack_side':team,'attack_score':round(float(score),2),'candidate_rows':int(count),'engine_rule':'ATTACK SIDE CONTEXT ONLY - NOT HARD KILL'})
...
def evaluate_candidate(row,locks):
    ...
    attack_pass=bool(locked and team==locked); add_gate(gates,1,'Attack-side lock',attack_pass,8 if attack_pass else 0,8,f'locked={locked}; team={team}',True)
    ...
    score=round(max(1,min(96.7,14+(pass_depth*1.85)+(quality*14)+(event*3.1)+(lane*.75)+(launch*.55)+(conversion*.7)+fingerprint-(cut_depth*8.5))),1)
...
def resolve_owners(survivors):
    ...
    for gid,g in passed.groupby('game_id',dropna=False):
        ...
        # TRUE owner isolation only
        g['_owner_score']=(
            g['event_ownership'].fillna(0)*1000+
            g['pass_depth'].fillna(0)*100+
            g['support_score'].fillna(0)
        )

        g=g.sort_values(
            ['_owner_score','elimination_score'],
            ascending=[False,False]
        )

        # HARD isolate single owner
        survivor=g.iloc[0].to_dict()
        survivor['resolved_owner']=True
    ...
    # ROLES BUILT AFTER OWNER ISOLATION
    def owner_role(row):
        who=row.get('role_WHO_score',0)
        adj=row.get('role_Adjacent_score',0)
        primary=row.get('role_Primary_score',0)

        if who > primary and who > adj:
            return 'WHO'
        if adj > primary:
            return 'Adjacent'
        return 'Primary'

    owners['official_core_role']=owners.apply(owner_role,axis=1)

    owners['_rank']=(
        owners['event_ownership'].fillna(0)*1000+
        owners['pass_depth'].fillna(0)*100+
        owners['elimination_score'].fillna(0)
    )

    return owners.sort_values(
        '_rank',
        ascending=False
...
def run_true_blender(df,*args,**kwargs):
    ...
    meta={'engine_version':'V164_TRUE_EVENT_OWNER_ENGINE','games':int(games),'input_rows':int(len(work)),'passed_rows':int((survivors['blender_eligible']==True).sum()),'cut_rows':int((survivors['blender_eligible']!=True).sum()),'owners_locked':int(len(owners)),'core_count':int(len(core)),'core_rule':'CORE_3_TOP_TRUE_EVENT_OWNERS_NO_SCORE_CLUSTER','core_slots':core.get('core_slot',pd.Series(dtype=str)).tolist() if not core.empty else [],'generic_refill':False,'fallback_sorting':False,'role_recycling':False,'attack_side_hard_kill':True,'message':f"V164 true event-owner engine: Core={len(core)} from {len(owners)} owners. Pass rows={int((survivors['blender_eligible']==True).sum())}; cuts={int((survivors['blender_eligible']!=True).sum())}."}
    results={'owners':owners,'core':core,'alt':alt,'chaos':chaos,'survivors':survivors,'cuts':cuts,'game_board':game_board,'role_board':role_board,'environment_board':environment_board,'state_log':pd.DataFrame([{'rule':'TRUE_EVENT_OWNER_ENGINE','no_score_cluster':True,'attack_side_context_not_hard_kill':True,'generic_refill':False,'fallback_sorting':False,'role_recycling':False}]),'meta':meta}
