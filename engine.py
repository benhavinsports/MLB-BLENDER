...
# === ENGINE ONLY REPAIR RUN HOOK ===
from engine_only_repair import true_blender_run, true_game_count as _repair_true_game_count
def actual_game_count(df): return _repair_true_game_count(df)
def attack_pool_count(df): return _repair_true_game_count(df)
def slate_game_count_from_public_context(ctx=None, df=None): return _repair_true_game_count(df if df is not None else ctx)
def run_true_blender(df, *args, **kwargs):
    results = true_blender_run(df)
    try:
...
# ============================================================
# OFFICIAL MLB SLATE PULL HOOK
# Uses MLB StatsAPI for real slate games/times/probable pitchers.
# ============================================================
try:
    from official_mlb_slate import fetch_official_mlb_slate as _official_fetch_slate
    from official_mlb_slate import attach_official_slate_to_feed as _official_attach_slate
    from official_mlb_slate import official_game_count as _official_game_count
except Exception:
    _official_fetch_slate = None
    _official_attach_slate = None
    _official_game_count = None

def fetch_live_public_slate(date_str=None):
    if _official_fetch_slate is None:
        return pd.DataFrame(), {"source":"OFFICIAL MLB unavailable", "games":0}
    return _official_fetch_slate(date_str)

def attach_slate_matchup_context(df, public_context=None):
    # public_context is now official MLB slate context.
    if _official_attach_slate is not None and isinstance(public_context, pd.DataFrame) and not public_context.empty:
        return _official_attach_slate(df, public_context)
    return df
...
def slate_game_count_from_public_context(ctx=None, df=None):
    target = ctx if ctx is not None else df
    if _official_game_count is not None:
        return _official_game_count(target)
    return actual_game_count(target)

# Keep hitter pool from official slate as context rows only, not hitter picks.
def fetch_live_public_hitter_pool(date_str=None):
    slate, meta = fetch_live_public_slate(date_str)
    # These are team/game context rows. User still uploads Star Tool PDF for hitter metrics.
    return slate, meta
