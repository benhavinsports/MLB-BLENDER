...
HR_HISTORY_PATH = DATA_DIR / "daily_hr_history.csv"
RUN_HISTORY_PATH = DATA_DIR / "blender_run_history.csv"
ENGINE_VERSION = "V0216_ORIGINAL_FEEDER_TRUE_DEATH_CHAIN_DAILY_LEARNING"
_DAILY_LEARNING: Dict[str, float] = {}
...
# -----------------------------------------------------------------------------
# Daily learning / recap calibration
# -----------------------------------------------------------------------------
...
def _signature_from_values(role: str, lane: float, launch: float, conversion: float, slot: float) -> str:
    def band(x: float, cuts=(0, 3, 6, 9)) -> str:
        x = _num(x, 0)
        if x >= cuts[3]: return "ELITE"
        if x >= cuts[2]: return "STRONG"
        if x >= cuts[1]: return "LIVE"
        if x > 0: return "THIN"
        return "NONE"
    slot_val = _num(slot, 0)
    if 1 <= slot_val <= 3: slot_band = "TOP3"
    elif 4 <= slot_val <= 6: slot_band = "MID"
    elif slot_val >= 7: slot_band = "BOTTOM"
    else: slot_band = "UNK"
    return f"{role}|LANE_{band(lane)}|LAUNCH_{band(launch)}|CONV_{band(conversion)}|SLOT_{slot_band}"


def _candidate_signature(cand: Dict[str, Any]) -> str:
    return _signature_from_values(
        _txt(cand.get("true_role_path") or cand.get("official_core_role") or "Primary"),
        _num(cand.get("lane_index"), 0),
        _num(cand.get("launch_index"), 0),
        _num(cand.get("conversion_index"), 0),
        _num(cand.get("slot"), 0),
    )


def load_daily_learning() -> Dict[str, float]:
    """Outcome memory uses event-profile signatures, not recycled names."""
    try:
        if not HR_HISTORY_PATH.exists():
            return {}
        hist = pd.read_csv(HR_HISTORY_PATH)
        if hist.empty or "signature" not in hist.columns:
            return {}
        counts = hist["signature"].value_counts().to_dict()
        total = max(1, sum(counts.values()))
        # Small bounded boost; never overrides hard death gates.
        return {str(k): min(0.75, (v / total) * 4.0) for k, v in counts.items()}
    except Exception:
        return {}


def _append_run_history(results: Dict[str, Any]) -> None:
    try:
        DATA_DIR.mkdir(exist_ok=True)
        core = results.get("core")
        if not isinstance(core, pd.DataFrame) or core.empty:
            return
        rows = core.copy()
        rows["run_timestamp_et"] = _today_et_date().isoformat(timespec="seconds")
        rows["engine_version"] = ENGINE_VERSION
        rows["signature"] = rows.apply(lambda r: _candidate_signature(r.to_dict()), axis=1)
        keep = [c for c in ["run_timestamp_et","engine_version","core_slot","player","team","game_key","pitcher","official_core_role","archetype","signature","data_binding_id"] if c in rows.columns]
        old = pd.read_csv(RUN_HISTORY_PATH) if RUN_HISTORY_PATH.exists() else pd.DataFrame()
        pd.concat([old, rows[keep]], ignore_index=True).tail(3000).to_csv(RUN_HISTORY_PATH, index=False)
    except Exception:
        pass


def update_daily_learning(date_str: str | None = None) -> Dict[str, Any]:
    """Pull completed MLB HR events from StatsAPI and store archetype-ready recap rows.

    This is safe to call from Streamlit. If network/API fails, it returns an error without breaking the Blender.
    """
    try:
        dt = datetime.strptime(date_str, "%Y-%m-%d") if date_str else (_today_et_date() - timedelta(days=1)).replace(tzinfo=None)
        date_s = dt.strftime("%Y-%m-%d")
        sched = requests.get("https://statsapi.mlb.com/api/v1/schedule", params={"sportId":1,"date":date_s}, timeout=15)
        ...
        for g in games:
            ...
            if not pk: continue
            try:
                feed = requests.get(f"https://statsapi.mlb.com/api/v1.1/game/{pk}/feed/live", timeout=15)
                feed.raise_for_status()
                data = feed.json()
                away = (((data.get("gameData") or {}).get("teams") or {}).get("away") or {}).get("name", "")
                home = (((data.get("gameData") or {}).get("teams") or {}).get("home") or {}).get("name", "")
                game_key = f"{away} vs {home}" if away and home else str(pk)
                for play in (((data.get("liveData") or {}).get("plays") or {}).get("allPlays") or []):
                    if ((play.get("result") or {}).get("event") or "").lower() == "home run":
                        batter = ((play.get("matchup") or {}).get("batter") or {}).get("fullName", "")
                        team = (((play.get("team") or {}).get("name")) or "")
                        rows.append({"date":date_s,"game_pk":pk,"game_key":game_key,"player":batter,"team":team,"event":"HR","signature":"UNKNOWN_RECAP_ONLY"})
            except Exception:
                continue
        new = pd.DataFrame(rows)
        DATA_DIR.mkdir(exist_ok=True)
        old = pd.read_csv(HR_HISTORY_PATH) if HR_HISTORY_PATH.exists() else pd.DataFrame()
        if not new.empty:
            all_rows = pd.concat([old, new], ignore_index=True).drop_duplicates(subset=["date","game_pk","player","event"], keep="last")
            all_rows.tail(10000).to_csv(HR_HISTORY_PATH, index=False)
        return {"ok": True, "date": date_s, "games_checked": len(games), "home_runs_logged": int(len(new)), "history_rows": int(len(pd.read_csv(HR_HISTORY_PATH))) if HR_HISTORY_PATH.exists() else 0}
...
def _candidate_base(row: Dict[str, Any], locks: Dict[str, str], idx: int = 0) -> Dict[str, Any]:
    ...
    # Daily learning is profile-based only. It never revives a cut player and never changes the locked game binding.
    signature = _signature_from_values(role, lane, launch, conversion, v.get("slot", 0))
    learning_delta = _DAILY_LEARNING.get(signature, 0.0)
    owner_key = round(raw_power + learning_delta + fingerprint / 1000000, 6)
    ...
    return {
        **row,
        ...
        "learning_delta": round(float(learning_delta), 6),
        "true_role_path": role,
...
def run_true_blender(df: pd.DataFrame, *args, **kwargs) -> Dict[str, Any]:
    ...
    _DAILY_LEARNING = load_daily_learning()
    ...
    role_board = survivors[[c for c in ["data_binding_id","binding_status","original_game_key","original_team","game_id","game_key","team","player","official_core_role","true_role_path","role_scores_json","pass_depth","cut_depth","death_step","lane_index","launch_index","conversion_index","support_index","learning_signature","learning_delta"] if c in survivors.columns]].copy()
    ...
    meta = {
        ...
        "daily_learning_profiles_loaded": int(len(_DAILY_LEARNING)),
        "message": f"{ENGINE_VERSION}: Core={len(core)} from {len(owners)} isolated game owners. Original feeder mapping preserved. Daily learning profiles={len(_DAILY_LEARNING)}. No 99-score engine, no refill, no projection fallback.",
    }
    results = {
        "owners": owners,
        "core": core,
