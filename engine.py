...
# UPLOAD_SYNC_BOARD_MEMORY_PATCH
# Build synced board memory from the same survivors/owners used for tickets.
_BASE_RUN_UPLOAD_SYNC = run_true_blender

def _usb_safe_df(x):
    return x if isinstance(x, pd.DataFrame) else pd.DataFrame()

def _usb_board_from_results(results):
    results = safe_results(results) if 'safe_results' in globals() else results
    owners = _usb_safe_df(results.get("owners"))
    survivors = _usb_safe_df(results.get("survivors"))

    board_src = survivors if not survivors.empty else owners
    gate_rows = []
    role_rows = []
    if not board_src.empty:
        for _, r in board_src.iterrows():
            role = str(r.get("official_core_role", ""))
            game = r.get("game_key", r.get("game", ""))
            player = r.get("player", "")
            gate_path = str(r.get("gate_path", ""))
            if gate_path:
                parts = [p.strip() for p in gate_path.split("|") if p.strip()]
                for i, p in enumerate(parts):
                    verdict = "PASS"
                    if "KILL" in p.upper() or "NO PLAY" in p.upper():
                        verdict = "KILL"
                    elif "WEAK" in p.upper():
                        verdict = "WEAK"
                    gate_rows.append({
                        "game_key": game,
                        "player": player,
                        ...
                        "gate": p[:90],
                        "verdict": verdict,
                        "blender_score": r.get("score", r.get("blender_score", 0)),
                        "reason": p
                    })
            else:
                gate_rows.append({
                    "game_key": game, "player": player, "role": role,
                    "gate_step": 0, "gate": "Board memory", "verdict": "PASS",
                    "blender_score": r.get("score", r.get("blender_score", 0)),
                    "reason": str(r.get("archetype",""))
                })
            role_rows.append({
                "game_key": game,
                "player": player,
...
def run_true_blender(df, *args, **kwargs):
    results = _BASE_RUN_UPLOAD_SYNC(df, *args, **kwargs)
    results = _usb_board_from_results(results)
    try:
