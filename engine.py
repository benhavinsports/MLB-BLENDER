...
def rebuild_live_blender_feed(upload_df=None, public_context=None, use_live_pool=False, use_live_metrics=False):
    ...
    # Do NOT hang the app on web metric pulls during normal uploaded-feed runs.
    if use_live_metrics:
        try:
            df = enrich_with_live_batter_metrics(df)
        except Exception:
...
def run_true_blender(df, *args, **kwargs):
    """
    v87 hard reset runner.
    Always returns visible outputs:
    - owners/core if clean survivors exist
    - survivors/no-play board if hard gates kill everyone
    - meta explaining exactly what happened
    """
    ...
    try:
        ...
        # last-resort: never crash run on enrichment
        pass

    meta={"input_rows":0 if df is None else int(len(df)),"games":0,"owners_locked":0,"survivor_rows":0,"no_play_games":0,"message":""}
    if df is None or getattr(df, "empty", True):
    ...
    for game,gdf in df.groupby(group_col,dropna=False):
        try:
            ...
            # If a single game crashes, quarantine that game instead of killing app.
            q=gdf.head(1).copy()
            q["official_core_role"]="NO PLAY"
            q["archetype"]="Engine Quarantine"
            q["score"]=0
            q["gate_path"]=f"ENGINE ERROR: {e}"
            q["hard_fails"]=f"ENGINE ERROR: {e}"
            alive=q
    ...
    meta["survivor_rows"]=int(len(survivors))
    ...
    if owners.empty:
        meta["message"]="Run completed: no clean owners survived. Game Board below shows the best survivor/no-play per game and every KILL reason."
    ...
    meta["message"]=f"Run completed: {len(owners)} owners locked from {meta['games']} games."
