...
# ============================================================
# V162 TRUE CORE 3 ENGINE FIX — FINAL OVERRIDE
# Correct rule: Core 3 always comes from top 3 TRUE EVENT OWNERS.
# Roles are truthful labels only; no refill labels, no forced Adjacent/WHO.
# ============================================================

def role_path(vals: Dict[str, float], notes: str) -> Tuple[str, Dict[str, float]]:
    pull, hard, barrel, dmg, hpi, hr, edge = vals["pull"], vals["hard"], vals["barrel"], vals["dmg"], vals["hpi"], vals["hr"], vals["edge"]
    adj_trigger = any(x in notes for x in ["adjacent", "decoy", "transfer", "coverage", "weak slot", "pressure", "behind", "after"])
    who_trigger = any(x in notes for x in ["who", "chaos", "low owned", "low-owned", "random", "bottom"] ) or (pull >= 30 and hard >= 34 and hpi < 42 and barrel >= 4)
    primary = (
        gate_strength(pull, 35, 58, 22) + gate_strength(barrel, 6, 18, 22) + gate_strength(dmg, .8, 2.7, 18)
        + gate_strength(hpi, 22, 88, 16) + gate_strength(hr, .7, 2.7, 16) + gate_strength(edge, 0, 22, 12)
    )
    adjacent = (
        gate_strength(pull, 29, 50, 16) + gate_strength(hard, 31, 57, 16) + gate_strength(dmg, .7, 2.0, 14)
        + gate_strength(hr, .6, 2.0, 10) + (34 if adj_trigger else 0)
    )
    who = (
        gate_strength(pull, 28, 48, 13) + gate_strength(hard, 32, 55, 13) + gate_strength(barrel, 4, 14, 10)
        + gate_strength(hr, .6, 1.8, 12) + gate_strength(edge, 0, 14, 10) + (40 if who_trigger else 0)
    )
    scores = {"Primary": round(primary,2), "Adjacent": round(adjacent,2), "WHO": round(who,2)}
    role = max(scores, key=scores.get)
    # Truthful lane guard: don't call a player Adjacent/WHO unless that path is real.
    if role == "Adjacent" and not adj_trigger and scores["Adjacent"] < scores["Primary"] + 16:
        role = "Primary"
...
def evaluate_candidate(row: Dict[str, Any], locks: Dict[str, str]) -> Tuple[Dict[str, Any], List[Dict[str, Any]], Dict[str, float]]:
    vals = metrics(row)
    player = _txt(row.get("player")); team = _txt(row.get("team")); gid = game_id(row)
    gkey = _txt(row.get("game_key") or row.get("game")); locked_team = locks.get(str(gid), "")
    ...
    add_gate(gates,1,"Attack-side lock",bool(locked_team and team==locked_team),10 if locked_team and team==locked_team else 0,10,f"locked={locked_team}; team={team}")
    trig = pull>=28 or hard>=30 or barrel>=4 or dmg>=.6 or hpi>=15 or hr>=.6 or edge>=0
    add_gate(gates,2,"Metric survival",mc>=3 and trig,10 if mc>=3 and trig else 0,10,f"metric_count={mc}; trigger={trig}")
    lane=max(gate_strength(hr,.6,2.4,11),gate_strength(edge,0,16,11),gate_strength(dmg,.6,2.5,11),gate_strength(hpi,15,85,11))
    add_gate(gates,3,"Pitcher HR lane",lane>0,lane,11,f"hr={hr}; edge={edge}; dmg={dmg}; hpi={hpi}")
    launch=max(gate_strength(pull,28,58,13),gate_strength(sweet,18,40,13),gate_strength(barrel,4,18,13))
    add_gate(gates,4,"Pull-air launch",launch>0,launch,13,f"pull={pull}; sweet={sweet}; barrel={barrel}")
    conv=max(gate_strength(dmg,.6,2.5,13),gate_strength(barrel,4,18,13),gate_strength(hpi,15,85,13),gate_strength(hr,.6,2.5,13))
    add_gate(gates,5,"Conversion DNA",conv>0,conv,13,f"dmg={dmg}; barrel={barrel}; hpi={hpi}; hr={hr}")
    opp=6 if slot==0 or slot<=7 else 1
    add_gate(gates,6,"Lineup opportunity",opp>=5,opp,6,f"slot={slot}",hard_gate=False)
    support=max(gate_strength(hard,30,62,8),gate_strength(barrel,4,18,8),gate_strength(dmg,.6,2.5,8))
    add_gate(gates,7,"Hard-hit support",support>0,support,8,f"hard={hard}; barrel={barrel}; dmg={dmg}",hard_gate=False)
    role, role_scores = role_path(vals, notes)
    add_gate(gates,8,"Role lane",True,role_scores.get(role,0),max(1,max(role_scores.values())),f"role={role}; role_scores={role_scores}",hard_gate=False)
    trap = "trap" in notes or "red flag" in notes or "fade" in notes
    add_gate(gates,9,"Trap audit",not trap,8 if not trap else 0,8,"no trap" if not trap else "trap/fade flag")
    fin=max(gate_strength(pull,32,58,7),gate_strength(hard,30,62,7),gate_strength(barrel,4,18,7),gate_strength(dmg,.6,2.5,7),gate_strength(hpi,15,85,7),gate_strength(hr,.6,2.5,7))
    add_gate(gates,10,"Finisher strength",fin>0,fin,7,f"finisher={round(fin,2)}")
    event=(lane*.32)+(launch*.25)+(conv*.30)+(support*.12)+(fin*.20)+(role_scores.get(role,0)*.08)
    add_gate(gates,11,"Event ownership",event>=3.5,event,10,f"event_ownership={round(event,2)}")
    hard_cuts=[g for g in gates if g["cut"]]
    pass_depth=sum(1 for g in gates if g["result"]=="PASS"); cut_depth=sum(1 for g in gates if g["result"]=="CUT")
    raw_score=sum(g["score"] for g in gates); max_score=sum(g["max_score"] for g in gates)
    quality=raw_score/max(1,max_score)
    fingerprint=(sum(ord(c) for c in (player+gkey))%29)/10.0
    score=round(max(1,min(98,28 + pass_depth*3.5 + quality*28 + event*3.1 - cut_depth*10 + fingerprint)),1)
    eligible=len(hard_cuts)==0 and event>=3.5 and mc>=3
    if not eligible: score=round(min(score,54.9),1)
    final_role=role if eligible else "CUT"
    archetype={"Primary":"Primary HR Owner","Adjacent":"Adjacent / Decoy Transfer","WHO":"WHO / Chaos Owner","CUT":"Cut by gates"}[final_role]
    result={**row,"game_id":gid,"locked_attack_side":locked_team,"blender_eligible":bool(eligible),"blender_score":score,
            "support_score":round(raw_score,2),"elimination_score":score,"score":score,"official_core_role":final_role,"true_role_path":role,
            "archetype":archetype,"metric_count":mc,"pass_depth":pass_depth,"cut_depth":cut_depth,"event_ownership":round(event,2),
            "role_Primary_score":role_scores.get("Primary",0),"role_Adjacent_score":role_scores.get("Adjacent",0),"role_WHO_score":role_scores.get("WHO",0),
            "stop_gate":"" if eligible else (hard_cuts[0]["gate"] if hard_cuts else "event ownership"),
            "final_reason":f"{final_role} survived true event-owner path" if eligible else "CUT — eliminated by Blender gates",
...
def resolve_owners_state_machine(survivors: pd.DataFrame) -> pd.DataFrame:
    ...
    rows=[]; used_players=set()
    for gid,g in passed.groupby("game_id",dropna=False):
        if not _txt(gid): continue
        g=g.copy(); g["_owner"] = g["pass_depth"].fillna(0)*1000 + g["event_ownership"].fillna(0)*100 + g["support_score"].fillna(0)
        g=g.sort_values(["_owner","elimination_score"],ascending=[False,False])
        for _,r in g.iterrows():
            p=_txt(r.get("player")).lower()
            if p and p not in used_players:
                used_players.add(p); rows.append(r.drop(labels=["_owner"],errors="ignore").to_dict()); break
    if not rows: return pd.DataFrame()
    owners=pd.DataFrame(rows)
    owners["_rank"] = owners["event_ownership"].fillna(0)*100 + owners["pass_depth"].fillna(0)*10 + owners["elimination_score"].fillna(0)
    return owners.sort_values("_rank",ascending=False).drop(columns=["_rank"],errors="ignore").reset_index(drop=True)


def build_core_no_refill(owners: pd.DataFrame) -> pd.DataFrame:
    # Corrected rule: Core 3 always fills from top 3 TRUE EVENT OWNERS. Roles are truthful labels, not forced slots.
    if not isinstance(owners,pd.DataFrame) or owners.empty: return pd.DataFrame()
    core=owners.head(3).copy()
    core["core_slot"] = ["CORE 1","CORE 2","CORE 3"][:len(core)]
    core["ticket_role"] = "CORE"
    return core.reset_index(drop=True)
