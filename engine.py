...
def evaluate_18_gates(r):
    ...
    slot=safe_num(r.get('lineup_slot'), None)
    weak_slot=_slot_match_v77(r) or bool(r.get('weak_slot_tag'))
    platoon=bool(r.get('platoon')); alert=bool(r.get('hr_alert'))
    chalk=bool(r.get('chalk_trap', False))
    metrics=sum(_present_v77(r,k) for k in ['pull_pct','sweet_spot_pct','dmg','hr_pa','hpi','pitch_edge'])
    ...
    hh_support=hh>=38 or barrel>=8 or dmg>=1.5 or not _present_v77(r,'hard_hit_pct')
    ...
    script=bool(r.get('cond_up')) or bool(r.get('environment_hot')) or not _present_v77(r,'cond_up')
    ...
    return {'gates':gates,'hard_failed':hard,'soft_failed':soft,'grade':round(passed/len(gates)*100,1),
