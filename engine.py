...
def canonical_game_key(team, opponent=None, pitcher=None, game=None):
    """
    ONE true game id. If a matchup has both teams, sort both sides so
    ARI vs COL and COL vs ARI become the same game.
    If opponent is missing, use game text/pitcher fallback.
    """
    t1 = team_abbr(team)
    text = str(game or "")
    ...
    if not opp and " vs " in text:
        ...
        # If right side looks like pitcher, keep pitcher fallback. If it looks like a team, use it.
        if any(name.lower() in right.lower() for name in TEAM_ABBR):
            opp = right
    ...
    # fallback by pitcher side but stable enough to prevent same team duplicate rows exploding game counts
