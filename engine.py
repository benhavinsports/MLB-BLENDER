...
APP_ENGINE_VERSION = "v144_TICKET_TIMEFRAME_SGP"
...
# -------------------- v143 COUNT / SUMMARY FIX --------------------
def get_current_feed_summary(df):
    d = normalize_game_frame(df)
    if d is None or not hasattr(d, "empty") or d.empty:
        return {"players": 0, "games": 0, "teams": 0, "pitchers": 0, "attack_pools": 0}
    games = attack_pool_count(d)
    teams = int(d["team"].replace("", np.nan).dropna().nunique()) if "team" in d.columns else 0
    pitchers = int(d["pitcher"].replace("", np.nan).dropna().nunique()) if "pitcher" in d.columns else 0
    return {
        "players": int(len(d)),
        "games": int(games),
        "teams": teams,
        "pitchers": pitchers,
        "attack_pools": int(games),
    }
...
def actual_game_count(df):
    return int(attack_pool_count(df))

def slate_game_count_from_public_context(public_context=None, fallback_df=None):
    if fallback_df is not None:
        return int(attack_pool_count(fallback_df))
    if public_context is not None and hasattr(public_context, "empty") and not public_context.empty:
        return int(attack_pool_count(public_context))
    return 0



# -------------------- v144 TICKET TIMEFRAME + SGP ENGINE --------------------
def _time_to_minutes(value):
    try:
        if value is None or str(value).strip() == "":
            return None
        s = str(value).strip().lower().replace("et","").replace("est","").replace("edt","").strip()
        ampm = None
        ...
        if ":" in s:
            h, m = s.split(":", 1)
            h = int(re.sub(r"[^0-9]", "", h) or 0)
            m = int(re.sub(r"[^0-9]", "", m) or 0)
        else:
            h = int(re.sub(r"[^0-9]", "", s) or 0)
            m = 0
...
def _row_game_minutes(row):
    for col in ["game_time","start_time","time","first_pitch","scheduled_time","game_start"]:
        if col in row and str(row.get(col, "")).strip():
            mins = _time_to_minutes(row.get(col))
            if mins is not None:
                return mins
    txt = " ".join(str(row.get(c, "")) for c in ["game","matchup","notes","parse_note"] if c in row)
    m = re.search(r"\b(\d{1,2})(?::(\d{2}))?\s*(am|pm|ET|EDT|EST)?\b", txt, flags=re.I)
    return _time_to_minutes(m.group(0)) if m else None

def add_time_bucket(df):
    if df is None or not hasattr(df, "empty") or df.empty:
        return df
    out = df.copy()
    out["game_minutes"] = out.apply(_row_game_minutes, axis=1)
    out["time_bucket"] = out["game_minutes"].apply(
        lambda x: "Early" if pd.notna(x) and x < 16*60 else ("Late" if pd.notna(x) and x >= 16*60 else "Unknown")
    )
    return out

def filter_results_by_timeframe(results, timeframe="Full slate", custom_start=None, custom_end=None):
    res = safe_results(results)
    timeframe = str(timeframe or "Full slate")
    for key in ["owners","survivors"]:
        d = res.get(key, pd.DataFrame())
        if d is None or not hasattr(d, "empty") or d.empty:
            continue
        d = add_time_bucket(d)
        if timeframe == "Early games":
            d = d[d["time_bucket"].eq("Early")].copy()
        elif timeframe == "Late games":
            d = d[d["time_bucket"].eq("Late")].copy()
        elif timeframe == "Custom window":
            start = _time_to_minutes(custom_start)
            end = _time_to_minutes(custom_end)
            if start is not None and end is not None:
                d = d[pd.notna(d["game_minutes"]) & d["game_minutes"].between(start, end, inclusive="both")].copy()
        res[key] = d.reset_index(drop=True)
    core, alt, chaos = build_tickets_from_owners(res.get("owners", pd.DataFrame()), res.get("survivors", pd.DataFrame()))
    res["core"], res["alt"], res["chaos"] = core, alt, chaos
    if "meta" not in res or res["meta"] is None:
        res["meta"] = {}
    res["meta"]["ticket_timeframe"] = timeframe
    res["meta"]["ticket_rows"] = int(len(res.get("survivors", pd.DataFrame())))
...
def identify_sgp_candidates(results, min_game_owners=2):
    ...
    if owners is None or not hasattr(owners, "empty") or owners.empty:
        return pd.DataFrame()
    owners = add_time_bucket(owners)
    if "game" not in owners.columns:
        owners["game"] = owners.apply(lambda r: f"{r.get('team','')} vs {r.get('pitcher','')}", axis=1)
    rows = []
    for game, g in owners.groupby("game", dropna=False):
        if len(g) < min_game_owners:
            continue
        roles = set(g.get("event_role", pd.Series(dtype=str)).astype(str).tolist())
        score_sum = float(pd.to_numeric(g.get("score", 0), errors="coerce").fillna(0).sum())
        chaos = int(("WHO" in roles) or g.astype(str).apply(lambda c: c.str.contains("chaos|WHO", case=False, na=False)).any().any())
        adjacent = int("Adjacent" in roles)
        environment_score = round(score_sum / max(1, len(g)) + chaos*8 + adjacent*5, 1)
        if environment_score >= 60 or (chaos and adjacent):
            top = g.sort_values("score", ascending=False).head(4)
            rows.append({
                "game": game,
                "sgp_type": "Blender SGP Environment",
                "environment_score": environment_score,
                "legs": " + ".join(top["player"].astype(str).tolist()),
                "roles": ", ".join(sorted(roles)),
                "why": "Multiple Blender-qualified owners in same game environment" + (" + WHO chaos" if chaos else "") + (" + adjacent transfer" if adjacent else ""),
            })
    return pd.DataFrame(rows).sort_values("environment_score", ascending=False).reset_index(drop=True) if rows else pd.DataFrame()

def run_ticket_view(results, timeframe="Full slate", custom_start=None, custom_end=None):
    filtered = filter_results_by_timeframe(results, timeframe, custom_start, custom_end)
    filtered["sgp"] = identify_sgp_candidates(filtered)
    return filtered


# ============================================================
# V147_GAMECOUNT_ENGINE_PATCH
# Correct game counting and expose synced board memory.
# ============================================================
def true_game_count_v147(df):
    df = df if isinstance(df, pd.DataFrame) else pd.DataFrame()
    if df.empty:
        return 0
    for c in ["game_key","game","matchup"]:
        if c in df.columns:
            s = df[c].astype(str).str.strip()
            s = s[(s!="") & (s!="nan") & (s!="TEAM vs OPP") & (s!=" vs ")]
            if len(s):
                return int(s.nunique())
    if "team" in df.columns and "opponent" in df.columns:
        return int((df["team"].astype(str)+" vs "+df["opponent"].astype(str)).nunique())
    return 0

# Override imported counting helpers.
def actual_game_count(df): return true_game_count_v147(df)
def attack_pool_count(df): return true_game_count_v147(df)
def slate_game_count_from_public_context(ctx=None, df=None): return true_game_count_v147(df if df is not None else ctx)

_BASE_RUN_V147_GAMECOUNT = run_true_blender
def run_true_blender(df, *args, **kwargs):
    results = _BASE_RUN_V147_GAMECOUNT(df, *args, **kwargs)
    results = safe_results(results) if "safe_results" in globals() else results
    owners = results.get("owners", pd.DataFrame())
    survivors = results.get("survivors", pd.DataFrame())

    # Synced board rows from same survivors/owners.
    src = survivors if isinstance(survivors, pd.DataFrame) and not survivors.empty else owners
    gate_rows = []
    role_rows = []
    if isinstance(src, pd.DataFrame) and not src.empty:
        for _, r in src.iterrows():
            ...
            if path:
                ...
                parts = [str(r.get("archetype","Board memory"))]
            for i, p in enumerate(parts):
                verdict = "KILL" if ("KILL" in p.upper() or "NO PLAY" in p.upper()) else ("WEAK" if "WEAK" in p.upper() else "PASS")
                gate_rows.append({
                    "game_key": game, "player": player, "role": role,
                    "gate_step": i, "gate": p[:100], "verdict": verdict,
                    "score": r.get("score", 0), "reason": p
                })
            role_rows.append({
                "game_key": game, "player": player, "assigned_role": role,
                "archetype": r.get("archetype",""), "score": r.get("score",0),
                "hard_fails": r.get("hard_fails",""), "soft_fails": r.get("soft_fails","")
            })
    results["game_board"] = pd.DataFrame(gate_rows)
    results["role_board"] = pd.DataFrame(role_rows)

    meta = results.get("meta", {}) or {}
    meta["games"] = true_game_count_v147(df)
    meta["slate_games"] = true_game_count_v147(df)
    meta["attack_pools"] = true_game_count_v147(df)
    meta["board_memory_sync"] = True
    meta["survivor_rows"] = int(len(survivors)) if isinstance(survivors, pd.DataFrame) else 0
    results["meta"] = meta
    return results
