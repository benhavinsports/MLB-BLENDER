...
# ============================================================
# V152 FINAL ENGINE OUTPUT REPAIR
# Fixes:
# - no 99% cap-looking scores
# - no duplicate game inside Core 3
# - core_slot is the displayed structure, separate from player role
# - Core 3 = Primary slot + Adjacent slot + WHO slot, each different game when possible
# - Pass/cut memory stays attached to same result object
# ============================================================

def _v152_build_results(df):
    # Use V151 if available, then repair final memory/output.
    if "_v151_build_results" in globals():
        results = _v151_build_results(df)
    elif "build_results" in globals():
        results = build_results(df)
    else:
        results = {"owners": pd.DataFrame(), "core": pd.DataFrame(), "alt": pd.DataFrame(), "chaos": pd.DataFrame(), "survivors": pd.DataFrame(), "cuts": pd.DataFrame(), "game_board": pd.DataFrame(), "role_board": pd.DataFrame(), "meta": {}}
    ...
    # Recalculate display score so it doesn't read like fake probability/capped 99.1.
    def recalc_display_score(r):
        pull = _v151_get(r, ["pull_pct","pull%","pull"], 0) if "_v151_get" in globals() else 0
        hard = _v151_get(r, ["hard_hit_pct","hardhit%","hard hit%","hh%"], 0) if "_v151_get" in globals() else 0
        barrel = _v151_get(r, ["barrel_pct","barrel%","brl%"], 0) if "_v151_get" in globals() else 0
        dmg = _v151_get(r, ["dmg","damage","ult","adj"], 0) if "_v151_get" in globals() else 0
        hpi = _v151_get(r, ["hpi","model","rating"], 0) if "_v151_get" in globals() else 0
        hr = _v151_get(r, ["hr_lane","hr_pa","hr/pa","hr9"], 0) if "_v151_get" in globals() else 0
        edge = _v151_get(r, ["pitch_edge","edge"], -99) if "_v151_get" in globals() else -99
        base = 38
        base += min(pull, 60) * 0.18
        base += min(hard, 65) * 0.10
        base += min(barrel, 18) * 0.85
        base += min(dmg, 3.0) * 4.5
        base += min(hpi, 85) * 0.11
        base += min(hr, 3.0) * 4.0
        if edge != -99:
            base += max(min(edge, 25), -15) * 0.28
        # Add tiny row/game differentiator to kill duplicate-looking scores.
        name = str(r.get("player",""))
        salt = (sum(ord(ch) for ch in name) % 17) / 10
        if not bool(r.get("blender_eligible", False)):
            return round(min(base, 58.9), 1)
        return round(max(42.0, min(91.7, base + salt)), 1)

    survivors["blender_score"] = survivors.apply(lambda r: recalc_display_score(r), axis=1)
    survivors["score"] = survivors["blender_score"]

    passed = survivors[survivors.get("blender_eligible", False) == True].copy()
    cuts = survivors[survivors.get("blender_eligible", False) != True].copy()

    # One owner per game, highest true display score.
    group_col = "game_pk" if "game_pk" in passed.columns and passed["game_pk"].dropna().astype(str).str.strip().replace("", pd.NA).dropna().nunique() > 0 else "game_key"
    owners_rows = []
    if not passed.empty and group_col in passed.columns:
        for game, g in passed.groupby(group_col, dropna=False):
            if str(game).strip() == "" or str(game).lower() == "nan":
                continue
            pick = g.sort_values(["blender_score","support_score"], ascending=[False, False]).iloc[0].to_dict()
            pick["game_owner"] = pick.get("game_key", game)
            owners_rows.append(pick)
    owners = pd.DataFrame(owners_rows)
    if not owners.empty:
        owners = owners.sort_values(["blender_score","support_score"], ascending=[False, False]).reset_index(drop=True)

    # Core 3 slot machine: no duplicate player AND no duplicate game unless unavoidable.
    def game_id(r):
        if "game_pk" in r and str(r.get("game_pk","")).strip() not in ["","nan","None"]:
            return str(r.get("game_pk"))
        return str(r.get("game_key",""))

    used_players = set()
    used_games = set()
    core_parts = []

    def take_for_slot(slot_name, role_names):
        nonlocal used_players, used_games, core_parts
        if owners.empty:
            return
        pool = owners[owners["official_core_role"].astype(str).isin(role_names)].copy() if "official_core_role" in owners.columns else owners.copy()
        pool = pool[~pool["player"].astype(str).isin(used_players)] if "player" in pool.columns else pool
        if not pool.empty:
            pool["_gid"] = pool.apply(game_id, axis=1)
            no_dup = pool[~pool["_gid"].isin(used_games)].copy()
            if not no_dup.empty:
                pool = no_dup
            pick = pool.sort_values(["blender_score","support_score"], ascending=[False, False]).head(1).copy()
            if not pick.empty:
                pick["core_slot"] = slot_name
                core_parts.append(pick.drop(columns=["_gid"], errors="ignore"))
                used_players.update(pick["player"].astype(str).tolist())
                used_games.update(pick["_gid"].astype(str).tolist())

    take_for_slot("PRIMARY", ["Primary"])
    take_for_slot("ADJACENT", ["Adjacent"])
    take_for_slot("WHO", ["WHO"])

    # Refill missing slots with next best owners from different games.
    slot_order = ["PRIMARY","ADJACENT","WHO"]
    current_slots = []
    if core_parts:
        current_slots = pd.concat(core_parts, ignore_index=True).get("core_slot", pd.Series(dtype=str)).tolist()
    for missing_slot in [s for s in slot_order if s not in current_slots]:
        if owners.empty:
            continue
        pool = owners[~owners["player"].astype(str).isin(used_players)].copy() if "player" in owners.columns else owners.copy()
        if not pool.empty:
            pool["_gid"] = pool.apply(game_id, axis=1)
            no_dup = pool[~pool["_gid"].isin(used_games)].copy()
            if not no_dup.empty:
            ...
            pick = pool.sort_values(["blender_score","support_score"], ascending=[False, False]).head(1).copy()
            if not pick.empty:
                pick["core_slot"] = missing_slot + " REFILL"
                core_parts.append(pick.drop(columns=["_gid"], errors="ignore"))
                used_players.update(pick["player"].astype(str).tolist())
                used_games.update(pick["_gid"].astype(str).tolist())

    core = pd.concat(core_parts, ignore_index=True).head(3) if core_parts else pd.DataFrame()
    if not core.empty:
        core["ticket_role"] = "CORE"

    used_players = set(core["player"].astype(str).tolist()) if not core.empty and "player" in core.columns else set()
    alt = owners[~owners["player"].astype(str).isin(used_players)].head(3).copy() if not owners.empty and "player" in owners.columns else pd.DataFrame()
    if not alt.empty:
        alt["ticket_role"] = "ALT"
    used_players.update(alt["player"].astype(str).tolist() if not alt.empty and "player" in alt.columns else [])
    chaos = owners[(owners["official_core_role"].astype(str) == "WHO") & (~owners["player"].astype(str).isin(used_players))].head(3).copy() if not owners.empty and "official_core_role" in owners.columns and "player" in owners.columns else pd.DataFrame()
    if chaos.empty and not owners.empty and "player" in owners.columns:
        chaos = owners[~owners["player"].astype(str).isin(used_players)].head(3).copy()
    if not chaos.empty:
        chaos["ticket_role"] = "WHO"

    # Rebuild board score values from survivors.
    board = results.get("game_board", pd.DataFrame())
    if isinstance(board, pd.DataFrame) and not board.empty and "player" in board.columns:
        score_map = survivors.set_index("player")["blender_score"].to_dict() if "player" in survivors.columns else {}
        board = board.copy()
        board["blender_score"] = board["player"].map(score_map).fillna(board.get("blender_score", 0))

    roles = results.get("role_board", pd.DataFrame())
    if isinstance(roles, pd.DataFrame) and not roles.empty and "player" in roles.columns:
        score_map = survivors.set_index("player")["blender_score"].to_dict() if "player" in survivors.columns else {}
        roles = roles.copy()
        roles["blender_score"] = roles["player"].map(score_map).fillna(roles.get("blender_score", 0))

    games = 0
    if "game_pk" in survivors.columns and survivors["game_pk"].dropna().astype(str).str.strip().replace("", pd.NA).dropna().nunique() > 0:
        games = int(survivors["game_pk"].dropna().astype(str).str.strip().replace("", pd.NA).dropna().nunique())
    elif "game_key" in survivors.columns:
        games = int(survivors["game_key"].dropna().astype(str).str.strip().replace("", pd.NA).dropna().nunique())

    meta = results.get("meta", {}) or {}
    meta.update({
        "engine_version": "V152_VISUAL_ENGINE_REPAIR",
        "games": games,
        "input_rows": len(survivors),
        "passed_rows": len(passed),
        "cut_rows": len(cuts),
        "owners_locked": len(owners),
        "core_count": len(core),
        "core_slots": core.get("core_slot", pd.Series(dtype=str)).tolist() if not core.empty else [],
        "message": f"V152 Blender: {len(owners)} owners, {len(passed)} pass rows, {len(cuts)} cuts. Core slots={core.get('core_slot', pd.Series(dtype=str)).tolist() if not core.empty else []}."
    })

    return {"owners": owners, "core": core, "alt": alt, "chaos": chaos, "survivors": survivors, "cuts": cuts, "game_board": board, "role_board": roles, "meta": meta}

def run_true_blender(df, *args, **kwargs):
    results = _v152_build_results(df)
    try:
        save_locked_results(results)
    except Exception:
        pass
