...
# -------------------- v83 FINAL SYNC HELPERS --------------------
DEFAULT_GATE_WEIGHTS = {
    "pull": 1.35, "launch": 1.15, "damage": 1.25, "pitch": 1.25,
    "conversion": 1.20, "opportunity": 0.80, "chaos": 0.70, "gate19": 1.25
}
SAVANT_BATTER_CACHE = DATA_DIR / "savant_batter_metrics_cache.csv"
ADAPTIVE_WEIGHTS_FILE = DATA_DIR / "adaptive_gate_weights.json"


try:
    from ai_oil import ai_field_mapper, ai_feed_validator, ai_chaos_who_detector, ai_calibrate_from_recap, save_ai_audit
except Exception:
    ai_field_mapper = ai_feed_validator = ai_chaos_who_detector = ai_calibrate_from_recap = save_ai_audit = None
...
def _name_key(x):
    return re.sub(r"[^a-z ]", "", str(x or "").lower()).strip()

def is_player_name_safe(x):
    s = str(x or "").strip()
    bad = {"low effort","medium effort","high effort","effort","fresh","moderate","elevated","hot","cold","home","away","cond","eats lhp","eats rhp","primary","adjacent","who","platoon","stack"}
    return bool(s) and len(s) > 2 and s.lower() not in bad and not s.replace(".","").isdigit()

def _slot_match(r):
    try:
        slot = _num(r.get("lineup_slot"), None)
        vals = [x.strip() for x in str(r.get("weak_slots","")).split(",") if x.strip()]
...
def _first_col(df, aliases):
    ...
    for a in aliases:
        if a.lower().strip() in low: return low[a.lower().strip()]
    return None
...
def normalize_public_batter_metrics(raw):
    if raw is None or raw.empty: return pd.DataFrame()
    pc = _first_col(raw, ["player_name","name","player"])
    if pc is None: return pd.DataFrame()
    out = pd.DataFrame({"player": raw[pc].astype(str)})
    idc = _first_col(raw, ["player_id","batter","mlb_id","id"])
    out["mlb_id"] = raw[idc] if idc else ""
    mapping = {
        "pull_pct":["pull_percent","pull%","pull_pct","pull"],
        "sweet_spot_pct":["sweet_spot_percent","sweet_spot%","sweet_spot_pct","sweet_spot"],
        "barrel_pct":["barrel_batted_rate","barrel%","barrel_pct","barrel"],
        "hard_hit_pct":["hard_hit_percent","hard_hit%","hard_hit_pct","hard_hit"],
        "dmg":["dmg","xslg","xwoba","xba"],
        "hr_pa":["hr_pa","hr/pa","home_run_per_pa"],
        "hpi":["hpi","home_run_index","hr_index"],
    }
    for target, aliases in mapping.items():
        c = _first_col(raw, aliases)
        if c is not None: out[target] = raw[c]
    for c in ["pull_pct","sweet_spot_pct","barrel_pct","hard_hit_pct"]:
        if c in out: out[c] = out[c].apply(_coerce_pct)
    for c in ["dmg","hr_pa","hpi"]:
        if c in out: out[c] = pd.to_numeric(out[c], errors="coerce")
    if "dmg" in out:
        out["dmg"] = out["dmg"].apply(lambda x: round(float(x)*3,3) if pd.notna(x) and float(x)<1 else x)
    return out.drop_duplicates("player", keep="first")



def fetch_team_roster_hitters(team_id):
    try:
        with urllib.request.urlopen(f"https://statsapi.mlb.com/api/v1/teams/{team_id}/roster?rosterType=active", timeout=12) as resp:
            data = json.loads(resp.read().decode("utf-8"))
        rows=[]
        for item in data.get("roster", []):
            if item.get("position",{}).get("abbreviation") == "P": continue
            p = item.get("person",{})
            if p.get("fullName"): rows.append({"player":p.get("fullName"),"mlb_id":p.get("id","")})
        return rows
    except Exception:
        return []
...
def grade_gate_board(gates):
    weights = {
        "0 Environment / game target":4, "1 Uploaded-pool legality":10, "2 Side/team lock":8,
        "3 Pitcher weakness / HR lane":5, "4 Pitch-type kill switch":5,
        "5 Pull-air trigger":15, "6 Launch / sweet spot":8, "7 Damage / barrel":13,
        "8 True HR conversion DNA":13, "9 Opportunity / lineup":5, "10 Hard-hit support":5,
        "10.5 Adjacent transfer":3, "11 WHO / chaos":3, "12 Game script":2,
        "13 Recap DNA pattern":4, "14 Anti-chalk trap":5, "15 Bullpen continuation":2,
        "16 Finisher gate":15, "17 One-owner isolation":4, "18 Final lock audit":4,
        "19 HR model confirmation":5
    }
    score=maxs=0.0; hard_kill=False
    for name, passed, hard in gates:
        w=weights.get(name,3); maxs += w
        if passed: score += w
        elif hard: hard_kill=True
    return (round(score/maxs*100,1) if maxs else 0.0), hard_kill
...
def rebuild_live_blender_feed(upload_df=None, public_context=None, use_live_pool=False, use_live_metrics=False):
    ...
    # AI Oil repair layer: field repair, validation, chaos flags. Never allowed to freestyle picks.
    try:
        if ai_field_mapper:
            df, _map_audit = ai_field_mapper(df)
        if ai_feed_validator:
            _audit = ai_feed_validator(df)
        if ai_chaos_who_detector:
            df = ai_chaos_who_detector(df)
    except Exception:
        pass

    # Only use web metrics when explicitly asked; uploaded feed should never hang.
    if use_live_metrics:
        try:
            df = enrich_with_live_batter_metrics(df)
        except Exception:
...
def evaluate_18_gates(r):
    pull=_num(r.get("pull_pct")); sweet=_num(r.get("sweet_spot_pct")); barrel=_num(r.get("barrel_pct"))
    hh=_num(r.get("hard_hit_pct")); dmg=_num(r.get("dmg")); hrpa=_num(r.get("hr_pa")); hpi=_num(r.get("hpi")); pe=_num(r.get("pitch_edge"))
    slot=_num(r.get("lineup_slot"), None); weak_slot=_slot_match(r) or bool(r.get("weak_slot_tag"))
    platoon=bool(r.get("platoon")); alert=bool(r.get("hr_alert")); chalk=bool(r.get("chalk_trap", False))
    has_pitch=_present(r,"pitch_edge")
    metric_hits=sum(_present(r,k) for k in ["pull_pct","sweet_spot_pct","barrel_pct","hard_hit_pct","dmg","hr_pa","hpi"])
    ...
    pool_ok=is_player_name_safe(r.get("player","")) and metric_hits>=3
    pitch = has_pitch and pe >= 0; pitch_unknown = not has_pitch; pitch_lane_ok = pitch or pitch_unknown
    pull_air = pull>=38 or (pull>=30 and sweet>=27) or (pull>=28 and dmg>=1.6)
    launch = sweet>=24 or barrel>=9 or hh>=42 or dmg>=1.7
    damage = dmg>=1.45 or hrpa>=2.0 or alert or hpi>=38
    conversion = hpi>=34 or hrpa>=1.5 or dmg>=1.45
    opportunity = slot is not None or weak_slot or platoon
    hardhit_support = hh>=38 or barrel>=8 or dmg>=1.5 or sweet>=28
    transfer = weak_slot or platoon or bool(r.get("adjacent_transfer"))
    chaos = alert and damage and conversion and (weak_slot or hrpa>=3.0 or dmg>=1.8)
    ...
    anti = not chalk; bullpen = not bool(r.get("bullpen_kill", False))
    finisher = pull_air and damage and conversion
    gate19 = hr_model_confirm(r)[0] if "hr_model_confirm" in globals() else recap_dna
    gate19_ok = gate19 or recap_dna or finisher
    gates=[
        ("0 Environment / game target",env_ok,False), ("1 Uploaded-pool legality",pool_ok,True),
        ("2 Side/team lock",env_ok,True), ("3 Pitcher weakness / HR lane",pitch_lane_ok or alert or dmg>=1.8,False),
        ("4 Pitch-type kill switch",pitch_lane_ok,False), ("5 Pull-air trigger",pull_air,True),
        ("6 Launch / sweet spot",launch,False), ("7 Damage / barrel",damage,True),
        ("8 True HR conversion DNA",conversion,True), ("9 Opportunity / lineup",opportunity,False),
        ("10 Hard-hit support",hardhit_support,False), ("10.5 Adjacent transfer",transfer,False),
        ("11 WHO / chaos",chaos,False), ("12 Game script",script,False),
        ("13 Recap DNA pattern",recap_dna,False), ("14 Anti-chalk trap",anti,True),
        ("15 Bullpen continuation",bullpen,False), ("16 Finisher gate",finisher,True),
        ("17 One-owner isolation",True,True), ("18 Final lock audit",True,True),
        ("19 HR model confirmation",gate19_ok,False),
    ]
    hard=[n for n,p,h in gates if h and not p]; soft=[n for n,p,h in gates if not h and not p]
    grade, hard_kill = grade_gate_board(gates)
    return {"gates":gates,"hard_failed":hard,"soft_failed":soft,"grade":grade,
            "hard_kill":len(hard)>0,"clean_owner":len(hard)==0,
            "adjacent_owner":len(hard)==0 and transfer and not chaos,
            "who_owner":len(hard)==0 and chaos}
...
def archetype(r):
    g=hard_gate_result(r)
    if not g["clean_owner"]: return "Audit Survivor / Data-Recovery Profile"
    if g["who_owner"]: return "WHO / Chaos Finisher"
    if g["adjacent_owner"]: return "Adjacent / Transfer Owner"
    pull=_num(r.get("pull_pct")); dmg=_num(r.get("dmg")); hpi=_num(r.get("hpi")); pe=_num(r.get("pitch_edge")); hrpa=_num(r.get("hr_pa")); sweet=_num(r.get("sweet_spot_pct")); barrel=_num(r.get("barrel_pct"))
    if pull>=40 and dmg>=1.5 and hpi>=35 and (pe>=0 or not _present(r,"pitch_edge")): return "Elite Converter"
    if _present(r,"pitch_edge") and pe>=10: return "Pitch-Type Punisher"
    if hrpa>=3.0: return "Nuke Lane Finisher"
    if sweet>=28 and barrel>=9: return "Launch Window Finisher"
    return "Primary HR Owner"


def blend_score(r):
    pull=_num(r.get("pull_pct")); sweet=_num(r.get("sweet_spot_pct"))
    barrel=_num(r.get("barrel_pct")); hh=_num(r.get("hard_hit_pct"))
    dmg=_num(r.get("dmg")); hrpa=_num(r.get("hr_pa"))
    hpi=_num(r.get("hpi")); pe=_num(r.get("pitch_edge"))
    slot=_num(r.get("lineup_slot"), None); has_pitch=_present(r,"pitch_edge")
    score=0.0
    score += min(18, max(0, (pull-25)*0.75))
    score += min(12, max(0, (sweet-18)*0.60))
    score += min(10, max(0, (barrel-5)*1.25))
    score += min(8, max(0, (hh-34)*0.45))
    score += 13 if dmg>=2.2 else 10 if dmg>=1.8 else 7 if dmg>=1.5 else 4 if dmg>=1.2 else 0
    score += 10 if hrpa>=4.0 else 8 if hrpa>=3.0 else 6 if hrpa>=2.0 else 3 if hrpa>=1.3 else 0
    ...
    if bool(r.get("environment_hot")) or bool(r.get("cond_up")): score += 2
    if pull < 25: score -= 12
    if sweet < 20 and barrel < 7: score -= 8
    if dmg < 1.0 and hrpa < 1.0 and hpi < 25: score -= 12
    if bool(r.get("chalk_trap")): score -= 8
    try:
        g = hard_gate_result(r)
        grade = _num(g.get("grade"))
        score += max(0, min(5, (grade-60)*0.06))
        score -= min(15, len(g.get("hard_failed", []))*4)
    except Exception:
        pass
    return round(max(0, min(92, score)), 1)


def build_player_gate_path(row, rank=None):
    g=hard_gate_result(row)
    parts=[]
    for idx,(name,passed,hard) in enumerate(g.get("gates",[])):
        status="PASS" if passed else ("KILL" if hard else "WEAK")
        parts.append(f"{idx} {name}: {status}")
    if g.get("hard_failed"):
        parts.append("HARD FAILS: " + ", ".join(g["hard_failed"]))
    if g.get("soft_failed"):
        parts.append("SOFT FLAGS: " + ", ".join(g["soft_failed"]))
    return " | ".join(parts)
...
def run_game(gdf):
    ...
    clean=pool[pool["_gate_obj"].apply(lambda x:x.get("clean_owner",False))].copy()
    if clean.empty:
        pool["_profile_score"]=pool["score"] + pool["gate_grade"]*.08
        pool=pool.sort_values("_profile_score",ascending=False).reset_index(drop=True)
        best=pool.head(1).copy()
        profile=float(best.iloc[0].get("score",0))
        if is_player_name_safe(best.iloc[0].get("player","")) and profile >= 42:
            best["official_core_role"]="Recovery"
            best["archetype"]="Audit Survivor / Data-Recovery Profile"
            best["score"]=best["score"].apply(lambda x:max(40.0, min(68.0, float(x))))
            best["gate_path"]=[build_player_gate_path(row,1) + " | RECOVERY: data incomplete, capped score" for _,row in best.iterrows()]
        else:
            best["official_core_role"]="NO PLAY"
            best["archetype"]="No Clean Owner"
            best["score"]=best["score"].apply(lambda x:min(float(x),39.0))
            best["gate_path"]=[build_player_gate_path(row,None) for _,row in best.iterrows()]
        return best.drop(columns=[c for c in ["_gate_obj","_profile_score"] if c in best.columns])
    clean["_owner_score"]=clean["score"]+clean["gate_grade"]*.05
    clean=clean.sort_values("_owner_score",ascending=False).reset_index(drop=True)
    clean["gate_path"]=[build_player_gate_path(row,i+1) for i,row in clean.iterrows()]
    return clean.drop(columns=[c for c in ["_gate_obj","_owner_score"] if c in clean.columns])


def build_tickets_from_owners(owners, survivors=None):
    if owners is None or owners.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    df=owners.copy().sort_values("score", ascending=False).reset_index(drop=True)
    clean=df[df["official_core_role"].astype(str).isin(["Primary","Adjacent","WHO"])].copy()
    recovery=df[df["official_core_role"].astype(str).eq("Recovery")].copy()
    core_pool = clean if len(clean)>=3 else pd.concat([clean, recovery], ignore_index=True)
    core = core_pool.head(3).copy()
    used=set(core.get("player",[]))
    alt = df[~df["player"].isin(used)].head(3).copy()
    used.update(alt.get("player",[]))
    chaos_pool = df[(~df["player"].isin(used)) & (df["archetype"].astype(str).str.contains("WHO|Chaos|Recovery|Transfer", case=False, na=False))]
...
def run_true_blender(df, *args, **kwargs):
    use_live_metrics = bool(kwargs.get("use_live_metrics", False))
    try:
        df = rebuild_live_blender_feed(upload_df=df, use_live_metrics=use_live_metrics)
    except TypeError:
        df = rebuild_live_blender_feed(upload_df=df)
    except Exception:
        pass
    meta={"input_rows":0 if df is None else int(len(df)),"games":0,"owners_locked":0,"survivor_rows":0,"no_play_games":0,"message":"","engine_version":"v96_TODAY_READY_19_GATE"}
    if df is None or df.empty:
    ...
    try: meta["games"]=actual_game_count(df)
    except Exception: meta["games"]=int(df["game_key"].nunique()) if "game_key" in df.columns else int(df["game"].nunique()) if "game" in df.columns else 0
    owners=[]; survivors=[]; group_col="game_key" if "game_key" in df.columns else "game"
    for game,gdf in df.groupby(group_col,dropna=False):
        try:
            alive=run_game(gdf)
        except Exception as e:
            alive=gdf.head(1).copy()
            alive["official_core_role"]="Recovery"; alive["archetype"]="Engine Recovery"; alive["score"]=40
            alive["gate_path"]=f"ENGINE RECOVERY: {e}"; alive["hard_fails"]=""; alive["soft_fails"]=f"ENGINE RECOVERY: {e}"
        if alive is None or alive.empty: continue
        alive=alive.assign(game_owner=game)
        survivors.append(alive)
        top=alive.sort_values("score", ascending=False).iloc[0].to_dict() if "score" in alive else alive.iloc[0].to_dict()
        if top.get("official_core_role")=="NO PLAY" and is_player_name_safe(top.get("player","")):
            top["official_core_role"]="Recovery"
            top["archetype"]="Visible Recovery / Weak Data"
            top["score"]=max(40.0, min(58.0, _num(top.get("score"),40)))
            top["gate_path"]=str(top.get("gate_path",""))+" | V96 visible recovery"
    ...
    meta["owners_locked"]=int(len(owners)); meta["survivor_rows"]=int(len(survivors))
    ...
    if owners.empty and not survivors.empty:
        top=survivors.sort_values("score",ascending=False).head(1).copy() if "score" in survivors else survivors.head(1).copy()
        top["official_core_role"]="Recovery"; top["archetype"]="Visible Recovery / Weak Data"
        top["score"]=top.get("score", pd.Series([45])).apply(lambda x:max(40,min(58,_num(x,45))))
        owners=top
        meta["owners_locked"]=int(len(owners))
    if owners.empty:
        meta["message"]="Blender completed but no valid player rows were found. Check feeder parsing."
    ...
    clean_count=int((owners["official_core_role"].astype(str)!="Recovery").sum()) if "official_core_role" in owners else 0
    recovery_count=int((owners["official_core_role"].astype(str)=="Recovery").sum()) if "official_core_role" in owners else 0
    meta["message"]=f"Blender complete: {len(owners)} game owners locked from {meta['games']} games. Clean={clean_count} · Recovery={recovery_count}."
