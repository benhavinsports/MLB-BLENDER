...
# ============================================================
# STAR TOOL EFFECTIVE ENGINE V8
# Uses parser-produced Star Tool fields.
# No fake picks; but Star Tool PDF rows now qualify.
# ============================================================

_STAR_BASE_RUN = run_true_blender

def _st_num(x, default=0.0):
    try:
...
def _st_get(row, names, default=None):
    for n in names:
        if n in row:
            v=row.get(n)
            try:
    ...
    for n in names:
        ...
        for ck,real in compact.items():
            if nn in ck or ck in nn: return row.get(real)
    return default

def _st_metrics(row):
    aliases={
        "slot":["lineup_slot","slot","order"],
        "pressure":["pull_pct","pressure","rating_pct"],
        "cond":["hard_hit_pct","cond"],
        "line":["barrel_pct","sweet_spot_pct","line_pct","launch"],
        "dmg":["dmg","damage"],
        "hpi":["hpi"],
        "hrpa":["hr_lane","hr_pa","hr/pa"],
        "edge":["pitch_edge","hr_edge","edge"],
        "best_hr":["best_hr"],
        "effort":["effort_score"],
        "gamehr":["projected_game_hr"]
    }
    vals={}; count=0
    for k,names in aliases.items():
        raw=_st_get(row,names,None)
        if raw is None: vals[k]=None; continue
        try:
            if pd.isna(raw): vals[k]=None; continue
        except Exception: pass
        try:
            vals[k]=float(str(raw).replace("%","").replace("+","").replace(",",""))
            count+=1
        except Exception:
            vals[k]=None
    return vals,count

def _st_strength(x, floor, elite, pts):
...
def _st_eligible(row):
    vals,count=_st_metrics(row)
    trigger = (
        (vals["dmg"] is not None and vals["dmg"]>=0.75) or
        (vals["hpi"] is not None and vals["hpi"]>=20) or
        (vals["hrpa"] is not None and vals["hrpa"]>=1.0) or
        (vals["edge"] is not None and vals["edge"]>=0) or
        (vals["line"] is not None and vals["line"]>=15) or
        (vals["pressure"] is not None and vals["pressure"]>=12)
    )
    return bool(count>=4 and trigger), count

def _st_score(row):
    vals,count=_st_metrics(row)
    ok,_=_st_eligible(row)
    if not ok: return 0.0,0.0,count,"NO PICK - insufficient Star Tool metrics"
    slot=vals["slot"] or 0
    pressure=vals["pressure"] or 0
    cond=vals["cond"] or 0
    line=vals["line"] or 0
    dmg=vals["dmg"] or 0
    hpi=vals["hpi"] or 0
    hrpa=vals["hrpa"] or 0
    edge=vals["edge"] if vals["edge"] is not None else -99
    best=vals["best_hr"] or 0
    effort=vals["effort"] or 0
    gamehr=vals["gamehr"] or 0
    notes=_st_txt(row.get("notes")).lower()

    raw=0; max_raw=0
    def add(p,m):
        nonlocal raw,max_raw
        raw += max(0.0,float(p)); max_raw += float(m)
    add(5 if _st_txt(row.get("player")) else 0,5)
    add(5 if (_st_txt(row.get("team")) or _st_txt(row.get("pitcher")) or _st_txt(row.get("game"))) else 0,5)
    add(_st_strength(gamehr,1.8,3.4,6),6)                       # environment
    add(max(_st_strength(hrpa,1.0,6.0,10), _st_strength(edge,0,40,10)),10)
    add(10 if edge>=0 else 0,10)                                 # pitch/park edge not negative
    add(max(_st_strength(line,14,35,10), _st_strength(cond,15,45,10)),10)
    add(max(_st_strength(dmg,.75,2.5,13), _st_strength(hpi,20,65,13)),13)
    add(max(_st_strength(hrpa,1.0,6.0,12), _st_strength(best,1,50,12)),12)
    add(6 if slot==0 or slot<=7 else 0,6)
    add(_st_strength(pressure,10,40,7),7)
    add(3,3)                                                     # 10.5 checked
    add(5 if ("weak slot" in notes or "who" in notes or "chaos" in notes) else 0,5)
    add(3,3)
    add(max(_st_strength(dmg,.75,2.5,8), _st_strength(hrpa,1.0,6.0,8)),8)
    add(8 if "trap" not in notes and "cold" not in notes else 0,8)
    add(4 if any(x in notes for x in ["alert","hot","laser","rakes","platoon"]) else 0,4)
    add(max(_st_strength(dmg,1.0,2.8,13), _st_strength(hpi,25,70,13), _st_strength(hrpa,2.0,8.0,13)),13)
    add(5,5)
    add(6 if _st_txt(row.get("player")) and (_st_txt(row.get("team")) or _st_txt(row.get("pitcher"))) else 0,6)
    add(max(_st_strength(hpi,20,65,12), _st_strength(dmg,.75,2.5,12), _st_strength(hrpa,1.0,6.0,12)),12)
    blender=(raw/max_raw)*100
    if "cold" in notes: blender-=5
    if "alert" in notes: blender+=4
    if "hot" in notes: blender+=3
    if "weak slot" in notes: blender+=2
    if "pitcher weak slot match" in notes: blender+=5
    if "laser" in notes: blender+=2
    if "rakes" in notes: blender+=2
    support = min(dmg,3)*18 + min(hpi,80)*.35 + min(hrpa,10)*4 + min(line,45)*.25 + min(edge if edge!=-99 else 0,50)*.15 + min(pressure,45)*.15
    return round(max(1,min(100,blender)),1), round(max(1,min(100,support)),1), count, "ELIGIBLE"

def validate_feed_for_blender(df):
    df=_bf_safe_df(df)
    if df.empty: return {"ok":False,"eligible_rows":0,"total_rows":0,"score":0,"message":"No feed rows loaded."}
    elig=0
    for _,r in df.iterrows():
        ok,_=_st_eligible(r.to_dict())
        elig += int(ok)
    return {"ok":elig>0,"eligible_rows":int(elig),"total_rows":int(len(df)),"score":round(elig/max(1,len(df))*100,1),"message":f"Feed validator: {elig}/{len(df)} rows have enough real Blender metrics."}

def _st_apply(df):
    df=_bf_safe_df(df).copy()
    if df.empty: return df
    bs=[]; ss=[]; counts=[]; flags=[]; reasons=[]
    for _,r in df.iterrows():
        b,s,c,reason=_st_score(r.to_dict())
        bs.append(b); ss.append(s); counts.append(c); flags.append(reason=="ELIGIBLE"); reasons.append(reason)
    df["blender_score"]=bs
    df["support_score"]=ss
    df["score"]=df["blender_score"]
    df["metric_count"]=counts
    df["blender_eligible"]=flags
...
def _st_tickets(owners):
    owners=_st_apply(owners)
    ...
    chaos=owners[owners["official_core_role"].astype(str).str.contains("WHO|Chaos|Validated|Recovery",case=False,na=False) & ~owners["player"].astype(str).isin(used)].head(3).copy() if "player" in owners else pd.DataFrame()
...
def run_true_blender(df,*args,**kwargs):
    ...
    results=_STAR_BASE_RUN(df,*args,**kwargs)
    if not isinstance(results,dict): return results
    survivors=_st_apply(results.get("survivors",pd.DataFrame()))
    owners=pd.DataFrame()
    if not survivors.empty:
        ...
        if group_col:
            ...
            for game,g in survivors.groupby(group_col,dropna=False):
                ...
                pick["game_owner"]=game
                if _st_txt(pick.get("official_core_role")) in {"","NO PLAY","NO PICK"}:
                    pick["official_core_role"]="Primary" if _st_num(pick.get("blender_score"))>=75 else "Validated"
                picks.append(pick)
            owners=pd.DataFrame(picks)
    if owners.empty:
        owners=_st_apply(results.get("owners",pd.DataFrame()))
        owners=owners[owners["blender_eligible"]].copy() if not owners.empty else owners
    if not owners.empty:
        owners=owners.sort_values(["blender_score","support_score"],ascending=[False,False]).reset_index(drop=True)
    core,alt,chaos=_st_tickets(owners)
    ...
    results["chaos"]=chaos
    meta=results.get("meta",{}) or {}
    meta["score_engine"]="STAR_TOOL_EFFECTIVE_ENGINE_V8"
    ...
    if owners.empty:
        ...
        meta["message"]=f"Blender complete: {len(owners)} owners locked from Star Tool PDF. {validation['message']}"
    results["meta"]=meta
