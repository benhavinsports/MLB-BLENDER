...
def rebuild_live_blender_feed(upload_df=None, public_context=None, use_live_pool=False, use_live_metrics=False):
    """
    Final synchronized feed builder.
    Important:
    - Uploaded feed must run immediately and must NOT hang on web metric calls.
    - Live public mode may try live metrics, but it still returns fast/fails safe.
    """
    ...
    # Only enrich from web if explicitly requested OR core metric coverage is badly missing.
    metric_cols = [c for c in ["pull_pct","sweet_spot_pct","barrel_pct","hard_hit_pct","dmg","hr_pa","hpi"] if c in df.columns]
    coverage = 0
    try:
        coverage = int(df[metric_cols].notna().any(axis=1).sum()) if metric_cols else 0
    except Exception:
        coverage = 0

    if use_live_metrics or coverage == 0:
        try:
            df = enrich_with_live_batter_metrics(df)
        except Exception:
            pass
...
def evaluate_18_gates(r):
    ...
    pitch=_present(r,"pitch_edge") and pe>=0
    ...
    anti=not chalk; bullpen=not bool(r.get("bullpen_kill", False)); finisher=pull_air and damage and conversion and pitch
    gate19=hr_model_confirm(r)[0] if "hr_model_confirm" in globals() else recap_dna
    gates=[("0 Environment",env_ok,False),("1 Pool legality",pool_ok,True),("2 Side lock",env_ok,True),("3 Pitcher weakness",pitch or alert or dmg>=1.8,True),("4 Pitch-type lane",pitch,True),("5 Pull-air",pull_air,True),("6 Launch/Sweet",launch,True),("7 Damage/Barrel",damage,True),("8 Conversion DNA",conversion,True),("9 Opportunity",opp,False),("10 Hard-hit support",hh_support,False),("10.5 Adjacent transfer",transfer,False),("11 WHO/Chaos",chaos,False),("12 Game script",script,False),("13 Recap DNA calibration",recap_dna,False),("14 Anti-chalk trap",anti,True),("15 Bullpen continuation",bullpen,False),("16 Finisher gate",finisher,True),("17 One-owner isolation",True,True),("18 Final lock",True,True),("19 HR model confirmation",gate19,True)]
    ...
    return {"gates":gates,"hard_failed":hard,"soft_failed":soft,"grade":grade,"hard_kill":hard_kill,"clean_owner":len(hard)==0,"adjacent_owner":len(hard)==0 and transfer and not chaos,"who_owner":len(hard)==0 and chaos}
...
def run_true_blender(df, use_live_metrics=False):
    df = rebuild_live_blender_feed(upload_df=df, use_live_metrics=use_live_metrics)
    meta={"input_rows":0 if df is None else int(len(df)),"games":0,"owners_locked":0,"no_play_games":0,"message":""}
    if df is None or df.empty:
    ...
    except Exception: meta["games"]=int(df["game_key"].nunique()) if "game_key" in df else 0
    ...
    for game,gdf in df.groupby(group_col,dropna=False):
        ...
        if top.get("official_core_role")!="NO PLAY": owners.append(top)
    ...
    if not survivors.empty and "official_core_role" in survivors: meta["no_play_games"]=int((survivors["official_core_role"].astype(str)=="NO PLAY").sum())
    if owners.empty:
        meta["message"]="Blender ran, but no clean owners survived. Game Board shows KILL reasons."
