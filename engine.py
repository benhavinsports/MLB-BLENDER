...
def run_true_blender(df=None, *args, **kwargs):
    ...
    if owners.empty:
        # REAL FIX: never return a dead slate when rows were read.
        # If clean hard gates are too strict, preserve strongest survivor/NO PLAY row per game
        # as FALLBACK_OWNER so the Game Board and Tickets can still render and show why.
        if not survivors.empty:
            fallback_rows = []
            group_field = "game_owner" if "game_owner" in survivors.columns else group_col
            for g, sg in survivors.groupby(group_field, dropna=False):
                sg = sg.copy()
                if "score" not in sg.columns:
                    sg["score"] = 0
                if "gate_grade" not in sg.columns:
                    sg["gate_grade"] = 0
                sg["_fallback_score"] = sg["score"].apply(safe_num) + sg["gate_grade"].apply(safe_num) * 0.50
                pick = sg.sort_values("_fallback_score", ascending=False).iloc[0].to_dict()
                pick["official_core_role"] = "FALLBACK_OWNER"
                pick["archetype"] = pick.get("archetype") or "Recovered Owner"
                if not pick.get("gate_path"):
                    pick["gate_path"] = "Fallback recovery: strict gates had no clean winner, strongest remaining player preserved for review."
                fallback_rows.append(pick)
            owners = pd.DataFrame(fallback_rows)
        elif df is not None and not df.empty:
            # absolute emergency: keep top row from each game so app never displays blank results
            rows = []
            for g, sg in df.groupby(group_col, dropna=False):
                sg = sg.copy()
                for c in ["score","gate_grade"]:
                    if c not in sg.columns: sg[c] = 0
                pick = sg.iloc[0].to_dict()
                pick["score"] = safe_num(pick.get("score"), 25)
                pick["official_core_role"] = "EMERGENCY_OWNER"
                pick["archetype"] = "Emergency Recovered Owner"
                pick["gate_path"] = "Emergency recovery: feed parsed but all engine rows were empty. Check source PDF columns."
                rows.append(pick)
            owners = pd.DataFrame(rows)

    if owners.empty:
        meta["message"] = "Blender ran, but no usable rows could be recovered. Upload/feed parsing failed before gates."
    ...
    meta["owners_locked"] = int(len(owners))
    meta["message"] = f"Blender complete: {len(owners)} owners locked from {meta['games']} games. Full board/tickets saved."
