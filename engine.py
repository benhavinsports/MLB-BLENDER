...
APP_ENGINE_VERSION = "v147_FEED_ENGINE_REPAIR"
...
# -------------------- v147 ENGINE NULL-SAFE OWNER REPAIR --------------------
def prepare_candidates(df):
    if df is None or not hasattr(df,"empty") or df.empty: return pd.DataFrame()
    out=normalize_game_frame(df)
    for c in ["player","team","pitcher","game","tags","gate_path","hard_fails","soft_fails"]:
    ...
    mets=["hr_pa","dmg","hpi","line_drive_pct","cond_pct","hr_edge_pct"]
    out=out[out[mets].notna().any(axis=1)].copy()
    return out.drop_duplicates(subset=["team","pitcher","player"],keep="first").reset_index(drop=True)

def _metric_score(row):
    hrpa=_num(row,"hr_pa"); dmg=_num(row,"dmg"); hpi=_num(row,"hpi"); line=_num(row,"line_drive_pct"); cond=_num(row,"cond_pct"); he=_num(row,"hr_edge_pct"); pe=_num(row,"pitch_edge_pct")
    tags=(str(row.get("tags",""))+" "+str(row.get("gate_path",""))).lower()
    score=min(35,hrpa*6)+min(30,dmg*16)+min(20,hpi*.4)+(8 if line>=25 else 4 if line>=18 else 0)+(7 if cond>=20 else 0)+(8 if he>0 else -4 if he<-10 else 0)+(8 if pe>10 else -4 if pe<-10 else 0)
...
def evaluate_candidates(df):
    ...
    out["event_role"]="CUT"
    out.loc[(out["score"]>=62)&(out["dmg"].fillna(0)>=1.25),"event_role"]="Primary"
    out.loc[(out["score"].between(50,61.999))|((out["tags"].astype(str).str.contains("Weak Slot|Laser",case=False,na=False))&(out["score"]>=45)),"event_role"]="Adjacent"
    out.loc[(out["event_role"].eq("CUT"))&(out["score"]>=40)&(out["hr_pa"].fillna(0)>=2.0),"event_role"]="WHO"
    out["ownership_eligible"]=out["event_role"].isin(["Primary","Adjacent","WHO"])
    out["event_note"]=out["event_role"].map({"Primary":"PRIMARY_EVENT_OWNER","Adjacent":"ADJACENT_TRANSFER_EVENT_OWNER","WHO":"WHO_CHAOS_EVENT_OWNER","CUT":"CUT_BY_SCORE"})
    out["archetype"]=out["event_role"].map({"Primary":"Primary Event Owner","Adjacent":"Adjacent / Transfer Event Owner","WHO":"WHO / Chaos Event Owner","CUT":"Audit / Cut"})
    try: return apply_board_memory(out.sort_values("score",ascending=False).reset_index(drop=True))
    except Exception: return out.sort_values("score",ascending=False).reset_index(drop=True)
