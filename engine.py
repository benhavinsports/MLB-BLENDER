...
def team_abbr(team):
    ...
    if s.upper() in ABBR_TEAM:
        return s.upper()
    try:
        cleaned = re.sub(r"[^A-Z]", "", s.upper())[:3]
    except Exception:
        cleaned = "".join([c for c in s.upper() if c.isalpha()])[:3]
    return TEAM_ABBR.get(s, cleaned or "UNK")


def safe_text_value(x):
    if isinstance(x, dict):
        return x.get("abbreviation") or x.get("teamCode") or x.get("fileCode") or x.get("name") or x.get("clubName") or x.get("shortName") or ""
    if x is None:
        return ""
    return str(x)

def canonical_game_key(team, opponent):
    team = safe_text_value(team).upper().strip()
    opponent = safe_text_value(opponent).upper().strip()
    if team and opponent:
        return f"{team} vs {opponent}"
    return team or opponent or "Unknown Game"


def normalize_game_frame(df):
    """
    Canonicalize mirrored matchups and remove exact duplicate player rows.
    Adds game_key while keeping display game text.
    """
    ...
    out = out.drop_duplicates(subset=["game_key","team","pitcher","player"], keep="first").reset_index(drop=True)
    return out
...
def actual_game_count(df):
    ...
    if "game_key" in df.columns:
        return df["game_key"].nunique()
    return df["game"].nunique() if "game" in df.columns else 0
...
def fetch_live_public_slate(date_iso=None):
    ...
    try:
        ...
        return pd.DataFrame(), {"source":"MLB Stats API", "date":date_iso, "error":str(e)}
    for d in data.get("dates", []):
        for g in d.get("games", []):
            teams = g.get("teams", {})
            away = teams.get("away", {}).get("team", {}).get("name", "")
    ...
    return df, {"source":"MLB Stats API", "date":date_iso, "games": df["game_key"].nunique() if not df.empty else 0}

def merge_public_context(feed_df, public_df):
    if feed_df is None or feed_df.empty:
        return public_df if public_df is not None else pd.DataFrame()
    ...
    if pub.empty:
        return feed
    context = pub.drop_duplicates(subset=["team"])[["team","opponent","pitcher","game","game_key"]]
    merged = feed.merge(context, on="team", how="left", suffixes=("", "_pub"))
    for c in ["opponent","pitcher","game","game_key"]:
        pc = c + "_pub"
        if pc in merged:
            merged[c] = merged[c].where(merged[c].astype(str).str.strip().ne("") & merged[c].notna(), merged[pc])
...
def build_full_live_hitter_pool(date=None):
    ...
    rows = []
    meta = {"source": "MLB Stats API", "date": date, "games": 0, "hitters": 0, "warnings": []}
    try:
        schedule = requests.get(f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&date={date}&hydrate=probablePitcher,team", timeout=10).json()
    except Exception as e:
        meta["warnings"].append(f"schedule failed: {e}")
        return pd.DataFrame(), meta
    games = []
    ...
    for g in games:
        game_pk = g.get("gamePk")
        teams = g.get("teams", {})
        away = safe_text_value(teams.get("away", {}).get("team", {})).upper()
        home = safe_text_value(teams.get("home", {}).get("team", {})).upper()
        if not game_pk:
            continue
        try:
            box = requests.get(f"https://statsapi.mlb.com/api/v1/game/{game_pk}/boxscore", timeout=8).json()
        except Exception as e:
            meta["warnings"].append(f"boxscore {game_pk} failed: {e}")
            continue
        bteams = box.get("teams", {})
        for side, oppside, team_abbr, opp_abbr in [("away","home",away,home),("home","away",home,away)]:
            players = (bteams.get(side, {}) or {}).get("players", {}) or {}
            opp_pitcher = ""
            try:
                opp_pitchers = (bteams.get(oppside, {}) or {}).get("pitchers", []) or []
                opp_players = (bteams.get(oppside, {}) or {}).get("players", {}) or {}
                if opp_pitchers:
                    opp_pitcher = (opp_players.get(f"ID{opp_pitchers[0]}", {}) or {}).get("person", {}).get("fullName", "")
            except Exception:
                pass
            for _, p in players.items():
                ...
                if pos == "P" and not batting_order:
                    continue
                try:
                    slot = int(str(batting_order)[:1]) if batting_order else np.nan
                except Exception:
                    slot = np.nan
                rows.append({
                    "game_id": canonical_game_key(team_abbr, opp_abbr),
                    "team": team_abbr,
                    "opponent": opp_abbr,
                    "player": name,
                    "pitcher": opp_pitcher,
                    "slot": slot,
                    "notes": f"live public slate | lineup slot {slot}",
                    "source": f"MLB Stats API gamePk={game_pk}",
                })
    df = pd.DataFrame(rows)
    meta["hitters"] = len(df)
    return df, meta

def load_live_public_slate(date=None):
    return build_full_live_hitter_pool(date=date)
