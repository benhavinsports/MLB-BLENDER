...
def evaluate_candidate(row,locks):
    ...
    attack_pass=bool(locked and team==locked); add_gate(gates,1,'Attack-side context',attack_pass,8 if attack_pass else 2,8,f'locked={locked}; team={team}',False)
    ...
    for gid,g in passed.groupby('game_id',dropna=False):
        ...
        for _,r in g.iterrows():
            ...
            if player and player not in used:
                used.add(player); rows.append(r.drop(labels=['_owner'],errors='ignore').to_dict()); break
    if not rows: return pd.DataFrame()
    owners=pd.DataFrame(rows); owners['_rank']=owners['event_ownership'].fillna(0)*1000+owners['pass_depth'].fillna(0)*100+owners['elimination_score'].fillna(0)
