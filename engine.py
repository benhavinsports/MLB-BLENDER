...
def build_core_top3(owners: pd.DataFrame) -> pd.DataFrame:
    ...
    # Do NOT refill a missing role with the wrong archetype.
    # Core integrity means Primary + Adjacent + WHO only when those role owners exist.
...
def run_true_blender(df: pd.DataFrame, *args, **kwargs) -> Dict[str, Any]:
    ...
    meta = {
        ...
        "message": f"{ENGINE_VERSION}: Core={len(core)} from {len(owners)} isolated game owners. Immutable player-game binding added. No 99-score engine, no refill, no projection fallback.",
    }
    results = {
        "owners": owners,
        "core": core,
