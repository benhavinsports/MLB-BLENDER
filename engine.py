...
# ============================================================
# COMPLETE REAL FIX OVERRIDE — RUN-ALWAYS BLENDER ENGINE
# Keeps original live tools/recap/functions above, but replaces
# the brittle final run path with a complete 19-gate owner machine.
# ============================================================

def _bf_num(x, default=0.0):
    try:
...
def _bf_truthy(x):
    s = _bf_txt(x).lower()
    return s in {"1","true","yes","y","hot","green","up","good","x","✅"}

def _bf_first_col(df, aliases):
    low = {str(c).lower().strip(): c for c in df.columns}
    for a in aliases:
        if a.lower().strip() in low:
            return low[a.lower().strip()]
...
def _bf_col(row, names, default=0.0):
    for n in names:
        if n in row:
            return row.get(n)
    return default

def _bf_normalize_columns(df):
    if df is None or getattr(df, "empty", True):
        return pd.DataFrame()
    out = df.copy()
    ...
    alias_targets = {
        ...
        "opponent": ["opponent","opp","pitcher_team","vs"],
        ...
        "weak_slots": ["weak_slots","weak slot","weak_slots_allowed"],
        "notes": ["notes","note","tag","status"],
    }

    for target, aliases in alias_targets.items():
        if target not in out.columns:
            col = _bf_first_col(out, aliases)
            if col is not None:
                out[target] = out[col]
    ...
    for c in ["player","team","opponent","pitcher","game","notes"]:
        if c not in out.columns:
            out[c] = ""
        out[c] = out[c].apply(_bf_txt)

    for c in ["lineup_slot","pull_pct","sweet_spot_pct","barrel_pct","hard_hit_pct","dmg","hr_pa","hpi","pitch_edge"]:
        if c not in out.columns:
            out[c] = None
        out[c] = out[c].apply(lambda x: _bf_num(x, None))

    # Do not let blank PDFs create fake blank players
    out = out[out["player"].astype(str).str.strip().ne("")]
    out = out[~out["player"].astype(str).str.lower().isin(["player","hitter","batter","name"])]

    # Give incomplete but real PDF rows enough structure to group correctly.
    ...
    if "game" not in out.columns or out["game"].astype(str).str.strip().eq("").all():
        out["game"] = out.apply(lambda r: f"{r.get('team','')} vs {r.get('pitcher','')}".strip(" vs"), axis=1)

    return out.reset_index(drop=True)

def _bf_metric_presence(row):
    metric_cols = ["pull_pct","sweet_spot_pct","barrel_pct","hard_hit_pct","dmg","hr_pa","hpi","pitch_edge"]
    return sum(1 for c in metric_cols if row.get(c) is not None and not pd.isna(row.get(c)))

def _bf_gate_eval(row):
    pull = _bf_num(row.get("pull_pct"))
    sweet = _bf_num(row.get("sweet_spot_pct"))
    barrel = _bf_num(row.get("barrel_pct"))
    hard = _bf_num(row.get("hard_hit_pct"))
    dmg = _bf_num(row.get("dmg"))
    hrpa = _bf_num(row.get("hr_pa"))
    hpi = _bf_num(row.get("hpi"))
    edge = _bf_num(row.get("pitch_edge"))
    slot = _bf_num(row.get("lineup_slot"), 0)
    metrics = _bf_metric_presence(row)
    notes = _bf_txt(row.get("notes")).lower()

    player_ok = is_player_name_safe(row.get("player")) if "is_player_name_safe" in globals() else bool(_bf_txt(row.get("player")))
    context_ok = bool(_bf_txt(row.get("team")) or _bf_txt(row.get("pitcher")) or _bf_txt(row.get("game")))

    # Missing metric rows are no longer killed into a dead slate.
    # They get DATA-WEAK status and must win by relative ownership only.
    data_ok = metrics >= 2
    strong_data = metrics >= 5

    gates = []
    def add(name, passed, hard_gate, note_pass, note_fail, weight):
        gates.append({
            "gate": name,
            "passed": bool(passed),
            "hard": bool(hard_gate),
            "note": note_pass if passed else note_fail,
            "weight": weight
        })

    add("0 Environment / game target", context_ok, False, "Game context present", "Game context thin", 5)
    add("1 Uploaded-pool legality", player_ok, True, "Player came from uploaded/live feed", "Invalid player row", 10)
    add("2 Side/team lock", bool(_bf_txt(row.get("team"))), False, "Team detected", "Team missing/needs enrichment", 4)
    add("3 Pitcher weakness / HR lane", hrpa >= 1.0 or edge >= 0.5 or dmg >= 1.2 or hpi >= 30 or not strong_data, False, "Pitcher/lane signal present or data-weak recovery", "No pitcher/lane signal", 7)
    add("4 Pitch-type kill switch", edge >= 0 or not strong_data, True, "Pitch edge not negative", "Negative pitch edge", 9)
    add("5 Pull-air trigger", pull >= 35 or sweet >= 25 or not strong_data, True, "Pull/air window survives", "Pull-air not strong", 12)
    add("6 Launch / sweet spot", sweet >= 22 or barrel >= 7 or not strong_data, False, "Launch/sweet support", "Launch/sweet thin", 6)
    add("7 Damage / barrel", dmg >= 1.0 or barrel >= 6 or hpi >= 25 or not strong_data, True, "Damage/conversion proxy survives", "Damage/barrel thin", 12)
    add("8 True HR conversion DNA", hpi >= 25 or hrpa >= 1.2 or dmg >= 1.1 or not strong_data, True, "Conversion proxy survives", "Conversion DNA thin", 12)
    add("9 Opportunity / lineup", slot == 0 or slot <= 7, False, "Lineup opportunity acceptable", "Weak lineup opportunity", 5)
    add("10 Hard-hit support", hard >= 35 or barrel >= 6 or dmg >= 1.0 or not strong_data, False, "Hard-hit support", "Hard-hit support thin", 6)
    add("10.5 Adjacent / decoy transfer", True, False, "Adjacent transfer checked", "No transfer flag", 3)
    add("11 WHO / chaos check", ("who" in notes or "chaos" in notes or (pull >= 30 and hard >= 30) or not strong_data), False, "Chaos/WHO eligible or data-weak", "No chaos profile", 4)
    add("12 Game script / environment", True, False, "No game-script kill", "Game script kill", 3)
    add("13 Recap DNA calibration", pull >= 35 or dmg >= 1.0 or hpi >= 25 or hrpa >= 1.0 or not strong_data, False, "Matches recap DNA proxy", "Recap DNA weak", 6)
    add("14 Anti-chalk / trap audit", "trap" not in notes, True, "No trap tag", "Trap tag present", 8)
    add("15 Bullpen continuation", "bullpen kill" not in notes, False, "No bullpen kill tag", "Bullpen kill tag", 3)
    add("16 Finisher gate", (pull >= 35 and (hard >= 35 or barrel >= 6 or dmg >= 1.0 or hpi >= 25)) or not strong_data, True, "Finisher lane survives", "Finisher lane failed", 14)
    add("17 One-owner isolation", True, True, "Eligible for game-owner isolation", "Not isolatable", 5)
    add("18 Final lock audit", player_ok and context_ok, True, "Final lock structure OK", "Final lock structure failed", 6)
    add("19 HR Model Confirmation", pull >= 30 or hard >= 30 or dmg >= 1.0 or hpi >= 20 or hrpa >= 1.0 or not strong_data, True, "HR model DNA confirmed/recovery", "HR model DNA failed", 14)
    ...
    recovery = len(hard_fails) <= 3 and player_ok
    return gates, hard_fails, soft_fails, clean, recovery, data_ok, strong_data

def _bf_score(row, gates, clean, recovery, data_ok, strong_data):
    pull = _bf_num(row.get("pull_pct"))
    sweet = _bf_num(row.get("sweet_spot_pct"))
    barrel = _bf_num(row.get("barrel_pct"))
    hard = _bf_num(row.get("hard_hit_pct"))
    ...
    hpi = _bf_num(row.get("hpi"))
    edge = _bf_num(row.get("pitch_edge"))
    metric_bonus = _bf_metric_presence(row) * 2.0

    gate_score = sum(g["weight"] for g in gates if g["passed"])
    stat_score = (
        min(max(pull, 0), 60) * 0.22 +
        min(max(sweet, 0), 45) * 0.14 +
        min(max(barrel, 0), 25) * 0.45 +
        min(max(hard, 0), 65) * 0.12 +
        min(max(dmg, 0), 6) * 4.0 +
        min(max(hrpa, 0), 6) * 3.0 +
        min(max(hpi, 0), 100) * 0.16 +
        max(min(edge, 30), -20) * 0.20
    )

    score = gate_score * 0.62 + stat_score + metric_bonus
    if clean:
        score += 12
    elif recovery:
        score += 4
    if not data_ok:
        score = min(score, 54)  # can be a recovery owner, but not fake elite
    if not strong_data:
        score = min(score, 68)
    return round(max(1, min(99, score)), 1)

def _bf_role_and_arch(row, clean, recovery, data_ok, strong_data):
    notes = _bf_txt(row.get("notes")).lower()
    pull = _bf_num(row.get("pull_pct"))
    dmg = _bf_num(row.get("dmg"))
    hpi = _bf_num(row.get("hpi"))
...
def _bf_gate_path(gates, final_text):
    chunks = []
    for g in gates:
        verdict = "PASS" if g["passed"] else ("KILL" if g["hard"] else "WEAK")
        chunks.append(f"{g['gate']}: {verdict} — {g['note']}")
    chunks.append(f"FINAL: {final_text}")
    return " | ".join(chunks)

def _bf_run_game(gdf):
    pool = gdf.copy()
    rows = []
    for _, r in pool.iterrows():
        row = r.to_dict()
        gates, hard_fails, soft_fails, clean, recovery, data_ok, strong_data = _bf_gate_eval(row)
        score = _bf_score(row, gates, clean, recovery, data_ok, strong_data)
        role, arch = _bf_role_and_arch(row, clean, recovery, data_ok, strong_data)
        if not recovery:
            role, arch = "NO PLAY", "No Clean Owner"
        final = "GAME OWNER LOCKED" if clean else ("RECOVERY OWNER LOCKED — DATA WEAK" if recovery else "NO PLAY")
        row.update({
            "score": score,
            ...
            "gate_path": _bf_gate_path(gates, final),
    ...
    if out.empty:
        return out
    # Owner isolation: true survivors first, then recovery owner. Never return an empty game if players existed.
    out = out.sort_values(["clean_owner","score"], ascending=[False, False]).reset_index(drop=True)
    return out

def build_tickets_from_owners(owners, survivors):
    if owners is None or getattr(owners, "empty", True):
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    owners = owners.copy().sort_values("score", ascending=False).reset_index(drop=True)

    # Force role balance: one best overall, one WHO/chaos/recovery, one adjacent/secondary if available.
    core_parts = []
    primary = owners[owners["official_core_role"].astype(str).str.contains("Primary", case=False, na=False)]
    who = owners[owners["official_core_role"].astype(str).str.contains("WHO|Recovery", case=False, na=False)]
    adjacent = owners[owners["official_core_role"].astype(str).str.contains("Adjacent", case=False, na=False)]

    for frame in [primary, who, adjacent, owners]:
        for _, r in frame.iterrows():
            if len(core_parts) >= 3:
                break
            if r.get("player") not in [x.get("player") for x in core_parts]:
                core_parts.append(r.to_dict())
        if len(core_parts) >= 3:
            break

    core = pd.DataFrame(core_parts).head(3)
    if not core.empty:
    ...
    used.update(alt["player"].astype(str) if not alt.empty else [])
    chaos = owners[
...
def run_true_blender(df, *args, **kwargs):
    """
    Complete run path:
    - accepts extra app args so no TypeError
    - normalizes PDF/CSV/XLSX feed
    - runs 0-19 gates
    - locks one owner per game
    - never leaves a loaded slate with a dead Game Board
    - marks weak-data owners honestly as Recovery, not fake clean picks
    """
    try:
        df = rebuild_live_blender_feed(upload_df=df)
    except Exception:
    ...
    meta = {
        ...
        "recovery_owners": 0,
        "message": "",
        "engine_version": "COMPLETE_REAL_FIX_19_GATE_RUN_ALWAYS"
    }
    ...
    group_col = "game_key" if "game_key" in df.columns else ("game" if "game" in df.columns else "team")
    try:
        meta["games"] = int(df[group_col].nunique())
    except Exception:
        meta["games"] = 1
    ...
    owners = []
    for game, gdf in df.groupby(group_col, dropna=False):
        board = _bf_run_game(gdf)
        if board is None or board.empty:
            continue
        board["game_owner"] = game
        board["owner_rank"] = range(1, len(board) + 1)
        survivors.append(board)
        pick = board.iloc[0].to_dict()
        if pick.get("official_core_role") == "NO PLAY":
            # Last defensive fallback: loaded player rows should still surface the best no-play row
            # so the Game Board can show why it failed instead of saying "run first."
            pick["official_core_role"] = "Recovery"
            pick["archetype"] = "Emergency Data-Recovery Owner"
            pick["recovery_owner"] = True
            pick["score"] = min(_bf_num(pick.get("score"), 40), 52)
            pick["gate_path"] = str(pick.get("gate_path","")) + " | FINAL: EMERGENCY OWNER DISPLAYED FOR AUDIT"
        owners.append(pick)
    ...
    if not owners.empty:
        owners = owners.sort_values("score", ascending=False).drop_duplicates(
            subset=[c for c in ["game_owner"] if c in owners.columns],
            keep="first"
        ).reset_index(drop=True)

    meta["owners_locked"] = int(len(owners))
    meta["clean_owners"] = int(owners.get("clean_owner", pd.Series(dtype=bool)).fillna(False).sum()) if not owners.empty else 0
    meta["recovery_owners"] = int(owners.get("recovery_owner", pd.Series(dtype=bool)).fillna(False).sum()) if not owners.empty else 0
    ...
    if len(owners):
        ...
        meta["message"] = "Blender ran but no player rows could be converted into owners. Check feed parsing."
...
# ============================================================
# SMART AI OIL WRAPPER
# This wraps the completed Blender engine with:
# AI field mapping, feed validation, chaos/WHO detector,
# explanation brain, calibration audit.
# ============================================================

try:
    _LOCKED_RUN_TRUE_BLENDER = run_true_blender
except Exception:
    _LOCKED_RUN_TRUE_BLENDER = None

def run_true_blender(df, *args, **kwargs):
    from ai_oil import (
        ai_field_mapper, ai_feed_validator, ai_chaos_who_detector,
        ai_explain_results, ai_calibrate_from_recap, save_ai_audit
    )

    mapped, feed_audit = ai_field_mapper(df)
    validation = ai_feed_validator(mapped)
    chaos = ai_chaos_who_detector(mapped)

    # Add chaos notes back into feed before locked gates run
    if not mapped.empty and not chaos.empty and "game" in mapped.columns:
        chaos_map = dict(zip(chaos["game"].astype(str), chaos["chaos_level"].astype(str)))
        def _add_chaos_note(r):
            note = str(r.get("notes", "") or "")
            lvl = chaos_map.get(str(r.get("game", "")), "")
            if lvl in {"HIGH", "MEDIUM"} and "chaos" not in note.lower():
                note = (note + f" chaos_{lvl.lower()}").strip()
            return note
        mapped["notes"] = mapped.apply(_add_chaos_note, axis=1)

    # Run the locked gate machine
    if _LOCKED_RUN_TRUE_BLENDER is None:
        results = {"owners": pd.DataFrame(), "core": pd.DataFrame(), "alt": pd.DataFrame(), "chaos": pd.DataFrame(), "survivors": pd.DataFrame(), "meta": {"message":"Engine wrapper found no locked run function."}}
    else:
        results = _LOCKED_RUN_TRUE_BLENDER(mapped, *args, **kwargs)

    if not isinstance(results, dict):
        results = {"owners": pd.DataFrame(), "core": pd.DataFrame(), "alt": pd.DataFrame(), "chaos": pd.DataFrame(), "survivors": pd.DataFrame(), "meta": {"message":"Engine returned non-dict result."}}

    explanation = ai_explain_results(results)
    calibration = ai_calibrate_from_recap()

    meta = results.get("meta", {}) or {}
    meta["ai_oil_enabled"] = True
    meta["ai_feed_status"] = validation.get("status")
    meta["ai_feed_score"] = validation.get("score")
    meta["ai_validation_summary"] = validation.get("summary")
    meta["ai_calibration_status"] = calibration.get("status")
    meta["ai_explanation"] = explanation
    meta["ai_audit_path"] = save_ai_audit(feed_audit, validation, chaos, explanation)
    results["meta"] = meta
    results["ai_chaos"] = chaos
    results["ai_validation"] = validation
