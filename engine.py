...
ENGINE_VERSION = "V0224_REAL_18_GATE_SURVIVAL_ONLY"
...
def build_core_top3(owners: pd.DataFrame) -> pd.DataFrame:
    ...
    for role, slot, label, arch in required:
        ...
        ].sort_values("owner_key", ascending=False)
        if pool.empty:
            continue
...
def run_true_blender(df: pd.DataFrame, *args, **kwargs) -> Dict[str, Any]:
    ...
    meta = {
        ...
        "core_rule": "HARD_ARCHETYPE_LOCK__CORE1_PRIMARY__CORE2_ADJACENT__CORE3_WHO__NO_DUPLICATE_REFILL",
