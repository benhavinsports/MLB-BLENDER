...
def load_adaptive_weights():
    ...
    try:
        if RECAP_LOG_FILE.exists():
            ...
            if "hit_hr" in hist.columns and len(hist) >= 10:
                hit = hist[hist["hit_hr"].astype(str).str.lower().isin(["true","1","yes"])]
                if not hit.empty:
                    if "pull_pct" in hit and pd.to_numeric(hit["pull_pct"], errors="coerce").mean() >= 38: weights["pull"] += .15
                    if "sweet_spot_pct" in hit and pd.to_numeric(hit["sweet_spot_pct"], errors="coerce").mean() >= 24: weights["launch"] += .10
                    if "dmg" in hit and pd.to_numeric(hit["dmg"], errors="coerce").mean() >= 1.5: weights["damage"] += .15
                    if "hpi" in hit and pd.to_numeric(hit["hpi"], errors="coerce").mean() >= 35: weights["conversion"] += .10
    except Exception:
        pass
    return weights
...
def fetch_live_public_hitter_pool(date_iso=None, max_hitters_per_team=9):
    ...
    for _, s in slate.iterrows():
        ...
        for idx, h in enumerate(hitters, 1):
            rows.append({
                "game": s.get("game",""), "game_key": s.get("game_key",""), "team": s.get("team",""),
                "opponent": s.get("opponent",""), "pitcher": s.get("pitcher",""),
                "player": h.get("player",""), "mlb_id": h.get("mlb_id",""), "lineup_slot": idx,
                "source": "LIVE_PUBLIC_POOL",
                "pull_pct": None, "sweet_spot_pct": None, "barrel_pct": None, "hard_hit_pct": None,
                "dmg": None, "hr_pa": None, "hpi": None, "pitch_edge": None
            })
    df = pd.DataFrame(rows)
    if not df.empty:
        df = normalize_game_frame(df)
    meta["hitter_rows"] = int(len(df))
    return df, meta
