...
def gate_path(r):
    ...
    hpi=safe_num(r.get("hpi"))
    pe=safe_num(r.get("pitch_edge"))
    gates.append("0 Environment checked")
    gates.append("1 Legal hitter")
    gates.append("2 Pool build pass")
    gates.append("3 Archetype detected")
    gates.append("4 Pitch match pass" if pe >= 0 or pd.isna(r.get("pitch_edge")) else "4 Pitch match weak")
    gates.append("5 Pull-air pass" if pull >= 20 or pd.isna(r.get("pull_pct")) else "5 Pull-air weak")
    gates.append("6 Damage pass" if dmg >= .7 or bool(r.get("hr_alert")) or pd.isna(r.get("dmg")) else "6 Damage weak")
    gates.append("7 HR/PA pass" if hrpa >= 1.4 or bool(r.get("hr_alert")) or pd.isna(r.get("hr_pa")) else "7 HR/PA weak")
    gates.append("8 Conversion pass" if hpi >= 18 or bool(r.get("hr_alert")) or pd.isna(r.get("hpi")) else "8 Conversion weak")
    gates.append("9 Launch window pass" if sweet >= 23 or pd.isna(r.get("sweet_spot_pct")) else "9 Launch weak")
    gates.append("10 Conditions checked")
    gates.append("10.5 Transfer checked")
    gates.append("11 Bullpen continuation checked")
    gates.append("12 Ownership isolation checked")
    gates.append("13 Chaos detection checked")
    gates.append("14 Final survivor lock")
    return " | ".join(gates)
...
def run_game(gdf):
    """
    TRUE BLENDER: one game enters, gates eliminate until one owner survives.
    Not a simple rank list.
    """
    alive = gdf.copy()
    if alive.empty:
        return alive
    ...
    def apply_gate(pool, mask, gate_name):
        if pool.empty or len(pool) <= 1:
            return pool
        if mask.fillna(False).any():
            return pool[mask.fillna(False)].copy()
        # If no one passes, do not kill entire game; keep highest measurable score later.
        return pool

    # Gate 1: valid core metrics must exist. A hitter with no real HR metrics cannot own the game.
    metric_cols = ["pull_pct","sweet_spot_pct","dmg","hr_pa","hpi","pitch_edge"]
    has_metrics = alive[metric_cols].notna().sum(axis=1) >= 3
    alive = apply_gate(alive, has_metrics, "metric_presence")

    # Gate 2: pull-air / launch profile
    alive = apply_gate(alive, (alive["pull_pct"].fillna(20) >= 20) | (alive["sweet_spot_pct"].fillna(23) >= 25), "pull_launch")

    # Gate 3: damage / HR conversion
    alive = apply_gate(alive, (alive["dmg"].fillna(0) >= .7) | (alive["hr_pa"].fillna(0) >= 1.4) | alive["hr_alert"].fillna(False), "damage_conversion")

    # Gate 4: pitch-type lane cannot be negative unless other chaos flags are strong.
    alive = apply_gate(alive, (alive["pitch_edge"].fillna(0) >= 0) | alive["hr_alert"].fillna(False), "pitch_lane")

    # Gate 5: HPI/conversion intent
    alive = apply_gate(alive, (alive["hpi"].fillna(0) >= 18) | alive["hr_alert"].fillna(False) | (alive["dmg"].fillna(0) >= 1.5), "conversion_intent")

    # Gate 6: 10.5 transfer / weak slot boost only after basic gates.
    ...
    # Ownership isolation: choose one final owner by score + transfer/chaos context.
    alive["_owner_score"] = (
    ...
    # Return full post-gate ordered pool, first row is official owner.
    return alive.drop(columns=[c for c in ["_owner_score","_transfer_score","_weak_match"] if c in alive.columns])
