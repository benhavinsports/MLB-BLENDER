...
# ============================================================
# 1-5 EFFECTIVE ENGINE PATCH V6
# Smart validator + true gate ranking + AI Oil audit.
# No fake picks. No alphabetical picks. No name-only tickets.
# ============================================================

_EFFECTIVE_BASE_RUN = run_true_blender
...
def _e_get(row, names, default=None):
    for n in names:
        if n in row:
            v = row.get(n)
            try:
                ...
                pass
            return v
    compact = {str(k).lower().replace(" ","").replace("_","").replace("%","").replace("/",""): k for k in row.keys()}
    for n in names:
        nn = str(n).lower().replace(" ","").replace("_","").replace("%","").replace("/","")
...
def _e_metrics(row):
    aliases = {
        ...
        "sweet": ["sweet_spot_pct","sweet%","sweet spot","launch","launch_angle","la"],
        ...
        "slot": ["lineup_slot","slot","order","batting_order","bo"],
    }
    vals, count = {}, 0
    for k, names in aliases.items():
        raw = _e_get(row, names, None)
        if raw is None:
            vals[k] = None
            continue
        try:
            ...
            pass
        try:
            vals[k] = float(str(raw).replace("%","").replace("+","").replace(",","").strip())
            count += 1
        except Exception:
            vals[k] = None
    return vals, count

def validate_feed_for_blender(df):
    df = _bf_safe_df(df)
    if df.empty:
        return {"ok":False,"score":0,"message":"No feed rows loaded.","eligible_rows":0,"total_rows":0}
    eligible = 0
    for _, r in df.iterrows():
        ok, _ = _eligible_row(r.to_dict())
        eligible += int(ok)
    pct = eligible / max(1, len(df))
    ok = eligible > 0
    return {
        "ok": ok,
        ...
        "message": f"Feed validator: {eligible}/{len(df)} rows have enough real Blender metrics."
    }

def _gate_strength(x, floor, elite, pts):
...
def _eligible_row(row):
    vals, count = _e_metrics(row)
    trigger = (
        (vals["pull"] is not None and vals["pull"] >= 30) or
        (vals["hard"] is not None and vals["hard"] >= 30) or
        (vals["barrel"] is not None and vals["barrel"] >= 5) or
        (vals["dmg"] is not None and vals["dmg"] >= 0.8) or
        (vals["hpi"] is not None and vals["hpi"] >= 20) or
        (vals["hr_lane"] is not None and vals["hr_lane"] >= 0.8) or
        (vals["pitch_edge"] is not None and vals["pitch_edge"] >= 0)
    )
    return bool(count >= 3 and trigger), count

def _score_row(row):
    vals, count = _e_metrics(row)
    eligible, _ = _eligible_row(row)
    if not eligible:
        return 0.0, 0.0, count, "NO PICK - insufficient metrics"

    pull = vals["pull"] or 0; hard = vals["hard"] or 0; barrel = vals["barrel"] or 0
    sweet = vals["sweet"] or 0; dmg = vals["dmg"] or 0; hpi = vals["hpi"] or 0
    hr = vals["hr_lane"] or 0; edge = vals["pitch_edge"] if vals["pitch_edge"] is not None else -99
    slot = vals["slot"] or 0
    notes = " ".join([_e_txt(row.get(k)) for k in row.keys() if "note" in str(k).lower() or "tag" in str(k).lower() or "raw" in str(k).lower()]).lower()
    hard_fails = _e_txt(row.get("hard_fails")).lower()
    soft_fails = _e_txt(row.get("soft_fails")).lower()

    raw = 0; max_raw = 0
    def add(p, m):
        nonlocal raw, max_raw
        raw += max(0.0, float(p)); max_raw += float(m)

    add(5 if _e_txt(row.get("player")) else 0, 5)
    add(5 if (_e_txt(row.get("team")) or _e_txt(row.get("pitcher")) or _e_txt(row.get("game")) or _e_txt(row.get("game_owner"))) else 0, 5)
    add(max(_gate_strength(hr,.8,2.0,8), _gate_strength(edge,0,10,8), _gate_strength(dmg,.8,2.2,8), _gate_strength(hpi,20,70,8)), 8)
    add(8 if edge >= 0 else 0, 8)
    add(max(_gate_strength(pull,30,52,11), _gate_strength(sweet,20,35,11)), 11)
    add(max(_gate_strength(sweet,20,35,6), _gate_strength(barrel,5,14,6)), 6)
    add(max(_gate_strength(dmg,.8,2.0,11), _gate_strength(barrel,5,14,11), _gate_strength(hpi,20,70,11)), 11)
    add(max(_gate_strength(hpi,20,70,11), _gate_strength(hr,.8,2.0,11), _gate_strength(dmg,.8,2.2,11)), 11)
    add(5 if slot == 0 or slot <= 7 else 0, 5)
    add(max(_gate_strength(hard,30,55,6), _gate_strength(barrel,5,14,6), _gate_strength(dmg,.8,2.0,6)), 6)
    add(3,3)
    add(5 if ("who" in notes or "chaos" in notes or (pull >= 30 and hard >= 30)) else 0,5)
    add(3,3)
    add(max(_gate_strength(pull,30,52,7), _gate_strength(dmg,.8,2.0,7), _gate_strength(hpi,20,70,7), _gate_strength(hr,.8,2.0,7)),7)
    add(8 if "trap" not in notes else 0,8)
    add(3 if "bullpen kill" not in notes else 0,3)
    fin = 0
    if pull >= 35:
        fin = max(_gate_strength(hard,30,55,13), _gate_strength(barrel,5,14,13), _gate_strength(dmg,.8,2.0,13), _gate_strength(hpi,20,70,13))
    add(fin,13)
    add(5,5)
    add(6 if _e_txt(row.get("player")) and (_e_txt(row.get("team")) or _e_txt(row.get("pitcher")) or _e_txt(row.get("game")) or _e_txt(row.get("game_owner"))) else 0,6)
    add(max(_gate_strength(pull,30,52,12), _gate_strength(hard,30,55,12), _gate_strength(dmg,.8,2.0,12), _gate_strength(hpi,20,70,12), _gate_strength(hr,.8,2.0,12)),12)

    blender = (raw / max_raw) * 100
    if hard_fails and hard_fails not in {"nan","none"}:
        blender -= min(35, len([x for x in hard_fails.split(",") if x.strip()]) * 8)
    if soft_fails and soft_fails not in {"nan","none"}:
        blender -= min(14, len([x for x in soft_fails.split(",") if x.strip()]) * 2)
    if "trap" in notes: blender -= 20
    if "red" in notes: blender -= 8
    if "green" in notes: blender += 4
    if "adjacent" in notes or "decoy" in notes or "transfer" in notes: blender += 3
    if "who" in notes or "chaos" in notes: blender += 2

    support = min(pull,65)*.16 + min(hard,70)*.12 + min(barrel,25)*.50 + min(dmg,8)*2.8 + min(hpi,100)*.10 + min(hr,6)*2.0
    if edge != -99: support += max(min(edge,30),-20)*.20
    if slot and slot <= 4: support += 4
    elif slot and slot <= 6: support += 2

    reason = "ELIGIBLE"
    return round(max(1,min(100,blender)),1), round(max(1,min(100,support)),1), count, reason

def _apply_effective_scores(df):
    df = _bf_safe_df(df).copy()
    if df.empty: return df
    bs=[]; ss=[]; counts=[]; eligible=[]; reasons=[]
    for _, r in df.iterrows():
        b,s,c,reason = _score_row(r.to_dict())
        bs.append(b); ss.append(s); counts.append(c); eligible.append(reason=="ELIGIBLE"); reasons.append(reason)
    df["blender_score"] = bs
    df["support_score"] = ss
    df["score"] = df["blender_score"]
    df["metric_count"] = counts
    df["blender_eligible"] = eligible
    df["eligibility_reason"] = reasons
    if "official_core_role" not in df.columns: df["official_core_role"] = ""
    if "archetype" not in df.columns: df["archetype"] = ""
    df.loc[~df["blender_eligible"], "official_core_role"] = "NO PICK"
    df.loc[~df["blender_eligible"], "archetype"] = "Insufficient data — audit only"
    return df

def _build_effective_tickets(owners):
    owners = _apply_effective_scores(owners)
    owners = owners[owners["blender_eligible"]].copy() if not owners.empty else owners
    if owners.empty: return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    owners = owners.sort_values(["blender_score","support_score"], ascending=[False,False]).reset_index(drop=True)
    core = owners.head(3).copy(); core["ticket_role"] = "CORE"
    used = set(core["player"].astype(str)) if "player" in core else set()
    alt = owners[~owners["player"].astype(str).isin(used)].head(3).copy() if "player" in owners else pd.DataFrame()
    if not alt.empty: alt["ticket_role"] = "ALT"
    used.update(alt["player"].astype(str).tolist() if not alt.empty and "player" in alt else [])
    chaos = owners[owners["official_core_role"].astype(str).str.contains("WHO|Chaos|Recovery|Validated", case=False, na=False) & ~owners["player"].astype(str).isin(used)].head(3).copy() if "player" in owners else pd.DataFrame()
    if chaos.empty and "player" in owners: chaos = owners[~owners["player"].astype(str).isin(used)].head(3).copy()
    if not chaos.empty: chaos["ticket_role"] = "WHO"
    return core, alt, chaos

def run_true_blender(df, *args, **kwargs):
    validation = validate_feed_for_blender(df)
    results = _EFFECTIVE_BASE_RUN(df, *args, **kwargs)
    if not isinstance(results, dict): return results

    survivors = _apply_effective_scores(results.get("survivors", pd.DataFrame()))
    owners = pd.DataFrame()
    ...
    if not survivors.empty:
        ...
        if group_col:
            picks = []
            for game, g in survivors.groupby(group_col, dropna=False):
                eligible = g[g["blender_eligible"]].copy()
                if eligible.empty:
                    continue
                pick = eligible.sort_values(["blender_score","support_score"], ascending=[False,False]).iloc[0].to_dict()
                pick["game_owner"] = game
                if _e_txt(pick.get("official_core_role")) in {"", "NO PLAY", "NO PICK"}:
                    pick["official_core_role"] = "Primary" if _e_num(pick.get("blender_score")) >= 75 else "Validated"
                picks.append(pick)
            owners = pd.DataFrame(picks)

    if owners.empty:
        owners = _apply_effective_scores(results.get("owners", pd.DataFrame()))
        owners = owners[owners["blender_eligible"]].copy() if not owners.empty else owners

    if not owners.empty:
        owners = owners.sort_values(["blender_score","support_score"], ascending=[False,False]).reset_index(drop=True)

    core, alt, chaos = _build_effective_tickets(owners)
    ...
    meta = results.get("meta", {}) or {}
    meta["score_engine"] = "EFFECTIVE_1_TO_5_V6"
    meta["feed_validation"] = validation
    meta["owners_locked"] = int(len(owners))
    if owners.empty:
        meta["message"] = f"Blender completed: no qualified owner survived. {validation['message']} Nothing random was forced."
    else:
        meta["message"] = f"Blender complete: {len(owners)} owners locked. {validation['message']}"
    results["meta"] = meta
