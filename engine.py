...
def run_true_blender(df, *args, **kwargs):
    """
    v94 guaranteed-run engine.
    It never returns a blank run:
    - reads feed
    - creates survivor rows
    - locks one best owner per game when possible
    - if data is weak, labels owner as Recovery instead of hiding output
    """
    try:
    ...
    meta = {
        ...
        "games": actual_game_count(df) if df is not None and not df.empty else 0,
        "owners_locked": 0,
        "survivor_rows": 0,
        "no_play_games": 0,
        "message": "",
        "engine_version": "v94_RUN_BUTTON_WORKING"
    }

    if df is None or df.empty:
        meta["message"] = "No feed rows available. Upload/feed data first."
        res = {"owners":pd.DataFrame(),"core":pd.DataFrame(),"alt":pd.DataFrame(),"chaos":pd.DataFrame(),"survivors":pd.DataFrame(),"meta":meta}
        save_locked_results(res)
        return res

    survivors = []
    owners = []
    group_col = "game_key" if "game_key" in df.columns else "game"

    for game, gdf in df.groupby(group_col, dropna=False):
        try:
            g = _run_game(gdf)
        except Exception as e:
            # never allow one game to kill the whole run
            g = gdf.copy()
            if g.empty:
                continue
            for col in ["score","official_core_role","archetype","gate_grade","hard_fails","soft_fails","gate_path"]:
                if col not in g.columns:
                    g[col] = ""
            g["score"] = 40.0
            g["official_core_role"] = "Recovery"
            g["archetype"] = "Engine Recovery"
            g["gate_grade"] = 40.0
            g["hard_fails"] = f"Engine recovered game: {e}"
            g["soft_fails"] = ""
            g["gate_path"] = f"Engine recovered game: {e}"

        if g is None or g.empty:
            continue

        g = g.copy().assign(game_owner=game)
        survivors.append(g)

        # pick best visible row per game; do not hide if role is Recovery
        if "score" not in g.columns:
            g["score"] = 40.0
        g2 = g.sort_values("score", ascending=False).reset_index(drop=True)
        top = g2.iloc[0].to_dict()

        # If the engine marked NO PLAY but there is a real player, promote to Recovery so the run visibly completes.
        if top.get("official_core_role") == "NO PLAY" and is_player_name_safe(top.get("player","")):
            ...
            top["score"] = max(40.0, min(58.0, safe_num(top.get("score"), 40)))
            top["gate_path"] = str(top.get("gate_path","")) + " | V94 visible recovery: weak data, not hidden"
    ...
    if owners.empty and not survivors.empty:
        # absolute fallback: first survivor becomes Recovery owner
        top = survivors.sort_values("score", ascending=False).head(1).copy() if "score" in survivors.columns else survivors.head(1).copy()
        top["official_core_role"] = "Recovery"
        top["archetype"] = "Visible Recovery / Weak Data"
        top["score"] = top.get("score", pd.Series([45.0])).apply(lambda x: max(40.0, min(58.0, safe_num(x,45))))
        owners = top
        meta["owners_locked"] = int(len(owners))

    if owners.empty:
        meta["message"] = "Blender ran, but no valid player rows were found. Check feeder parsing."
    ...
    clean = int((owners["official_core_role"].astype(str)!="Recovery").sum()) if "official_core_role" in owners.columns else 0
    recovery = int((owners["official_core_role"].astype(str)=="Recovery").sum()) if "official_core_role" in owners.columns else 0
