...
APP_ENGINE_VERSION = "v136_FULL_IMPORT_COMPAT_FIX"
...
def results_to_csv_bytes(results):
    try:
        return csv_bytes(export_results_csv(results))
    except Exception:
        return b""
...
def safe_df(df=None):
    return df if df is not None and hasattr(df, "empty") else pd.DataFrame()

def as_dataframe(x=None):
    if x is None:
        return pd.DataFrame()
    if hasattr(x, "empty"):
        return x
...
def recalc_adaptive_weights_from_history(*args, **kwargs):
    return {"ok": True, "engine_version": globals().get("APP_ENGINE_VERSION", "v136_FULL_IMPORT_COMPAT_FIX")}
...
def clean_results_for_display(results):
    return safe_results(results)

def get_display_frames(results):
    results = safe_results(results)
    return {
        "core": results.get("core", pd.DataFrame()),
        "alt": results.get("alt", pd.DataFrame()),
        "chaos": results.get("chaos", pd.DataFrame()),
        "game_board": results.get("survivors", pd.DataFrame()),
    }
