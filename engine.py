...
ENGINE_VERSION = "V0214_FINAL_DEATH_CHAIN_ENGINE"
...
def run_true_blender(df: pd.DataFrame, *args, **kwargs) -> Dict[str, Any]:
    ...
    role_board = survivors[[c for c in ["game_id","game_key","player","official_core_role","true_role_path","role_scores_json","pass_depth","cut_depth","death_step","lane_index","launch_index","conversion_index","support_index"] if c in survivors.columns]].copy()
    ...
    meta = {
        ...
        "message": f"{ENGINE_VERSION}: Core={len(core)} from {len(owners)} isolated game owners. Final gate-power equalizer removed. No 99-score engine, no refill, no projection fallback.",
    }
    results = {
        "owners": owners,
        "core": core,
