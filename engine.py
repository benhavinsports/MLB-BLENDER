...
def evaluate_18_gates(r):
    """
    v90 19-gate Blender audit.
    Missing pitch data becomes a soft unknown, not a fake elite pass and not a total app blocker.
    """
    ...
    pitch = has_pitch and pe >= 0
    pitch_unknown = not has_pitch
    pitch_lane_ok = pitch or pitch_unknown
    ...
    gates=[
        ("0 Environment / game target",env_ok,False),
        ("1 Uploaded-pool legality",pool_ok,True),
        ("2 Side/team lock",env_ok,True),
        ("3 Pitcher weakness / HR lane",pitch_lane_ok or alert or dmg>=1.8,False),
        ("4 Pitch-type kill switch",pitch_lane_ok,False),
        ("5 Pull-air trigger",pull_air,True),
        ("6 Launch / sweet spot",launch,False),
        ("7 Damage / barrel",damage,True),
        ("8 True HR conversion DNA",conversion,True),
        ("9 Opportunity / lineup",opportunity,False),
        ("10 Hard-hit support",hardhit_support,False),
        ("10.5 Adjacent transfer",transfer,False),
        ("11 WHO / chaos",chaos,False),
        ("12 Game script",script,False),
        ("13 Recap DNA pattern",recap_dna,False),
        ("14 Anti-chalk trap",anti,True),
        ("15 Bullpen continuation",bullpen,False),
        ("16 Finisher gate",finisher,True),
        ("17 One-owner isolation",True,True),
        ("18 Final lock audit",True,True),
    ...
    return {
        "gates": gates, "hard_failed": hard, "soft_failed": soft, "grade": grade,
...
def blend_score(r):
    """
    v90 calibrated Blender score.
    This prevents fake 99s and rewards true HR-event profiles only.
    Clean elite range is earned; recovery/data-weak profiles are capped elsewhere.
    """
    ...
    # Tier 1 true HR traits
    score += min(18, max(0, (pull - 25) * 0.75))          # 40 pull ~= 11, 45 ~= 15
    score += min(12, max(0, (sweet - 18) * 0.60))         # launch window
    score += min(10, max(0, (barrel - 5) * 1.25))         # barrel
    score += min(8, max(0, (hh - 34) * 0.45))             # hard-hit support

    # Conversion / damage
    if dmg >= 2.2: score += 13
    elif dmg >= 1.8: score += 10
    elif dmg >= 1.5: score += 7
    elif dmg >= 1.2: score += 4

    if hrpa >= 4.0: score += 10
    elif hrpa >= 3.0: score += 8
    elif hrpa >= 2.0: score += 6
    elif hrpa >= 1.3: score += 3
    ...
    # Pitch edge is strong, but missing pitch edge cannot create elite score
    if has_pitch:
        if pe >= 15: score += 12
        elif pe >= 8: score += 9
        elif pe >= 0: score += 5
    ...
    # Gate support is capped; it cannot create a 99
    try:
        g = hard_gate_result(r)
        grade = _num(g.get("grade"))
        score += max(0, min(6, (grade - 55) * 0.08))
...
def run_true_blender(df, *args, **kwargs):
    ...
    meta={"input_rows":0 if df is None else int(len(df)),"games":0,"owners_locked":0,"survivor_rows":0,"no_play_games":0,"message":"","engine_version":"v90_STRONG_AI_OIL_19_GATE"}
    if df is None or df.empty:
        ...
        save_locked_results(empty); return empty
    ...
    for game,gdf in df.groupby(group_col,dropna=False):
        try:
            ...
            alive["official_core_role"]="NO PLAY"; alive["archetype"]="Engine Quarantine"; alive["score"]=0
            alive["gate_path"]=f"ENGINE ERROR: {e}"; alive["hard_fails"]=f"ENGINE ERROR: {e}"; alive["soft_fails"]=""
    ...
    if owners.empty:
        meta["message"]="Blender completed: no qualified owner survived. Game Board shows kill reasons."
