...
try:
    ...
    def run_true_blender(df, *args, **kwargs):
        ...
        if isinstance(res, dict) and "meta" in res:
            ...
            res["meta"]["message"] = f"Blender complete: {res['meta'].get('owners_locked',0)} owners from {res['meta'].get('attack_pools', res['meta'].get('games',0))} attack pools. Rows={len(fixed)}."
        return res
except NameError:
    pass
...
# StarTool PDFs contain attack-pool rows: TEAM vs OPPOSING PITCHER.
# Those are not the same as true MLB slate games.
# v111 separates:
# - slate_games = actual scheduled MLB games
# - attack_pools = team/pitcher attack pools parsed from PDF
# This prevents 26 attack pools from being displayed as 26 real games.
...
def slate_game_count_from_public_context(public_context=None, fallback_df=None):
    """
    Count true scheduled games from PUBLIC_SLATE rows when available.
    Public slate has two team rows per game, but same matchup text.
    """
    try:
        if public_context is not None and not public_context.empty and "game" in public_context.columns:
            return int(public_context["game"].dropna().astype(str).map(lambda x: canonical_game_key(x)).replace("", pd.NA).dropna().nunique())
    except Exception:
        pass
    try:
        if fallback_df is not None and not fallback_df.empty and "slate_game_key" in fallback_df.columns:
            return int(fallback_df["slate_game_key"].replace("", pd.NA).dropna().nunique())
    except Exception:
        pass
    # fallback: attack pool count, but UI should label it as attack pools
    return attack_pool_count(fallback_df)

def attach_slate_matchup_context(feed_df, public_context):
    """
    Attach true slate game keys if public slate is available.
    This does not overwrite hitter metrics; it only adds schedule context.
    """
    if feed_df is None or feed_df.empty:
        return feed_df
    out = feed_df.copy()
    if public_context is None or public_context.empty:
        out["attack_pool_key"] = out.apply(lambda r: canonical_game_key(r.get("team",""), r.get("opponent",""), r.get("pitcher","")), axis=1)
        return out
    ctx = public_context.copy()
    if "game" not in ctx.columns or "team" not in ctx.columns:
        return out
    ctx["team_norm_v111"] = ctx["team"].astype(str).map(_v110_team_key)
    ctx["slate_game_key"] = ctx["game"].astype(str).map(lambda x: canonical_game_key(x))
    ctx_small = ctx[["team_norm_v111","game","slate_game_key","opponent"]].drop_duplicates("team_norm_v111", keep="first")
    out["team_norm_v111"] = out["team"].astype(str).map(_v110_team_key) if "team" in out.columns else ""
    out = out.merge(ctx_small, on="team_norm_v111", how="left", suffixes=("","_slate"))
    out["attack_pool_key"] = out.apply(lambda r: canonical_game_key(r.get("team",""), r.get("opponent",""), r.get("pitcher","")), axis=1)
    return out.drop(columns=[c for c in ["team_norm_v111"] if c in out.columns])

try:
    _run_true_blender_pre_v111 = run_true_blender
    def run_true_blender(df, *args, **kwargs):
        fixed = normalize_game_frame(df)
        res = _run_true_blender_pre_v111(fixed, *args, **kwargs)
        if isinstance(res, dict) and "meta" in res:
            res["meta"]["engine_version"] = "v111_TRUE_SLATE_GAME_COUNT_FIX"
            res["meta"]["attack_pools"] = attack_pool_count(fixed)
            # Keep games as true slate when already provided by UI/session; otherwise attack pools.
            res["meta"]["games"] = res["meta"].get("slate_games", res["meta"].get("games", attack_pool_count(fixed)))
        return res
except NameError:
    pass
...
# -------------------- v114 UNIQUE TICKET BUCKET LOCK --------------------
# One player can appear in ONE official ticket bucket only.
# Priority order: CORE first, then ALT, then CHAOS.
# No duplicate player across Core/Alt/Chaos.

def _v114_player_set(df):
    if df is None or not hasattr(df, "empty") or df.empty or "player" not in df.columns:
        return set()
    return set(df["player"].astype(str).str.lower().str.strip().tolist())

try:
    _build_tickets_from_owners_pre_v114 = build_tickets_from_owners

    def build_tickets_from_owners(owners, survivors=None):
        core, alt, chaos = _build_tickets_from_owners_pre_v114(owners, survivors)

        # Core locks first.
        used = _v114_player_set(core)

        # ALT cannot contain Core players.
        if alt is not None and hasattr(alt, "empty") and not alt.empty and "player" in alt.columns:
            alt = alt[~alt["player"].astype(str).str.lower().str.strip().isin(used)].copy()
            used.update(_v114_player_set(alt))

        # CHAOS cannot contain Core OR ALT players.
        if chaos is not None and hasattr(chaos, "empty") and not chaos.empty and "player" in chaos.columns:
            chaos = chaos[~chaos["player"].astype(str).str.lower().str.strip().isin(used)].copy()

        # Keep each bucket internally unique too.
        for name, df in [("core", core), ("alt", alt), ("chaos", chaos)]:
            if df is not None and hasattr(df, "empty") and not df.empty and "player" in df.columns:
        ...
        return (
            core.reset_index(drop=True) if core is not None and hasattr(core, "reset_index") else pd.DataFrame(),
            alt.reset_index(drop=True) if alt is not None and hasattr(alt, "reset_index") else pd.DataFrame(),
            chaos.reset_index(drop=True) if chaos is not None and hasattr(chaos, "reset_index") else pd.DataFrame(),
        )
...
try:
    _run_true_blender_pre_v114 = run_true_blender

    def run_true_blender(df, *args, **kwargs):
        res = _run_true_blender_pre_v114(df, *args, **kwargs)
        if isinstance(res, dict):
            owners = res.get("owners", pd.DataFrame())
            core, alt, chaos = build_tickets_from_owners(owners, res.get("survivors", pd.DataFrame()))
            res["core"] = core
            res["alt"] = alt
            res["chaos"] = chaos
            if "meta" not in res or res["meta"] is None:
                res["meta"] = {}
            res["meta"]["engine_version"] = "v114_UNIQUE_TICKET_BUCKETS_NO_CODE_LEAK"
        return res
...
# -------------------- v115 PDF TRUTH + BUCKET ASSIGNMENT NOT BLOCKING --------------------
# PDF rows are authoritative for player/team/pitcher.
# Public slate/context must never overwrite PDF player/team/pitcher.
# Ticket buckets assign/refill from remaining eligible picks instead of blocking and shrinking.

def _v115_clean(x):
    return re.sub(r"\s+", " ", str(x or "").strip())

def _v115_player_key(x):
    return _v115_clean(x).lower()
...
def _v115_is_recovery(row):
    txt = (str(row.get("official_core_role","")) + " " + str(row.get("archetype",""))).lower()
    return "recovery" in txt or "audit" in txt or "data-recovery" in txt

def _v115_clean_ticket(row):
    return (not _v115_is_recovery(row)) and (not _v115_hard(row)) and _v115_score(row) >= 50

def _v115_gate_pass(row, gate):
    return f"{gate.lower()}: pass" in _v115_path(row)

def _v115_gate_weak(row, gate):
    return gate.lower() in _v115_soft(row) or f"{gate.lower()}: weak" in _v115_path(row)

def _v115_primary_ok(row):
    if not _v115_clean_ticket(row): return False
    return not _v115_gate_weak(row, "5 Pull-air") and not _v115_gate_weak(row, "16 Finisher")

def _v115_adjacent_ok(row):
    if not _v115_clean_ticket(row): return False
    return _v115_gate_pass(row, "10.5 Adjacent") and not _v115_gate_weak(row, "10.5 Adjacent")

def _v115_who_ok(row):
    if not _v115_clean_ticket(row): return False
    return _v115_gate_pass(row, "11 WHO/Chaos") and not _v115_gate_weak(row, "11 WHO/Chaos")

def _v115_pick(pool, func, used):
    if pool is None or pool.empty:
        return pd.DataFrame()
    d = pool[pool.apply(func, axis=1)].copy()
    if "player" in d.columns:
        d = d[~d["player"].astype(str).str.lower().str.strip().isin(used)]
    if d.empty:
        return pd.DataFrame()
    return d.sort_values("score", ascending=False).head(1).copy()

def _v115_take_many(pool, func, used, n=3):
    if pool is None or pool.empty:
        return pd.DataFrame()
    d = pool[pool.apply(func, axis=1)].copy()
    if "player" in d.columns:
...
def build_tickets_from_owners(owners, survivors=None):
    """
    Assignment, not blocking:
    - Core attempts Primary, Adjacent, WHO, but only if each truly passes.
    - ALT refills from clean non-Core survivors.
    - Chaos refills from WHO-pass players not already in Core or ALT.
    - Player can appear in only one official bucket.
    """
    pool = owners.copy() if owners is not None and hasattr(owners, "empty") and not owners.empty else pd.DataFrame()
    if pool.empty and survivors is not None and hasattr(survivors, "empty") and not survivors.empty:
        pool = survivors.copy()
    if pool.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    primary = _v115_pick(pool, _v115_primary_ok, used)
    if not primary.empty:
        primary["official_core_role"] = "Primary"
        primary["archetype"] = "Primary HR Owner"
        core_parts.append(primary)
        used.update(primary["player"].astype(str).str.lower().str.strip().tolist())

    adjacent = _v115_pick(pool, _v115_adjacent_ok, used)
    if not adjacent.empty:
        adjacent["official_core_role"] = "Adjacent"
        adjacent["archetype"] = "Adjacent / Transfer Owner"
        core_parts.append(adjacent)
        used.update(adjacent["player"].astype(str).str.lower().str.strip().tolist())

    who = _v115_pick(pool, _v115_who_ok, used)
    if not who.empty:
        who["official_core_role"] = "WHO"
        who["archetype"] = "WHO / Chaos Owner"
        core_parts.append(who)
        used.update(who["player"].astype(str).str.lower().str.strip().tolist())

    core = pd.concat(core_parts, ignore_index=True) if core_parts else pd.DataFrame()

    alt = _v115_take_many(pool, _v115_clean_ticket, used, 3)
    if not alt.empty:
        alt["official_core_role"] = "ALT"
        alt["archetype"] = alt["archetype"].fillna("Clean Blender Alternate")
        used.update(alt["player"].astype(str).str.lower().str.strip().tolist())

    chaos = _v115_take_many(pool, _v115_who_ok, used, 3)
    if not chaos.empty:
        chaos["official_core_role"] = "WHO"
        chaos["archetype"] = "WHO / Chaos Owner"
...
def merge_public_context(df, public_context=None):
    """
    v115: Never merge public context into uploaded PDF player rows.
    Public data can exist separately for slate count only.
    This prevents player/team/pitcher cross-contamination.
    """
    if df is None or df.empty:
        return pd.DataFrame()
    out = df.copy()
    # Preserve source-of-truth columns exactly.
    for c in ["player","team","pitcher"]:
        if c in out.columns:
            out[c] = out[c].astype(str).map(_v115_clean)
    if "game" not in out.columns or out["game"].astype(str).str.strip().eq("").all():
        out["game"] = out.apply(lambda r: f"{r.get('team','')} vs {r.get('pitcher','')}", axis=1)
    else:
        # For StarTool PDF rows, display as team vs pitcher to avoid wrong matchup merges.
        out["game"] = out.apply(lambda r: f"{r.get('team','')} vs {r.get('pitcher','')}" if str(r.get("pitcher","")).strip() else str(r.get("game","")), axis=1)
    try:
        out = normalize_game_frame(out)
    except Exception:
        pass
    return out
...
def attach_slate_matchup_context(feed_df, public_context):
    """
    v115: Attach only slate_game_key metadata. Do NOT overwrite player/team/pitcher/game from PDF.
    """
    if feed_df is None or feed_df.empty:
        return feed_df
    out = feed_df.copy()
    out = merge_public_context(out, None)
    if public_context is not None and hasattr(public_context, "empty") and not public_context.empty and "game" in public_context.columns:
        try:
            slate_games = public_context["game"].dropna().astype(str).map(lambda x: canonical_game_key(x)).replace("", pd.NA).dropna().nunique()
            out["slate_game_count"] = int(slate_games)
        except Exception:
            pass
    return out

try:
    _run_true_blender_pre_v115 = run_true_blender
    def run_true_blender(df, *args, **kwargs):
        fixed = merge_public_context(df, None)
        res = _run_true_blender_pre_v115(fixed, *args, **kwargs)
        if isinstance(res, dict):
            owners = res.get("owners", pd.DataFrame())
            core, alt, chaos = build_tickets_from_owners(owners, res.get("survivors", pd.DataFrame()))
            res["core"] = core
            ...
            if "meta" not in res or res["meta"] is None:
                res["meta"] = {}
            res["meta"]["engine_version"] = "v115_PDF_TRUTH_NO_MIX_BUCKET_ASSIGN"
            res["meta"]["input_rows"] = len(fixed)
            try:
                res["meta"]["attack_pools"] = attack_pool_count(fixed)
            except Exception:
...
# Prevents impossible rows like Byron Buxton showing on Kansas City.
# Runs BEFORE Blender scoring/tickets.
# PDF stays source of truth for metrics, but player-team identity is sanity checked.

KNOWN_PLAYER_TEAM_V117 = {
...
def _v117_norm_player(x):
    s = re.sub(r"\s+", " ", str(x or "").strip().lower())
    s = s.replace("’", "'")
    return s

def _v117_norm_team(x):
    s = re.sub(r"\s+", " ", str(x or "").strip())
    return s

def _v117_apply_known_player_team_lock(df):
    if df is None or df.empty or "player" not in df.columns:
        return df
    out = df.copy()
    if "team" not in out.columns:
        out["team"] = ""
    if "integrity_note" not in out.columns:
        out["integrity_note"] = ""
    corrections = []
    for idx, row in out.iterrows():
        pkey = _v117_norm_player(row.get("player",""))
        locked_team = KNOWN_PLAYER_TEAM_V117.get(pkey)
        if locked_team:
            old_team = _v117_norm_team(row.get("team",""))
            if old_team and old_team.lower() != locked_team.lower():
                out.at[idx, "team"] = locked_team
                out.at[idx, "integrity_note"] = f"TEAM_CORRECTED_FROM_{old_team}_TO_{locked_team}"
                corrections.append((str(row.get("player","")), old_team, locked_team))
    if corrections:
        out.attrs["player_team_corrections"] = corrections
    return out

def _v117_rebuild_display_game(df):
    if df is None or df.empty:
        return df
    out = df.copy()
    if "team" in out.columns and "pitcher" in out.columns:
        out["game"] = out.apply(
            lambda r: f"{r.get('team','')} vs {r.get('pitcher','')}" if str(r.get("pitcher","")).strip() else str(r.get("game","")),
            axis=1
        )
    return out

try:
    _merge_public_context_pre_v117 = merge_public_context
    def merge_public_context(df, public_context=None):
        out = _merge_public_context_pre_v117(df, None)
        out = _v117_apply_known_player_team_lock(out)
        out = _v117_rebuild_display_game(out)
        try:
            out = normalize_game_frame(out)
        except Exception:
            pass
...
try:
    _attach_slate_matchup_context_pre_v117 = attach_slate_matchup_context
    def attach_slate_matchup_context(feed_df, public_context):
        out = _attach_slate_matchup_context_pre_v117(feed_df, public_context)
        out = _v117_apply_known_player_team_lock(out)
        out = _v117_rebuild_display_game(out)
        try:
            out = normalize_game_frame(out)
...
try:
    _run_true_blender_pre_v117 = run_true_blender
    def run_true_blender(df, *args, **kwargs):
        fixed = _v117_apply_known_player_team_lock(df)
        fixed = _v117_rebuild_display_game(fixed)
        res = _run_true_blender_pre_v117(fixed, *args, **kwargs)
        if isinstance(res, dict):
            # Re-apply to all output tables too.
            for k in ["owners", "core", "alt", "chaos", "survivors"]:
                d = res.get(k)
                if d is not None and hasattr(d, "empty") and not d.empty:
                    d = _v117_apply_known_player_team_lock(d)
                    d = _v117_rebuild_display_game(d)
                    try:
                        d = normalize_game_frame(d)
                    except Exception:
                        pass
                    res[k] = d
            if "meta" not in res or res["meta"] is None:
                res["meta"] = {}
            res["meta"]["engine_version"] = "v117_PLAYER_TEAM_INTEGRITY_GUARD"
            corr = getattr(fixed, "attrs", {}).get("player_team_corrections", [])
            if corr:
                res["meta"]["player_team_corrections"] = corr
        return res
except NameError:
    pass
...
PUBLIC_PRIMARY_STAR_NAMES_V123 = {
    "kyle schwarber","austin riley","aaron judge","shohei ohtani","mookie betts","freddie freeman",
    "matt olson","jose ramirez","josé ramírez","vladimir guerrero jr","vladimir guerrero jr.",
    "bobby witt jr","bobby witt jr.","james wood","byron buxton","pete alonso","yordan alvarez",
    "juan soto","rafael devers"
}
def _r123_num(row, col, default=0.0):
    try:
...
def _r123_txt(row):
    return " ".join(str(row.get(c,"")) for c in ["player","team","pitcher","game","official_core_role","archetype","gate_path","soft_fails","hard_fails","event_note","parse_note","tags"]).lower()
def _r123_player(row):
    return re.sub(r"\s+"," ",str(row.get("player","")).strip().lower())
def _r123_dna(row):
    pull=_r123_num(row,"pull_pct"); sweet=_r123_num(row,"sweet_spot_pct"); barrel=_r123_num(row,"barrel_pct")
    hard=_r123_num(row,"hard_hit_pct"); dmg=_r123_num(row,"dmg"); hrpa=_r123_num(row,"hr_pa")
    hpi=_r123_num(row,"hpi"); pitch=_r123_num(row,"pitch_edge")
    dna=0
    dna += 24 if pull>=35 else 17 if pull>=30 else 10 if pull>=25 else 1
    dna += 12 if sweet>=28 else 7 if sweet>=24 else 1
    dna += 16 if barrel>=10 else 10 if barrel>=7 else 2
    dna += 8 if hard>=42 else 3
    dna += 22 if dmg>=1.8 else 15 if dmg>=1.35 else 7 if dmg>=1.05 else 0
    dna += 22 if hrpa>=3.0 else 15 if hrpa>=1.8 else 8 if hrpa>=1.0 else 0
    dna += 8 if hpi>=42 else 4 if hpi>=28 else 0
    dna += min(10,pitch/2) if pitch>0 else 0
    return dna
def _r123_primary_score(row):
    txt=_r123_txt(row); val=_r123_dna(row)
    if "5 pull-air: pass" in txt: val+=8
    if "16 finisher: pass" in txt: val+=8
    if "8 conversion: pass" in txt: val+=6
    if "primary" in txt: val+=8
    if "5 pull-air: weak" in txt or "16 finisher: weak" in txt: val-=24
    if str(row.get("hard_fails","")).strip(): val-=150
    return max(0,val)
def _r123_adjacent_score(row):
    txt=_r123_txt(row); old=_r123_num(row,"raw_blender_score",_r123_num(row,"score")); val=0
    if "10.5 adjacent: pass" in txt: val+=34
    elif "adjacent" in txt and "10.5 adjacent: weak" not in txt: val+=10
    if _r123_dna(row)>=38 and old<78: val+=12
    if _r123_player(row) in PUBLIC_PRIMARY_STAR_NAMES_V123: val-=16
    if "10.5 adjacent: weak" in txt: val-=26
    if str(row.get("hard_fails","")).strip(): val-=150
    return max(0,val+_r123_dna(row)*0.45)
def _r123_who_score(row):
    txt=_r123_txt(row); old=_r123_num(row,"raw_blender_score",_r123_num(row,"score")); name=_r123_player(row); val=0
    if "11 who/chaos: pass" in txt: val+=36
    elif "who" in txt or "chaos" in txt: val+=10
    if _r123_dna(row)>=34 and old<68: val+=18
    elif _r123_dna(row)>=40 and old<76: val+=8
    if name in PUBLIC_PRIMARY_STAR_NAMES_V123:
        val-=35
        if "11 who/chaos: pass" in txt and old<70: val+=8
    if "11 who/chaos: weak" in txt: val-=35
    if "primary" in txt and "11 who/chaos: pass" not in txt: val-=22
    if str(row.get("hard_fails","")).strip(): val-=150
    return max(0,val+_r123_dna(row)*0.30)
def _r123_mistake(row):
    return (15 if _r123_num(row,"pitch_edge")>0 and _r123_num(row,"dmg")>=1.25 else 0)+(10 if _r123_num(row,"hr_pa")>=1.2 and _r123_num(row,"barrel_pct")>=7 else 0)
def _r123_role(row):
    txt=_r123_txt(row)
    name=_r123_player(row)
    ps=_r123_primary_score(row); ad=_r123_adjacent_score(row); wh=_r123_who_score(row)
    # Role identity first: a true WHO pass on a non-primary-star belongs in WHO pool.
    if "11 who/chaos: pass" in txt and name not in PUBLIC_PRIMARY_STAR_NAMES_V123 and "11 who/chaos: weak" not in txt and wh>=24:
        return "WHO"
    # Public primary stars cannot become WHO unless they are not carrying primary identity.
    if wh>=32 and wh>=ad and wh>=ps and name not in PUBLIC_PRIMARY_STAR_NAMES_V123:
        return "WHO"
    if ad>=28 and ad>=wh:
        return "Adjacent"
    if ps>=30:
    ...
    return "ALT"
def _r123_score(row):
    role=_r123_role(row); old=_r123_num(row,"raw_blender_score",_r123_num(row,"score")); mis=_r123_mistake(row)
    if role=="WHO": val=_r123_who_score(row)*0.95+mis*0.35+old*0.05
    elif role=="Adjacent": val=_r123_adjacent_score(row)*0.95+mis*0.35+old*0.05
    elif role=="Primary": val=_r123_primary_score(row)*0.80+mis*0.25+old*0.10
    else: val=_r123_dna(row)*0.55+mis*0.25+old*0.08
    return max(0,min(100,val))
def apply_role_locked_event_engine_v123(df):
    if df is None or not hasattr(df,"empty") or df.empty: return df
    out=df.copy()
    if "raw_blender_score" not in out.columns and "score" in out.columns: out["raw_blender_score"]=out["score"]
    out["conversion_dna"]=out.apply(_r123_dna,axis=1).round(1)
    out["primary_event_score"]=out.apply(_r123_primary_score,axis=1).round(1)
    out["adjacent_transfer_score"]=out.apply(_r123_adjacent_score,axis=1).round(1)
    out["who_chaos_score"]=out.apply(_r123_who_score,axis=1).round(1)
    out["mistake_pitch_score"]=out.apply(_r123_mistake,axis=1).round(1)
    out["event_role"]=out.apply(_r123_role,axis=1)
    out["event_isolation_score"]=out.apply(_r123_score,axis=1).round(1)
    out["score"]=out["event_isolation_score"]
    out["event_note"]=out["event_role"].map({"Primary":"PRIMARY_EVENT_OWNER","Adjacent":"ADJACENT_TRANSFER_EVENT_OWNER","WHO":"WHO_CHAOS_EVENT_OWNER","ALT":"ALT_EVENT_SURVIVOR"})
    return out.sort_values("score",ascending=False).reset_index(drop=True)
def _v123_take(pool,used,n=1):
...
def build_tickets_from_owners(owners,survivors=None):
    ...
    if ranked is None or not hasattr(ranked,"empty") or ranked.empty: return pd.DataFrame(),pd.DataFrame(),pd.DataFrame()
    ranked=apply_role_locked_event_engine_v123(ranked)
    used=set(); parts=[]
    pools={"Primary":ranked[ranked["event_role"].eq("Primary")].copy(),"Adjacent":ranked[ranked["event_role"].eq("Adjacent")].copy(),"WHO":ranked[ranked["event_role"].eq("WHO")].copy(),"ALT":ranked[ranked["event_role"].isin(["Primary","Adjacent","ALT"])].copy()}
    for role,arch in [("Primary","Primary Event Owner"),("Adjacent","Adjacent / Transfer Event Owner"),("WHO","WHO / Chaos Event Owner")]:
        one=_v123_take(pools[role],used,1)
        if not one.empty:
            one["official_core_role"]=role; one["archetype"]=arch
    ...
    alt=_v123_take(pools["ALT"],used,3)
    if not alt.empty:
        alt["official_core_role"]="ALT"; alt["archetype"]="Event-Calibrated Alternate"
        used.update(alt["player"].astype(str).str.lower().str.strip().tolist())
    chaos=_v123_take(pools["WHO"],used,3)
    if not chaos.empty:
        chaos["official_core_role"]="WHO"; chaos["archetype"]="WHO / Chaos Event Owner"
    return core.reset_index(drop=True),alt.reset_index(drop=True),chaos.reset_index(drop=True)
_run_true_blender_pre_v123=run_true_blender
def run_true_blender(df,*args,**kwargs):
    res=_run_true_blender_pre_v123(df,*args,**kwargs)
    if not isinstance(res,dict): return res
    for k in ["owners","survivors"]:
        d=res.get(k)
        if d is not None and hasattr(d,"empty") and not d.empty: res[k]=apply_role_locked_event_engine_v123(d)
    core,alt,chaos=build_tickets_from_owners(res.get("owners",pd.DataFrame()),res.get("survivors",pd.DataFrame()))
    res["core"]=core; res["alt"]=alt; res["chaos"]=chaos
    if "meta" not in res or res["meta"] is None: res["meta"]={}
    res["meta"]["engine_version"]="v123_ROLE_LOCKED_EVENT_ENGINE"
    res["meta"]["role_lock"]="primary/adjacent/who pools separated before ticket build"
    return res



# -------------------- v125 NO STALE RESULTS / DAILY ONLY --------------------
# Prevents yesterday's picks from ever loading into today's app.
# Saved/locked results are disabled for live daily runs.
...
def load_locked_results(*args, **kwargs):
    return {
        ...
        "meta": {"engine_version": "v125_NO_STALE_RESULTS_DAILY_ONLY", "loaded_saved_results": False}
    }
...
def save_locked_results(*args, **kwargs):
    # Disabled by design. Results live in current session only.
    return True

try:
    _run_true_blender_pre_v125 = run_true_blender
    def run_true_blender(df, *args, **kwargs):
        # Every run is from current df only. No external saved picks.
        res = _run_true_blender_pre_v125(df, *args, **kwargs)
        if isinstance(res, dict):
            if "meta" not in res or res["meta"] is None:
                res["meta"] = {}
            res["meta"]["engine_version"] = "v125_NO_STALE_RESULTS_DAILY_ONLY"
            res["meta"]["stale_results_blocked"] = True
            res["meta"]["run_source"] = "current_uploaded_or_current_live_feed_only"
        return res
except NameError:
    pass
...
# -------------------- v126 FRESH SLATE / ANTI-REPEAT EVENT LAYER --------------------
FAILED_RECENT_OWNER_NAMES_V126 = {
    "kyle schwarber", "austin riley", "byron buxton", "james wood", "nick kurtz",
    "jac caglianone"
}
PUBLIC_STAR_ANCHOR_NAMES_V126 = {
...
def _v126_name(row):
    return re.sub(r"\s+", " ", str(row.get("player","")).strip().lower())
def _v126_num(row, col, default=0.0):
    try:
        v = row.get(col, default)
        if pd.isna(v):
            return default
...
def _v126_txt(row):
    return " ".join(str(row.get(c,"")) for c in [
        ...
        "gate_path","soft_fails","hard_fails","event_note","parse_note","tags"
    ]).lower()
def _v126_repeat_exception(row):
    txt = _v126_txt(row)
    flags = 0
    ...
    if _v126_num(row, "pull_pct") >= 34 and _v126_num(row, "dmg") >= 1.5: flags += 1
    if _v126_num(row, "barrel_pct") >= 10 and _v126_num(row, "hr_pa") >= 1.8: flags += 1
    return flags >= 3
def _v126_freshness_adjust(row):
    name = _v126_name(row)
    penalty = 0.0
    boost = 0.0
    txt = _v126_txt(row)
    if name in FAILED_RECENT_OWNER_NAMES_V126 and not _v126_repeat_exception(row):
        penalty += 38
    if name in PUBLIC_STAR_ANCHOR_NAMES_V126:
        penalty += 12
        if _v126_repeat_exception(row):
            penalty -= 18
    raw = _v126_num(row, "raw_blender_score", _v126_num(row, "score"))
    event = _v126_num(row, "event_isolation_score", _v126_num(row, "score"))
    if raw < 70 and event >= 45:
        boost += 12
    if "11 who/chaos: pass" in txt and name not in PUBLIC_STAR_ANCHOR_NAMES_V126:
        boost += 14
    if "10.5 adjacent: pass" in txt and name not in PUBLIC_STAR_ANCHOR_NAMES_V126:
        boost += 10
    return boost - penalty
def apply_fresh_slate_anti_repeat_v126(df):
    if df is None or not hasattr(df, "empty") or df.empty:
        return df
    out = df.copy()
    if "pre_fresh_score" not in out.columns and "score" in out.columns:
        out["pre_fresh_score"] = out["score"]
    out["fresh_slate_adjust"] = out.apply(_v126_freshness_adjust, axis=1).round(1)
    out["repeat_exception"] = out.apply(_v126_repeat_exception, axis=1)
    out["score"] = (out["score"].astype(float) + out["fresh_slate_adjust"].astype(float)).clip(lower=0, upper=100).round(1)
    out["fresh_note"] = out.apply(
        lambda r: "REPEAT_ALLOWED_TODAY_EVIDENCE" if _v126_name(r) in FAILED_RECENT_OWNER_NAMES_V126 and r.get("repeat_exception")
        else ("RECENT_FAIL_SUPPRESSED" if _v126_name(r) in FAILED_RECENT_OWNER_NAMES_V126 else "FRESH_SLATE"),
        axis=1
    )
    return out.sort_values("score", ascending=False).reset_index(drop=True)
try:
    _run_true_blender_pre_v126 = run_true_blender
    def run_true_blender(df, *args, **kwargs):
        res = _run_true_blender_pre_v126(df, *args, **kwargs)
        if not isinstance(res, dict):
            return res
        for k in ["owners","survivors"]:
            d = res.get(k)
            if d is not None and hasattr(d, "empty") and not d.empty:
                res[k] = apply_fresh_slate_anti_repeat_v126(d)
        try:
            core, alt, chaos = build_tickets_from_owners(res.get("owners", pd.DataFrame()), res.get("survivors", pd.DataFrame()))
            res["core"] = apply_fresh_slate_anti_repeat_v126(core) if core is not None and hasattr(core, "empty") and not core.empty else core
            res["alt"] = apply_fresh_slate_anti_repeat_v126(alt) if alt is not None and hasattr(alt, "empty") and not alt.empty else alt
            res["chaos"] = apply_fresh_slate_anti_repeat_v126(chaos) if chaos is not None and hasattr(chaos, "empty") and not chaos.empty else chaos
        except Exception:
            pass
        if "meta" not in res or res["meta"] is None:
            res["meta"] = {}
        res["meta"]["engine_version"] = "v126_FRESH_SLATE_ANTI_REPEAT"
        res["meta"]["anti_repeat"] = "active: recent failed owners suppressed unless repeat exception passes"
        return res
except NameError:
    pass
...
STAR_PRIMARY_NAMES_V127 = {
    "kyle schwarber","austin riley","aaron judge","shohei ohtani","mookie betts","freddie freeman",
    "matt olson","jose ramirez","josé ramírez","vladimir guerrero jr","vladimir guerrero jr.",
    "bobby witt jr","bobby witt jr.","james wood","byron buxton","pete alonso","yordan alvarez",
    "juan soto","rafael devers","nick kurtz"
}
def _v127_name(row):
    return re.sub(r"\s+"," ",str(row.get("player","")).strip().lower())
def _v127_txt(row):
    return " ".join(str(row.get(c,"")) for c in ["player","team","pitcher","game","official_core_role","archetype","gate_path","soft_fails","hard_fails","event_note","fresh_note","tags"]).lower()
def _v127_num(row,col,default=0.0):
    try:
        v=row.get(col,default)
        if pd.isna(v): return default
        return float(v)
        ...
        return default
def _v127_weak_count(row):
    txt=_v127_txt(row); soft=str(row.get("soft_fails","")).lower()
    count=0
    for g in ["5 pull-air","8 conversion","9 opportunity","10.5 adjacent","11 who/chaos","16 finisher"]:
        if f"{g}: weak" in txt or g in soft:
            count += 1
    return count
def _v127_primary_ok(row):
    txt=_v127_txt(row)
    if str(row.get("hard_fails","")).strip(): return False
    if any(x in txt for x in ["5 pull-air: weak","16 finisher: weak","8 conversion: weak"]): return False
    return _v127_num(row,"pull_pct")>=30 and _v127_num(row,"dmg")>=1.25 and (_v127_num(row,"hr_pa")>=1.2 or _v127_num(row,"barrel_pct")>=7)
def _v127_adjacent_ok(row):
    txt=_v127_txt(row)
    if str(row.get("hard_fails","")).strip(): return False
    if "10.5 adjacent: weak" in txt: return False
    if "10.5 adjacent: pass" not in txt: return False
    if _v127_name(row) in STAR_PRIMARY_NAMES_V127 and _v127_primary_ok(row): return False
    return _v127_num(row,"pull_pct")>=25 and _v127_num(row,"dmg")>=1.0
def _v127_who_ok(row):
    txt=_v127_txt(row)
    if str(row.get("hard_fails","")).strip(): return False
    if "11 who/chaos: weak" in txt: return False
    if "11 who/chaos: pass" not in txt and "who" not in txt and "chaos" not in txt: return False
    if _v127_primary_ok(row): return False
    if _v127_name(row) in STAR_PRIMARY_NAMES_V127 and "11 who/chaos: pass" not in txt: return False
    return _v127_num(row,"pull_pct")>=22 and _v127_num(row,"dmg")>=0.8
def _v127_dna(row):
    pull=_v127_num(row,"pull_pct"); sweet=_v127_num(row,"sweet_spot_pct"); barrel=_v127_num(row,"barrel_pct"); dmg=_v127_num(row,"dmg"); hrpa=_v127_num(row,"hr_pa"); hpi=_v127_num(row,"hpi")
    return ((22 if pull>=35 else 16 if pull>=30 else 8 if pull>=25 else 2) +
...
def _v127_role(row):
    if _v127_primary_ok(row): return "Primary"
    if _v127_who_ok(row): return "WHO"
    if _v127_adjacent_ok(row): return "Adjacent"
    return "CUT"
def _v127_ceiling(row, role):
    weak=_v127_weak_count(row)
    if role=="CUT": return 49
    if weak>=3: return 59
    if weak==2: return 74
    if weak==1: return 88
    return 100
def _v127_score(row):
    role=_v127_role(row); raw=_v127_num(row,"raw_blender_score",_v127_num(row,"score")); dna=_v127_dna(row)
    mistake=10 if _v127_num(row,"pitch_edge")>0 and _v127_num(row,"dmg")>=1.25 else 0
    if role=="Primary": score=raw*.18+dna*.78+mistake
    elif role=="Adjacent": score=raw*.10+dna*.55+24+mistake
    elif role=="WHO": score=raw*.06+dna*.45+28+mistake
    else: score=min(raw,49)
    return round(max(0,min(_v127_ceiling(row,role),score)),1)
def apply_hard_role_lock_v127(df):
    if df is None or not hasattr(df,"empty") or df.empty: return df
    out=df.copy()
    if "raw_blender_score" not in out.columns and "score" in out.columns: out["raw_blender_score"]=out["score"]
    out["weak_gate_count"]=out.apply(_v127_weak_count,axis=1)
    out["event_role"]=out.apply(_v127_role,axis=1)
    out["score_ceiling"]=out.apply(lambda r:_v127_ceiling(r,r["event_role"]),axis=1)
    out["conversion_dna"]=out.apply(_v127_dna,axis=1).round(1)
    out["event_isolation_score"]=out.apply(_v127_score,axis=1)
    out["score"]=out["event_isolation_score"]
    ...
    out["event_note"]=out["event_role"].map({"Primary":"PRIMARY_EVENT_OWNER","Adjacent":"ADJACENT_TRANSFER_EVENT_OWNER","WHO":"WHO_CHAOS_EVENT_OWNER","CUT":"CUT_BY_ROLE_LOCK"})
    return out[out["event_role"].ne("CUT")].sort_values("score",ascending=False).reset_index(drop=True)
def _v127_take(pool,used,n=1):
    if pool is None or not hasattr(pool,"empty") or pool.empty: return pd.DataFrame()
    d=pool.copy()
    if "player" in d.columns:
        d=d[~d["player"].astype(str).str.lower().str.strip().isin(used)].drop_duplicates(subset=["player"],keep="first")
    return d.sort_values("score",ascending=False).head(n).copy()
def build_tickets_from_owners(owners,survivors=None):
    ranked=owners if owners is not None and hasattr(owners,"empty") and not owners.empty else survivors
    ranked=apply_hard_role_lock_v127(ranked)
    if ranked is None or not hasattr(ranked,"empty") or ranked.empty: return pd.DataFrame(),pd.DataFrame(),pd.DataFrame()
    used=set(); parts=[]
    for role,arch in [("Primary","Primary Event Owner"),("Adjacent","Adjacent / Transfer Event Owner"),("WHO","WHO / Chaos Event Owner")]:
        one=_v127_take(ranked[ranked["event_role"].eq(role)].copy(),used,1)
        if not one.empty:
            one["official_core_role"]=role; one["archetype"]=arch
            used.update(one["player"].astype(str).str.lower().str.strip().tolist()); parts.append(one)
    core=pd.concat(parts,ignore_index=True) if parts else pd.DataFrame()
    alt=_v127_take(ranked[ranked["event_role"].isin(["Primary","Adjacent"])].copy(),used,3)
    if not alt.empty:
        alt["official_core_role"]="ALT"; alt["archetype"]="Role-Locked Alternate"
        used.update(alt["player"].astype(str).str.lower().str.strip().tolist())
    chaos=_v127_take(ranked[ranked["event_role"].eq("WHO")].copy(),used,3)
    if not chaos.empty:
        chaos["official_core_role"]="WHO"; chaos["archetype"]="WHO / Chaos Event Owner"
    return core.reset_index(drop=True),alt.reset_index(drop=True),chaos.reset_index(drop=True)
try:
    _run_true_blender_pre_v127=run_true_blender
    def run_true_blender(df,*args,**kwargs):
        res=_run_true_blender_pre_v127(df,*args,**kwargs)
        if not isinstance(res,dict): return res
        for k in ["owners","survivors"]:
            d=res.get(k)
            if d is not None and hasattr(d,"empty") and not d.empty: res[k]=apply_hard_role_lock_v127(d)
        core,alt,chaos=build_tickets_from_owners(res.get("owners",pd.DataFrame()),res.get("survivors",pd.DataFrame()))
        res["core"]=core; res["alt"]=alt; res["chaos"]=chaos
        if "meta" not in res or res["meta"] is None: res["meta"]={}
        res["meta"]["engine_version"]="v127_HARD_ROLE_LOCK_SCORE_CAPS"
        res["meta"]["role_lock"]="mutual exclusive Primary/Adjacent/WHO"
        res["meta"]["score_caps"]="active"
        return res
except NameError:
    pass
