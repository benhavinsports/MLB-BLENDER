...
def build_core_alt(owners):
    ...
    for _,r in pool.iterrows():
        ...
        g=r.get("game_owner_key",r.get("game",""))
        if g not in used_games:
            core.append(r.to_dict()); used_games.add(g)
...
def run_machine(df):
    ...
    for g,gdf in df.groupby("game",dropna=False):
        ...
        if not lg.empty:
            lg.insert(0,"Game",g); logs.append(lg)
        if not alive.empty:
            alive=alive.copy()
            ...
            owners.append(top)
            survivors.append(alive.assign(game_owner_key=g))
...
def audit_feed(df):
    ...
    unknown=int(game.str.contains("Unknown Game",na=False).sum())
    fake_team=team.str.match(r"^Team\\s+\\d+$",na=False).any()
    fake_pitcher=pitcher.str.match(r"^Pitcher\\s+\\d+$",na=False).any()

    if blank_team: issues.append(f"Team missing on {blank_team} rows.")
    if blank_pitcher: issues.append(f"Pitcher missing on {blank_pitcher} rows.")
    if unknown: issues.append(f"Unknown game label on {unknown} rows.")
    if fake_team or fake_pitcher: issues.append("Fake team/pitcher labels detected.")

    # Full slate should separate into multiple games.
    if len(df)>=80 and game.nunique()<6:
        issues.append("Full-slate game separation failed: too few games extracted.")
