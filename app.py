...
from engine import run_true_blender, run_recap_check, csv_bytes, load_locked_results, fetch_live_public_slate, merge_public_context, fetch_live_public_hitter_pool
...
with tabs[0]:
    ...
    if st.button("RUN LIVE PUBLIC BLENDER", key="run_live_public_blender_v78", use_container_width=True):
        try:
            ...
            st.success(f"Live public Blender finished: {len(results.get('owners', []))} locked owners")
            st.rerun()
        except Exception as e:
