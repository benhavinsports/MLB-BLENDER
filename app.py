...
with tabs[0]:
    ...
    st.markdown("### RUN THE MACHINE")
    if st.button("RUN FULL BLENDER ENGINE", use_container_width=True, key="manual_blender_run_final"):
        try:
            if st.session_state.feed_df is not None and not st.session_state.feed_df.empty:
                ...
                st.session_state.results = results
                st.session_state.machine_state = "OWNERS LOCKED"
                st.success("Blender engine completed successfully.")
                st.rerun()
            else:
                st.warning("Upload a feed first.")
        except Exception as e:
            st.error(f"Engine failed safely: {e}")
