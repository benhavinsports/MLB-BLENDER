...
from ui import inject_css, hero, blender_machine, tickets_view, game_board_view, recap_view, blender_visual, compact_game_board_view
...
with tabs[0]:
    ...
    if uploaded is not None:
        try:
            with st.spinner("Fast-reading feed (OCR skipped to prevent hang)..."):
            ...
            if df is None or df.empty:
                ...
                st.success(f"Fast feed loaded: {len(df)} usable rows. Click RUN BLENDER NOW.")
    ...
    st.markdown("---")
    compact_game_board_view(st.session_state.results,"main_gb_v107")
    st.markdown("---")
...
with tabs[2]:
    compact_game_board_view(st.session_state.results,"tab_gb_v107")
