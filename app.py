...
def official_core_bucket(r):
    a=str(r.get("archetype","")).lower()
    role_txt=str(r.get("role","")).lower()
    if any(x in a for x in ["who","chaos"]) or "chaos" in role_txt:
        return "WHO"
    if any(x in a for x in ["adjacent","transfer","weak-slot","weak slot"]) or "transfer" in role_txt:
        return "Adjacent"
    return "Primary"

def label_official_buckets(df):
    if df is None or df.empty:
        return df
    out=df.copy()
    out["official_core_role"]=out.apply(official_core_bucket, axis=1)
    return out
...
def run_blender(df):
    ...
    for game,gdf in run.groupby("game",dropna=False):
        alive=run_game(gdf)
        if "sanitize_players_df" in globals():
            alive=sanitize_players_df(alive)
        else:
            alive=alive[alive["player"].astype(str).apply(is_player_name)]
        if alive is None or alive.empty:
            continue
        alive=label_official_buckets(alive)
    ...
    if "sanitize_players_df" in globals():
        owners=sanitize_players_df(owners)
    else:
        owners=owners[owners["player"].astype(str).apply(is_player_name)].copy()
    owners=label_official_buckets(owners)
    ...
    # Official Blender Core 3: one Primary, one Adjacent, one WHO.
    core_rows=[]
    used_games=set()
    used_players=set()

    def add_bucket(bucket):
        pool=owners[owners["official_core_role"]==bucket].sort_values("score",ascending=False)
        for _,r in pool.iterrows():
            game=str(r.get("game",""))
            player=str(r.get("player",""))
            if game in used_games or player in used_players:
                continue
            d=r.to_dict()
            d["ticket_role"]=bucket
            core_rows.append(d)
            used_games.add(game)
    ...
    for bucket in ["Primary","Adjacent","WHO"]:
        add_bucket(bucket)

    # If one bucket has no natural survivor, fill with best remaining but preserve official ticket slot.
    for bucket in ["Primary","Adjacent","WHO"]:
        if not any(x.get("ticket_role")==bucket for x in core_rows):
            for _,r in owners.sort_values("score",ascending=False).iterrows():
                game=str(r.get("game",""))
                player=str(r.get("player",""))
                if game in used_games or player in used_players:
                    continue
                d=r.to_dict()
                d["ticket_role"]=bucket
                d["official_core_role"]=bucket
                core_rows.append(d)
                used_games.add(game)
                used_players.add(player)
                break

    core=pd.DataFrame(core_rows[:3]) if core_rows else pd.DataFrame()

    # ALT 3: next backups, no same player as core.
    alt_rows=[]
    alt_used=set(used_players)
    for _,r in owners.sort_values("score",ascending=False).iterrows():
        player=str(r.get("player",""))
        if player in alt_used:
            continue
        d=r.to_dict()
        d["ticket_role"]="ALT"
        alt_rows.append(d)
        ...
        if len(alt_rows)>=3:
            break
    alt=pd.DataFrame(alt_rows[:3]) if alt_rows else pd.DataFrame()

    # CHAOS 3: WHO/chaos/weak-slot/alert profiles first.
    chaos_source=surv.copy() if surv is not None and not surv.empty else owners.copy()
    if "sanitize_players_df" in globals():
        chaos_source=sanitize_players_df(chaos_source)
    else:
        chaos_source=chaos_source[chaos_source["player"].astype(str).apply(is_player_name)].copy()
    chaos_source=label_official_buckets(chaos_source)

    chaos_rows=[]
    if chaos_source is not None and not chaos_source.empty:
        chaos_source["_chaos_score"]=(
            chaos_source["score"].fillna(0)*.35
            + chaos_source["dmg"].fillna(0)*20
            + chaos_source["hr_pa"].fillna(0)*7
            + chaos_source["weak_slot_tag"].fillna(False).astype(int)*15
            + chaos_source["hr_alert"].fillna(False).astype(int)*12
            + (chaos_source["official_core_role"]=="WHO").astype(int)*18
        )
        chaos_source=chaos_source.sort_values("_chaos_score",ascending=False).drop(columns=["_chaos_score"])
        chaos_used=set()
        for _,r in chaos_source.iterrows():
            player=str(r.get("player",""))
            if player in chaos_used:
                continue
            d=r.to_dict()
            d["ticket_role"]="WHO"
            chaos_rows.append(d)
            chaos_used.add(player)
            if len(chaos_rows)>=3:
                break

    chaos=pd.DataFrame(chaos_rows[:3]) if chaos_rows else pd.DataFrame()

    owners=label_official_buckets(owners)
    surv=label_official_buckets(surv) if surv is not None and not surv.empty else surv
...
def card(r):
    ...
    st.markdown(f"""<div class="card{elite_class}" style="border-color:{color}99"><span class="role">{r.get('ticket_role', r.get('official_core_role', r.get('role','')))}</span><span class="badge">{r.get('archetype','')}</span><span class="fire-tag">{fire}</span><div class="card-name">{'🔥 ' if elite else ''}{r.get('player','')}</div><div class="meta"><span style="color:{color};font-weight:900">{r.get('team','')}</span> vs {r.get('pitcher','')} · Slot {fmt(r.get('lineup_slot'))}</div><div class="score">{fmt(r.get('score'))}<small style="display:block;font-family:Rajdhani;color:#aaa;font-size:13px">TRUE BLEND SCORE</small></div><div class="bar"><div class="fill" style="width:{p}%"></div></div><div class="stats"><div class="stat"><b>{fmt(r.get('pull_pct'))}</b><span>Pull</span></div><div class="stat"><b>{fmt(r.get('pitch_edge'))}</b><span>Pitch</span></div><div class="stat"><b>{fmt(r.get('dmg'))}</b><span>DMG</span></div><div class="stat"><b>{fmt(r.get('hr_pa'))}</b><span>HR/PA</span></div><div class="stat"><b>{fmt(r.get('hpi'))}</b><span>HPI</span></div><div class="stat"><b>{fmt(r.get('sweet_spot_pct'))}</b><span>Sweet</span></div></div><div class="reason"><b>Blend path:</b> {short_path}</div></div>""",unsafe_allow_html=True)
    ...
    for label,key in [("CORE 3","core"),("ALT 3","alt"),("CHAOS 3","chaos")]:
        ...
        if df is None or df.empty: st.info("Run the Blender first.")
            ...
            for _,r in df.iterrows():
                ...
                st.markdown(f"<div class='leg'><span><b>{fire}{r.get('player','')}</b><br><small>{r.get('ticket_role', r.get('official_core_role',''))} · {r.get('archetype','')} · {r.get('team','')} vs {r.get('pitcher','')}</small></span><span class='odds'>{int(r.get('score',0))}%</span></div>",unsafe_allow_html=True)
...
with tabs[2]:
    ...
    if owners is None or owners.empty: st.info("Run the Blender first.")
        ...
        with st.expander("All Survivors + Gates"):
            cols=[c for c in ["page","game","player","team","pitcher","official_core_role","ticket_role","archetype","role","score","weak_slots","gate_path","notes"] if c in surv.columns]
