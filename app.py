...
def audit_feed(df):
    ...
    if int((team=="").sum()):
        issues.append(f"Team missing on {int((team=='').sum())} rows.")
    if int((pitcher=="").sum()):
        issues.append(f"Pitcher missing on {int((pitcher=='').sum())} rows.")
    if int(game.str.contains("Unknown Game",na=False).sum()):
        issues.append(f"Unknown game label on {int(game.str.contains('Unknown Game',na=False).sum())} rows.")
    ...
    metric_cols=["pull_pct","barrel_pct","sweet_spot_pct","hard_hit_pct","hpi","dmg","hr_pa","pitch_edge"]
    blank=int(df[metric_cols].isna().all(axis=1).sum())
    if blank:
        issues.append(f"{blank} rows have no usable hitter metrics.")
