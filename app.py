...
def run(df, label):
    ...
    if "meta" in st.session_state.results:
        st.session_state.results["meta"]["slate_games"] = st.session_state.slate_games or slate_game_count_from_public_context(st.session_state.public_context, st.session_state.feed_df)
        st.session_state.results["meta"]["attack_pools"] = attack_pool_count(st.session_state.feed_df)
        st.session_state.results["meta"]["games"] = st.session_state.results["meta"]["slate_games"]
        st.session_state.results["meta"]["message"] = f"Blender complete: {st.session_state.results['meta'].get('owners_locked',0)} owners from {st.session_state.results['meta'].get('slate_games',0)} slate games. Attack pools={st.session_state.results['meta'].get('attack_pools',0)}. Rows={len(df)}."
...
with tabs[0]:
    ...
    if uploaded is not None:
        try:
            with st.spinner("Reading feed..."):
                ...
                # v111: auto-load public slate context so "Games" = true scheduled MLB games,
                # while PDF team/pitcher sides remain "Attack Pools".
                try:
    ...
    if df is not None and not df.empty:
        c = st.columns(5)
        true_games = st.session_state.slate_games or slate_game_count_from_public_context(st.session_state.public_context, df)
