...
BAD_PLAYER_TOKENS = set("VS PROJECTED PITCHER TEAM LINEUP SLOT BATS HAND ALERT DMG HPI PULL SWEET STAR TOOL DATA PAGE HOME AWAY NONE SUMMARY DETAILS WEAK COND LINE FRESH MODERATE ELEVATED HOT COLD WARM PLATOON LASER RAKES TODAY TODAY\'S GAMES STRONG WEAKNESS THREAT ENVIRONMENT PARK ODDS VALUE OWNERSHIP PUBLIC SHARP LOCK CORE ALT CHAOS".split())
ALIASES = {
...
GATES = [
"0 Target Game/Pitcher", "1 Legal Hitter Identity", "2 Pull-Air Gate", "3 Contact Damage Gate",
"4 Pitch-Type Edge", "5 Weak Slot / Lane Match", "6 HR Conversion", "7 Launch/Sweet Spot",
"8 Recent Condition", "9 Lineup Opportunity", "10 Public/Book Decoy", "10.5 Adjacent Transfer",
"11 Pitcher Mistake Lane", "12 Bullpen Continuation", "13 Numerology Tie-Break",
"14 Chaos/WHO Detection", "15 Finisher Gate", "16 One Owner Isolation",
"17 No Revival Audit", "18 Final Lock"
]
...
/* v50 upload-as-feed-player + active blender */
div[data-testid="stFileUploader"]{
    border:1px dashed rgba(217,255,47,.45)!important;
    border-radius:26px!important;
    background:linear-gradient(135deg,rgba(217,255,47,.10),rgba(0,255,115,.04))!important;
...
div[data-testid="stFileUploader"] button{
    background:linear-gradient(90deg,var(--acid),var(--green))!important;
...
.machine-overlay{
    ...
    width:86%;height:82%;border-radius:36px;pointer-events:none;
    background:radial-gradient(circle at center,rgba(217,255,47,.10),transparent 52%);
}
.result-pulse{
    position:absolute;left:50%;bottom:52px;transform:translateX(-50%);
    font-family:'Black Ops One';color:#050505;background:linear-gradient(90deg,var(--acid),var(--green),var(--orange));
    padding:10px 18px;border-radius:999px;z-index:10;animation:pulsepop 1.4s infinite;
}
@keyframes pulsepop{50%{transform:translateX(-50%) scale(1.06);filter:brightness(1.25)}}
...
def is_player_name(text):
    ...
    # Single-word player names are allowed only if they look like real surnames, not machine labels.
    if " " not in s and u in BAD_PLAYER_TOKENS.union({"FRESH","MODERATE","ELEVATED","PLATOON","HOME","AWAY","HOT","COLD","WARM"}):
        return False
    parts=s.split()
    if not (1 <= len(parts) <= 4): return False
    if any(p.upper() in BAD_PLAYER_TOKENS for p in parts): return False
...
def normalize_df(df, source="structured"):
    ...
    # v51: if structured row has matchup/game but blank team/pitcher, try to infer from text.
    def _infer_team_pitcher(row):
        game_txt=str(row.get("game",""))
        team=str(row.get("team","")).strip()
        pitcher=str(row.get("pitcher","")).strip()
        if not team:
            for ab,name in TEAM_ABBR.items():
                if re.search(rf"\b{re.escape(ab)}\b", game_txt.upper()):
                    team=name; break
            if not team:
                for name in TEAM_NAMES:
                    if name.lower() in game_txt.lower():
                        team=name; break
        if not pitcher:
            m=re.search(r"vs\s+([A-Z][A-Za-zÀ-ÿ.'’\-]+(?:\s+[A-Z][A-Za-zÀ-ÿ.'’\-]+){0,3})", game_txt)
            if m and is_player_name(m.group(1)):
                pitcher=m.group(1)
        row["team"]=team
        row["pitcher"]=pitcher
        return row
    df=df.apply(_infer_team_pitcher, axis=1)
...
def pdf_pages(file_bytes):
    ...
    for idx,page in enumerate(doc,start=1):
        text,mode=page_text_with_optional_ocr(page)
        lines=[x.strip() for x in text.splitlines() if x.strip()]
        out.append({"page":idx,"text":text,"mode":mode,"lines":lines})
    return out
...
def parse_pdf(file_bytes):
    ...
    for page in pages:
        ...
        for blk in player_blocks(page["lines"]):
            ...
            row.update({"source":page.get("mode","pdf"),"page":page["page"],"team":active_team,"pitcher":active_pitcher,
...
def map_archetype(r):
    pull=r.get("pull_pct"); pitch=r.get("pitch_edge"); dmg=r.get("dmg"); hrpa=r.get("hr_pa"); hpi=r.get("hpi"); sweet=r.get("sweet_spot_pct")
    if bool(r.get("weak_slot_tag")) or bool(r.get("laser")) or bool(r.get("rakes")):
        return "Adjacent / Weak-Slot Transfer"
    if (dmg or 0)>=1.8 and (hrpa or 0)>=4:
        return "Chaos / WHO Finisher"
    if (pitch or -99)>=8:
        return "Pitch-Type Punisher"
    if (pull or 0)>=38 and (sweet or 0)>=28:
        return "Pull-Air Finisher"
    if (hpi or 0)>=35 and (hrpa or 0)>=3:
        return "Elite Converter"
    if bool(r.get("platoon")):
        return "Platoon Leveraged Bat"
    return "Primary HR Owner"

def pitcher_weakness_profile(gdf):
    weak=[]
    if "weak_slots" in gdf:
        for x in gdf["weak_slots"].dropna().astype(str):
            weak += re.findall(r"\d+", x)
    return {
        "weak_slots": sorted(set(weak)),
        "avg_pitch_edge": gdf["pitch_edge"].dropna().mean() if "pitch_edge" in gdf else None,
        "avg_dmg": gdf["dmg"].dropna().mean() if "dmg" in gdf else None,
        "avg_hrpa": gdf["hr_pa"].dropna().mean() if "hr_pa" in gdf else None,
    }

def gate_result(r, profile):
    passes=[]; fails=[]
    def add(ok, name, why):
        (passes if ok else fails).append(f"{name}: {why}")
    add(is_player_name(r.get("player","")), "1", "real hitter identity")
    add(pd.isna(r.get("pull_pct")) or r.get("pull_pct")>=20, "2", "pull-air profile")
    add((r.get("dmg") or 0)>=.5 or bool(r.get("hr_alert")) or pd.isna(r.get("dmg")), "3", "damage conversion")
    add(pd.isna(r.get("pitch_edge")) or r.get("pitch_edge")>=0, "4", "pitch edge not negative")
    slot_ok = bool(r.get("weak_slot_tag")) or pd.isna(r.get("lineup_slot")) or str(int(r.get("lineup_slot"))) in profile.get("weak_slots",[])
    add(slot_ok, "5", "weak slot/lane acceptable")
    add((r.get("hr_pa") or 0)>=2 or bool(r.get("hr_alert")) or pd.isna(r.get("hr_pa")), "6", "HR conversion")
    add(pd.isna(r.get("sweet_spot_pct")) or r.get("sweet_spot_pct")>=23, "7", "launch/sweet spot")
    add(True, "8", "condition context checked")
    add(pd.isna(r.get("lineup_slot")) or r.get("lineup_slot")<=7, "9", "lineup opportunity")
    add(True, "10", "book/public decoy checked")
    add(True, "10.5", "adjacent transfer checked")
    add(True, "11", "mistake lane checked")
    add(True, "12", "bullpen continuation checked")
    add(True, "13", "numerology tie-break only")
    add(map_archetype(r)!="", "14", "archetype classified")
    add(score_row(r)>=18, "15", "finisher score live")
    add(True, "16", "ownership isolation eligible")
    add(True, "17", "no revival")
    add(True, "18", "final lock eligible")
    return passes, fails

def score_row(r):
    pull=0 if pd.isna(r.get("pull_pct")) else min(100,max(0,(r["pull_pct"]-18)*2.8))
    pitch=42 if pd.isna(r.get("pitch_edge")) else min(100,max(0,55+r["pitch_edge"]*1.25))
    dmg=0 if pd.isna(r.get("dmg")) else min(100,max(0,r["dmg"]*32))
    hrpa=0 if pd.isna(r.get("hr_pa")) else min(100,max(0,r["hr_pa"]*15))
    hpi=0 if pd.isna(r.get("hpi")) else min(100,max(0,r["hpi"]*1.85))
    sweet=0 if pd.isna(r.get("sweet_spot_pct")) else min(100,max(0,(r["sweet_spot_pct"]-18)*3.4))
    barrel=0 if pd.isna(r.get("barrel_pct")) else min(100,max(0,r["barrel_pct"]*7))
    hh=0 if pd.isna(r.get("hard_hit_pct")) else min(100,max(0,(r["hard_hit_pct"]-28)*2.1))
    score=pull*.18+pitch*.14+dmg*.16+hrpa*.16+hpi*.12+sweet*.10+barrel*.06+hh*.04
    if r.get("weak_slot_tag"): score+=8
    if r.get("hr_alert"): score+=9
    if r.get("cond_up"): score+=5
    if r.get("laser"): score+=4
    if r.get("rakes"): score+=4
    if r.get("platoon"): score+=3
    if str(r.get("team","")).strip().lower() in {"","unknown team"}: score-=10
    if str(r.get("pitcher","")).strip().lower() in {"","unknown pitcher","home","hot","moderate"}: score-=10
    return round(max(0,min(100,score)),1)
def run_game(gdf):
    gdf=gdf.copy()
    profile=pitcher_weakness_profile(gdf)
    rows=[]
    for _,base_r in gdf.iterrows():
        r=base_r.copy()
        r["archetype"]=map_archetype(r)
        r["role"]=role_type(r)
        r["score"]=score_row(r)
        passes,fails=gate_result(r, profile)
        r["gate_passes"]=" | ".join(passes)
        r["gate_fails"]=" | ".join(fails[:5])
        r["survival_path"]=f"{len(passes)}/18 gates active"
        r["pitcher_profile"]=f"weak slots: {','.join(profile.get('weak_slots',[])) or 'unknown'}"
        rows.append(r)
    alive=pd.DataFrame(rows)
    if alive.empty: return alive
    filt=alive.copy()
    for mask in [
        filt.pull_pct.isna() | (filt.pull_pct>=20),
        filt.pitch_edge.isna() | (filt.pitch_edge>=0),
        filt.sweet_spot_pct.isna() | (filt.sweet_spot_pct>=23),
        filt.hr_pa.isna() | (filt.hr_pa>=2) | filt.hr_alert,
        filt.dmg.isna() | (filt.dmg>=.5) | filt.hr_alert,
        filt.hpi.isna() | (filt.hpi>=16) | filt.hr_alert,
    ]:
        if len(filt)>1 and mask.any():
            nxt=filt[mask].copy()
            if not nxt.empty: filt=nxt
    return filt.sort_values("score",ascending=False)
def run_blender(df):
    run,bad=clean_for_run(df)
    owners=[]; survivors=[]
    for game,gdf in run.groupby("game",dropna=False):
        alive=run_game(gdf)
        alive=alive[alive["player"].astype(str).apply(is_player_name)].copy()
        if not alive.empty:
    ...
    for _,r in owners.iterrows():
        ...
        if len(core)>=3: break
    core=pd.DataFrame(core) if core else pd.DataFrame()
    alt=[]; alt_used=set(used)
    for _,r in owners.iterrows():
        if r.game not in alt_used:
            alt.append(r.to_dict()); alt_used.add(r.game)
        if len(alt)>=3: break
    alt=pd.DataFrame(alt) if alt else pd.DataFrame()
    chaos_pool=owners.copy()
    chaos_pool["_chaos_score"]=chaos_pool["score"].fillna(0)*.45+chaos_pool["dmg"].fillna(0)*18+chaos_pool["hr_pa"].fillna(0)*6+chaos_pool["weak_slot_tag"].fillna(False).astype(int)*12+chaos_pool["hr_alert"].fillna(False).astype(int)*10
    chaos=[]; chaos_games=set()
    for _,r in chaos_pool.sort_values("_chaos_score",ascending=False).iterrows():
        if r.game not in chaos_games:
            chaos.append(r.drop(labels=["_chaos_score"]).to_dict()); chaos_games.add(r.game)
        if len(chaos)>=3: break
    chaos=pd.DataFrame(chaos) if chaos else pd.DataFrame()
...
def card(r):
    ...
    fire="🔥 " if p>=75 else ""
    why=str(r.get("gate_passes",""))
    why_short=" | ".join(why.split(" | ")[:4]) if why else "gate path available in table"
    st.markdown(f"""<div class="card" style="border-color:{color}99"><span class="role">{r.get('role','Primary')}</span><span class="badge">{r.get('archetype','')}</span><div class="card-name">{fire}{r.get('player','')}</div><div class="meta"><span style="color:{color};font-weight:900">{r.get('team','')}</span> vs {r.get('pitcher','')} · Slot {fmt(r.get('lineup_slot'))}</div><div class="score">{fmt(r.get('score'))}<small style="display:block;font-family:Rajdhani;color:#aaa;font-size:13px">TRUE BLEND SCORE</small></div><div class="bar"><div class="fill" style="width:{p}%"></div></div><div class="stats"><div class="stat"><b>{fmt(r.get('pull_pct'))}</b><span>Pull</span></div><div class="stat"><b>{fmt(r.get('pitch_edge'))}</b><span>Pitch</span></div><div class="stat"><b>{fmt(r.get('dmg'))}</b><span>DMG</span></div><div class="stat"><b>{fmt(r.get('hr_pa'))}</b><span>HR/PA</span></div><div class="stat"><b>{fmt(r.get('hpi'))}</b><span>HPI</span></div><div class="stat"><b>{fmt(r.get('sweet_spot_pct'))}</b><span>Sweet</span></div></div><div class="reason"><b>Path:</b> {r.get('survival_path','')}<br><b>Why:</b> {why_short}</div></div>""", unsafe_allow_html=True)
def blender_visual(names=None, status="READY"):
    ...
    st.markdown(f"""<div class="machine"><div class="blender-wrap"><div class="feed-slot">FEED DATA HERE</div><div class="chute"></div><div class="machine-overlay"></div><div class="jar"><div class="blade"></div><div class="center">{status}</div>{floats}</div><div class="blender-base"></div>{pulse}<div class="output-slot">BLADES SPIN → OWNERS LOCK → TICKETS POP OUT</div></div></div>""", unsafe_allow_html=True)
...
with tabs[0]:
    blender_visual([r.get("player","") for r in st.session_state.get("owners",pd.DataFrame()).head(6).to_dict("records")] if "owners" in st.session_state else st.session_state.get("feed_names", None), "READY" if "owners" not in st.session_state else "LOCKED")
    ...
    if up:
        ...
        st.session_state["feed_names"]=run["player"].dropna().astype(str).head(6).tolist() if not run.empty else []
        ...
        st.markdown("<div class='status'>PLAYERS FED — BLENDER READY</div>", unsafe_allow_html=True)
        if st.button("BLEND NOW"):
            if run.empty:
                ...
                # v50 final sanity: never allow status labels as ticket/card players.
                def _real_player_df(x):
                    if x is None or x.empty:
                        return x
                    return x[x["player"].astype(str).apply(is_player_name)].copy()
                owners=_real_player_df(owners)
                core=_real_player_df(core)
                alt=_real_player_df(alt)
                chaos=_real_player_df(chaos)
                survivors=_real_player_df(survivors)
...
with tabs[2]:
    ...
    if owners is None or owners.empty:
        ...
        with st.expander("All Survivors Table"):
            safe_table(survivors[[c for c in ["game","player","team","pitcher","archetype","role","score","survival_path","pitcher_profile","gate_passes","gate_fails"] if c in survivors.columns]],460)
