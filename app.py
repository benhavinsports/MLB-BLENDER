...
def looks_like_player_name(name: Any, raw_line: Any = "") -> bool:
    n = clean_text(name)
    low = n.lower()
    if not n or len(n) < 5 or len(n) > 40:
        return False
    if n.isupper():
        return False
...
def feed_many_uploads(uploaded_files) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if frames:
        ...
        agg = {c: first_real for c in STANDARD_COLUMNS if c != "notes"}
        agg["notes"] = lambda s: " | ".join(dict.fromkeys([clean_text(x) for x in s if clean_text(x)]))[:2400]
        df = df.groupby(["game_id", "player"], dropna=False, as_index=False).agg(agg)
...
def classify_archetype(row):
    ...
    pull, hh, barrel, la, slot = fnum(row,"pull_pct"), fnum(row,"hard_hit_pct"), fnum(row,"barrel_pct"), fnum(row,"launch_angle"), fnum(row,"slot")
    flags = []
    ...
    if "CHAOS" in flags or "LOWER-SLOT" in flags: archetype = "WHO / CHAOS"
    elif "ADJACENT" in flags: archetype = "ADJACENT / DECOY TRANSFER"
    elif "PITCH-KILL" in flags and ("PULL-AIR" in flags or "BARREL" in flags): archetype = "LANE MATCH FINISHER"
    elif "PULL-AIR" in flags and "HARD-HIT" in flags: archetype = "CLEAN POWER OWNER"
    elif "BARREL" in flags: archetype = "CONVERSION BAT"
...
def gate18_engine_row(row):
    ...
    score += 4; logs.append("G1 No Empty Bat: PASS")
    game_id = clean_text(row.get("game_id","")) or "Unknown Game"
    if game_id != "Unknown Game": score += 5; logs.append("G2 Game Context: PASS")
    else: score -= 4; logs.append("G2 Game Context: UNKNOWN")

    if archetype != "RECOVERED / NEEDS DATA": score += 7; logs.append(f"G3 Archetype: {archetype}")
    else: score -= 5; logs.append("G3 Archetype: data gap")

    if not pd.isna(pull):
        if pull >= 45: score += 14; logs.append(f"G4 Pull-Air: ELITE {pull}")
        elif pull >= 42: score += 11; logs.append(f"G4 Pull-Air: PASS {pull}")
        elif pull >= 35: score += 4; logs.append(f"G4 Pull-Air: BORDERLINE {pull}")
        else: score -= 12; logs.append(f"G4 Pull-Air: FAIL {pull}")
    else: score -= 7; logs.append("G4 Pull-Air: MISSING")

    if not pd.isna(hh):
        if hh >= 50: score += 14; logs.append(f"G5 Hard-Hit: ELITE {hh}")
        elif hh >= 45: score += 11; logs.append(f"G5 Hard-Hit: PASS {hh}")
        elif hh >= 38: score += 3; logs.append(f"G5 Hard-Hit: BORDERLINE {hh}")
        else: score -= 11; logs.append(f"G5 Hard-Hit: FAIL {hh}")
    else: score -= 6; logs.append("G5 Hard-Hit: MISSING")
    ...
    else: score -= 4; logs.append("G6 Barrel/Launch: MISSING")
    ...
    if pitch_pass: score += 14; logs.append("G7 Pitch-Type Kill Switch: PASS")
    else: score -= 7; logs.append("G7 Pitch-Type Kill Switch: NOT CONFIRMED")
    ...
    else: logs.append("G9 Recent HR Allowed: not confirmed")

    if not pd.isna(slot):
        if 1 <= slot <= 5: score += 9; logs.append(f"G10 Lineup Opportunity: TOP HALF {slot}")
        else: score += 4; logs.append(f"G10 Lineup Opportunity: LOWER WHO SLOT {slot}")
    else: logs.append("G10 Lineup Opportunity: missing")

    adjacent = any(w in notes for w in ["adjacent","decoy","secondary","behind","after","protection","next man"])
    if adjacent: score += 10; logs.append("G10.5 Adjacent / Decoy Transfer: TRIGGERED")
    else: logs.append("G10.5 Adjacent / Decoy Transfer: no trigger")
    ...
    else: logs.append("G11 Protection/Pitch-Around: neutral")

    if any(w in notes for w in ["bullpen", "reliever", "pen", "continues lane"]):
        score += 5; logs.append("G12 Bullpen Continuation: support")
    else: logs.append("G12 Bullpen Continuation: unknown")

    chaos = any(w in notes for w in ["chaos","who","value","green","blowout","wind","weather","bullpen"]) or (not pd.isna(slot) and slot >= 6)
    if chaos: score += 8; logs.append("G13 WHO / Chaos Flag: TRIGGERED")
    else: logs.append("G13 WHO / Chaos Flag: no trigger")

    if (not pd.isna(barrel) and barrel >= 10) or (not pd.isna(hh) and hh >= 50):
        score += 7; logs.append("G14 True HR Conversion Profile: PASS")
    else: logs.append("G14 True HR Conversion Profile: not fully confirmed")

    clean_power = (not pd.isna(pull) and pull >= 42) and (not pd.isna(hh) and hh >= 45)
    if clean_power and (launch_pass or pitch_pass): score += 8; logs.append("G15 Event Ownership Isolation: PASS")
    elif adjacent or chaos: score += 4; logs.append("G15 Event Ownership Isolation: ALT/CHAOS path")
    else: score -= 3; logs.append("G15 Event Ownership Isolation: weak")

    if not pd.isna(odds):
        if abs(odds) <= 700: score += 3; logs.append(f"G16 Market/Odds: usable {odds}")
        else: score += 1; logs.append(f"G16 Market/Odds: longshot/volatile {odds}")
    else: logs.append("G16 Market/Odds: missing")

    finisher = clean_power and (launch_pass or pitch_pass)
    if finisher: score += 10; logs.append("G17 Finisher Gate: PASS")
    elif adjacent: score += 5; logs.append("G17 Finisher Gate: adjacent shadow pass")
    elif chaos: score += 4; logs.append("G17 Finisher Gate: chaos pass")
    else: score -= 5; logs.append("G17 Finisher Gate: FAIL")

    if finisher and pitch_pass: score += 8; logs.append("G18 Final Lock Audit: CLEAN LOCK")
    elif adjacent: score += 4; logs.append("G18 Final Lock Audit: ALT LOCK")
    elif chaos: score += 3; logs.append("G18 Final Lock Audit: CHAOS LOCK")
    else: logs.append("G18 Final Lock Audit: incomplete lock")
    ...
    reasons = [x for x in ["PULL-AIR","HARD-HIT","BARREL","LAUNCH","PITCH-KILL","ADJACENT","CHAOS","LOWER-SLOT"] if x in flags]
    if not reasons: reasons = ["18-gate recovered data gap"]
...
def score_bar_html(score):
    ...
    s = max(0, min(100, s))
    return f"""<div style="height:13px;background:rgba(255,255,255,.12);border-radius:999px;overflow:hidden;margin:8px 0 4px;"><div style="height:100%;width:{s}%;background:linear-gradient(90deg,#5dff7b,#caff32,#ff8a00);border-radius:999px;"></div></div>"""
...
def machine_visual(rows, games, teams, pitchers, file_count):
    ...
    <div class="shell">
      ...
      <div>Full 18-gate Blender engine → score bar → alert locks.</div>
      <div class="machine">
        <div class="panel"><h3>DATA FEEDER</h3><p>Upload all split CSVs/PDFs/images together.</p><div class="metric"><div class="big">{file_count}</div>Files loaded</div><div class="metric"><div class="big">{rows}</div>18-gate pool rows</div><div class="feedstream"><span>CSV 1 →</span><span>CSV 2 →</span><span>CSV 3/4 →</span></div></div>
...
with tabs[0]:
    uploaded_files = st.file_uploader(
        ...
        help="Select all split CSV files together."
    )
