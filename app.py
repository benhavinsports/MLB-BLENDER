...
from engine import run_true_blender, load_locked_results, fetch_live_public_slate, attach_slate_matchup_context, update_daily_learning
...
with tabs[0]:
    ...
    if st.button("UPDATE DAILY LEARNING / RECAP MEMORY", use_container_width=True):
        with st.spinner("Pulling yesterday HR recap and updating Blender memory..."):
            meta = update_daily_learning()
        if meta.get("ok"):
            st.success(f"Daily learning updated: {meta.get('home_runs_logged',0)} HR events logged from {meta.get('games_checked',0)} games.")
        else:
            st.warning(f"Daily learning pull did not complete: {meta.get('error','unknown error')}")
...
with tabs[2]: game_board_grid_view(st.session_state.results, feed_df=st.session_state.feed_df)
