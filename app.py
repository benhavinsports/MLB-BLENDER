...
if st.session_state.get("ready_to_run", False):
    st.markdown('<div class="real-run-wrap">', unsafe_allow_html=True)
    run_clicked = st.button("🔥 RUN BLENDER", key="real_run_blender_button", use_container_width=True)
...
if run_clicked:
    ...
    if parsed_df is None:
        st.session_state["blender_error"] = "Upload a slate PDF with START before pressing RUN BLENDER."
        st.session_state["run_status"] = "Upload PDF first."
    else:
        try:
            ...
            st.session_state["run_status"] = "Blender run failed."
