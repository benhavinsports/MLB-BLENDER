...
def score_row(r):
    ...
    env=0 if pd.isna(r.get("weather_score")) else min(100,max(0,r["weather_score"]))
    bp=0 if pd.isna(r.get("bullpen_dmg")) else min(100,max(0,r["bullpen_dmg"]*25))

    # True blend score: event likelihood, not just stat stacking.
    score = (
        pull*.18 + pitch*.14 + dmg*.16 + hrpa*.16 + hpi*.12 +
        sweet*.10 + barrel*.06 + hh*.04 + env*.02 + bp*.02
    )
    ...
    # Unknown team/pitcher should not block, but it should not outrank clean mapped games.
...
def run_game(gdf):
    ...
    # Data quality boost: rows with real player + at least 3 metrics preferred.
    metric_cols=["pull_pct","pitch_edge","dmg","hr_pa","hpi","sweet_spot_pct","barrel_pct","hard_hit_pct"]
    ...
    cut(alive.sweet_spot_pct.isna() | (alive.sweet_spot_pct>=23))
    cut(alive.hr_pa.isna() | (alive.hr_pa>=2) | alive.hr_alert)
    cut(alive.dmg.isna() | (alive.dmg>=.5) | alive.hr_alert)
    cut(alive.hpi.isna() | (alive.hpi>=16) | alive.hr_alert)
    ...
    alive["score"]=alive["score"] + (alive["_metric_count"].clip(0,6) * 1.2)
    alive["score"]=alive["score"].clip(0,100).round(1)
    alive=alive.drop(columns=["_metric_count"], errors="ignore")
...
def run_blender(df):
    ...
    # Core 3 = one owner per game only. Highest true owners.
    core=[]; used_games=set()
    for _,r in owners.iterrows():
        if r.game not in used_games:
            core.append(r.to_dict()); used_games.add(r.game)
        if len(core)>=3: break
    core=pd.DataFrame(core) if core else pd.DataFrame()

    # Alt 3 = next different games, not same-game duplicates.
    alt=[]; alt_games=set(used_games)
    for _,r in owners.iterrows():
        if r.game not in alt_games:
            alt.append(r.to_dict()); alt_games.add(r.game)
        if len(alt)>=3: break
    alt=pd.DataFrame(alt) if alt else pd.DataFrame()

    # Chaos 3 = chaos/transfer/high-DMG owners from different games.
    chaos_pool=owners.copy()
    chaos_pool["_chaos_score"]=(
        chaos_pool["score"].fillna(0)*.45 +
        chaos_pool["dmg"].fillna(0)*18 +
        chaos_pool["hr_pa"].fillna(0)*6 +
        chaos_pool["weak_slot_tag"].fillna(False).astype(int)*12 +
        chaos_pool["hr_alert"].fillna(False).astype(int)*10
    )
    chaos_pool=chaos_pool.sort_values("_chaos_score",ascending=False)
    chaos=[]; chaos_games=set()
    for _,r in chaos_pool.iterrows():
