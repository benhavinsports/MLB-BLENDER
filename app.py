...
:root{--cream:#f4ead7;--muted:#aaa093;--acid:#d7ff2f;--money:#00ff73;--danger:#ff264a;--orange:#ff8a00;--line:rgba(244,234,215,.16)}
html,body,[class*="css"]{font-family:Inter,sans-serif}.stApp{background:radial-gradient(circle at 20% 8%,rgba(215,255,47,.14),transparent 26%),radial-gradient(circle at 90% 15%,rgba(255,138,0,.12),transparent 30%),linear-gradient(180deg,#050505 0%,#0c0c0c 48%,#050505 100%);color:var(--cream)}
.stApp:before{content:"";position:fixed;inset:0;pointer-events:none;background:linear-gradient(rgba(244,234,215,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(244,234,215,.035) 1px,transparent 1px);background-size:34px 34px}.block-container{max-width:1280px;padding:1rem 1rem 5rem;position:relative;z-index:1}h1,h2,h3,p,span,div{color:var(--cream)}
.top{position:sticky;top:0;z-index:999;display:flex;justify-content:space-between;align-items:center;background:rgba(5,5,5,.82);border:1px solid var(--line);border-radius:22px;padding:12px 14px;margin-bottom:14px;backdrop-filter:blur(15px)}
.brand{display:flex;gap:12px;align-items:center}.logo{width:58px;height:58px;border-radius:50%;display:grid;place-items:center;font-family:'Black Ops One';font-size:22px;color:#050505;background:radial-gradient(circle at 35% 35%,#fff,#d7ff2f 34%,#00ff73 72%,#141414 73%);box-shadow:0 0 24px rgba(215,255,47,.35),inset 0 0 8px rgba(0,0,0,.35);animation:spin 3.5s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.brand-title{font-family:'Black Ops One';font-size:24px}.brand-sub{font-family:Rajdhani;color:var(--muted);font-size:13px;font-weight:700;letter-spacing:1.3px;text-transform:uppercase}.status{font-family:Rajdhani;font-weight:700;letter-spacing:1px;font-size:13px;padding:9px 12px;border-radius:999px;background:rgba(215,255,47,.10);border:1px solid rgba(215,255,47,.28);color:#efffc0}
.hero{min-height:430px;position:relative;overflow:hidden;border-radius:34px;padding:26px;margin-bottom:16px;border:1px solid var(--line);background:linear-gradient(145deg,rgba(26,26,26,.96),rgba(5,5,5,.98));box-shadow:0 32px 90px rgba(0,0,0,.48),inset 0 0 60px rgba(255,255,255,.025)}
.hero-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:18px;align-items:stretch}.hero h1{font-family:'Black Ops One';font-size:clamp(48px,7vw,92px);line-height:.82;margin:0}.hero .hot{color:var(--acid);text-shadow:0 0 22px rgba(215,255,47,.28)}.hero p{max-width:740px;color:var(--muted);font-size:15px;font-weight:800}.chip{display:inline-block;margin:6px 6px 0 0;padding:9px 12px;border-radius:999px;background:rgba(244,234,215,.055);border:1px solid var(--line);font-family:Rajdhani;font-weight:700;letter-spacing:.8px;font-size:13px;text-transform:uppercase}
.blender-visual{min-height:330px;border-radius:30px;border:1px solid rgba(244,234,215,.13);background:radial-gradient(circle at center,rgba(215,255,47,.18),transparent 28%),linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.015));position:relative;overflow:hidden}.blender-visual:before{content:"";position:absolute;inset:50% auto auto 50%;width:240px;height:240px;margin:-120px 0 0 -120px;border-radius:50%;background:conic-gradient(from 0deg,transparent 0deg,rgba(215,255,47,.95) 35deg,transparent 75deg,rgba(0,255,115,.9) 135deg,transparent 180deg,rgba(255,138,0,.9) 245deg,transparent 300deg);animation:blade 1.05s linear infinite}@keyframes blade{to{transform:rotate(360deg)}}.blender-visual:after{content:"OWNER";position:absolute;inset:50% auto auto 50%;width:104px;height:104px;margin:-52px 0 0 -52px;border-radius:50%;background:#050505;border:2px solid rgba(244,234,215,.25);display:grid;place-items:center;font-family:Rajdhani;font-weight:700;color:var(--acid)}
.ticker{position:absolute;left:0;right:0;bottom:0;background:rgba(5,5,5,.82);border-top:1px solid var(--line);overflow:hidden;white-space:nowrap}.ticker span{display:inline-block;padding:13px 0;font-family:Rajdhani;font-weight:700;letter-spacing:1.2px;color:#efffc0;animation:ticker 18s linear infinite}@keyframes ticker{from{transform:translateX(100%)}to{transform:translateX(-100%)}}
.stTabs [data-baseweb="tab-list"]{gap:8px;background:rgba(16,16,16,.82);border:1px solid var(--line);border-radius:18px;padding:8px}.stTabs [data-baseweb="tab"]{border-radius:13px;color:var(--cream);font-family:Rajdhani;font-size:15px;font-weight:700}.stTabs [aria-selected="true"]{background:linear-gradient(90deg,var(--acid),var(--money))!important;color:#050505!important}
...
.stFileUploader section{background:rgba(244,234,215,.045)!important;border:1px dashed rgba(215,255,47,.38)!important;border-radius:22px!important}[data-testid="stMetric"]{background:rgba(16,16,16,.84);border:1px solid var(--line);border-radius:22px;padding:16px}[data-testid="stMetricLabel"]{color:var(--muted);font-family:Rajdhani;font-size:15px;font-weight:700}[data-testid="stMetricValue"]{font-family:'Black Ops One';color:var(--cream)}
.section{display:flex;justify-content:space-between;align-items:center;margin:22px 0 12px}.section h2{font-family:'Black Ops One';margin:0;font-size:34px}.tag{padding:9px 12px;border-radius:999px;border:1px solid var(--line);background:rgba(244,234,215,.055);font-family:Rajdhani;font-weight:700}
.gate-strip{display:flex;gap:8px;overflow-x:auto;padding:10px 0 14px}.gate-tile{min-width:104px;height:86px;background:linear-gradient(180deg,rgba(26,26,26,.92),rgba(10,10,10,.92));border:1px solid var(--line);border-radius:18px;display:grid;place-items:center}.gate-tile b{font-family:'Black Ops One';font-size:22px;color:var(--acid)}.gate-tile span{font-family:Rajdhani;font-weight:700;font-size:12px;color:var(--muted);text-transform:uppercase}
.card{position:relative;overflow:hidden;min-height:292px;border-radius:28px;border:1px solid var(--line);padding:18px;background:radial-gradient(circle at 85% 10%,rgba(215,255,47,.13),transparent 34%),linear-gradient(180deg,rgba(26,26,26,.96),rgba(6,6,6,.98));box-shadow:0 22px 60px rgba(0,0,0,.43)}.card:before{content:"";position:absolute;inset:0 0 auto 0;height:7px;background:linear-gradient(90deg,var(--acid),var(--money),var(--orange))}.role{display:inline-block;padding:7px 10px;border-radius:999px;border:1px solid var(--line);background:rgba(244,234,215,.06);font-family:Rajdhani;font-size:14px;font-weight:700;text-transform:uppercase}.player{font-family:'Black Ops One';font-size:31px;line-height:1.02;margin:15px 0 5px}.meta{color:var(--muted);font-family:Rajdhani;font-size:15px;font-weight:700;min-height:38px}.score{font-family:'Black Ops One';font-size:44px}.score small{display:block;font-family:Rajdhani;color:var(--muted);font-size:12px}.ring{height:13px;border-radius:999px;overflow:hidden;background:rgba(244,234,215,.09);margin-top:10px}.fill{height:100%;border-radius:999px;background:linear-gradient(90deg,var(--acid),var(--money),var(--orange))}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:16px}.stat{border:1px solid rgba(244,234,215,.10);background:rgba(244,234,215,.045);border-radius:15px;padding:9px}.stat b{display:block;font-size:16px}.stat span{display:block;font-family:Rajdhani;color:var(--muted);font-weight:700;font-size:11px;text-transform:uppercase}.badge{display:inline-block;margin:10px 5px 0 0;padding:6px 8px;border-radius:999px;background:rgba(215,255,47,.10);border:1px solid rgba(215,255,47,.24);color:#efffc0;font-family:Rajdhani;font-weight:700;font-size:12px}
.ticket{border:1px solid var(--line);border-radius:28px;background:linear-gradient(180deg,rgba(26,26,26,.97),rgba(6,6,6,.98));padding:18px;box-shadow:0 22px 60px rgba(0,0,0,.43)}.ticket h3{font-family:'Black Ops One';font-size:30px;margin:0 0 12px}.ticket-leg{display:flex;justify-content:space-between;border-bottom:1px solid rgba(244,234,215,.1);padding:12px 0;font-family:Rajdhani;font-weight:700}.ticket-leg:last-child{border-bottom:0}.lock{margin-top:14px;padding:14px;border-radius:18px;background:linear-gradient(90deg,var(--acid),var(--money));color:#050505;font-family:'Black Ops One';text-align:center}
.feed{background:rgba(16,16,16,.88);border:1px solid var(--line);border-radius:22px;padding:14px}.feed-row{padding:10px;border-bottom:1px solid rgba(244,234,215,.08);font-family:Rajdhani}.feed-row:last-child{border-bottom:0}.feed-gate{color:var(--acid);font-weight:700}.feed-cut{color:var(--danger);font-weight:700}.feed-alive{color:var(--money);font-weight:700}
div[data-testid="stExpander"]{background:rgba(16,16,16,.82)!important;border:1px solid var(--line)!important;border-radius:20px!important}div[data-testid="stDataFrame"]{border-radius:18px;overflow:hidden;border:1px solid var(--line)}
...
def card(r):
    ...
    <div class="player-card">
      <span class="role">{r.get('role','Primary')}</span>
      <div class="player-name">{r.get('player','')}</div>
      <div class="meta">{r.get('game','')} · Slot {fmt(r.get('lineup_slot'))}</div>
      <div class="score-row"><div class="score">{fmt(r.get('score'))}<small>BLEND SCORE</small></div><div class="ring"><div class="fill" style="width:{safe_pct(r.get('score'))}%"></div></div></div>
      <div class="stats">
        <div class="stat"><b>{fmt(r.get('pull_pct'))}</b><span>Pull</span></div><div class="stat"><b>{fmt(r.get('pitch_edge'))}</b><span>Pitch</span></div><div class="stat"><b>{fmt(r.get('dmg'))}</b><span>DMG</span></div>
        <div class="stat"><b>{fmt(r.get('hr_pa'))}</b><span>HR/PA</span></div><div class="stat"><b>{fmt(r.get('hpi'))}</b><span>HPI</span></div><div class="stat"><b>{fmt(r.get('sweet_spot_pct'))}</b><span>Sweet</span></div>
      </div>
      <span class="badge">LEGAL</span><span class="badge">NO REVIVAL</span>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
<div class="blender-top"><div class="brand-block"><div class="blender-logo">BH</div><div><div class="brand-title">THE BLENDER</div><div class="brand-sub">Proprietary HR Machine</div></div></div><div class="machine-status">SYSTEM ARMED</div></div>
<div class="hero"><div class="hero-grid"><div><h1>THE <span class="highlight">BLENDER</span><br/>LOCK ROOM</h1><p>This is the machine view: upload slate, engage Blender, watch legal game owners become Core / Alt through the official gate system.</p><div class="machine-chips"><span class="chip">official gates</span><span class="chip">10.5 in sequence</span><span class="chip">one owner per game</span><span class="chip">no revivals</span></div></div><div class="blender-visual"><div class="ticker"><span> MACHINE SPEAKS: DATA IN → GATES SPIN → DEAD IS DEAD → OWNER LOCKED → CORE BUILT </span></div></div></div></div>
""", unsafe_allow_html=True)
gate_strip()
...
with tabs[2]:
    if "core" in st.session_state and not st.session_state["core"].empty:
        ...
        for _,r in st.session_state["core"].iterrows():
            st.markdown(f"<div class='ticket-leg'><span>{r['player']}</span><span>{r.get('role','')}</span></div>", unsafe_allow_html=True)
...
with tabs[3]:
    if "logs" in st.session_state:
        ...
        st.markdown("<div class='kill-feed'>", unsafe_allow_html=True)
        for _,r in logs.head(80).iterrows():
