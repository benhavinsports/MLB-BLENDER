...
def read_uploaded_into_state(uploaded):
    ...
    # CRITICAL FIX:
    # Uploaded PDFs must parse locally and immediately become the active feed.
    # Do NOT fetch live public slate here. That network/context merge was freezing
    # upload state and making the app show "No feed loaded" after reading rows.
    with st.spinner("Reading uploaded feed..."):
        ...
        # Never let public context overwrite/flatten the uploaded Star Tool PDF.
        st.session_state.feed_df = df.copy()
        st.session_state.last_audit = audit
        st.session_state.slate_games = actual_game_count(df) if df is not None and not df.empty else 0
        st.session_state.attack_pools = attack_pool_count(df) if df is not None and not df.empty else 0
        st.session_state.machine_state = "FEED LOADED" if df is not None and not df.empty else "READY"
...
with tabs[0]:
    ...
    if df is not None and not df.empty:
        ...
        # UPLOAD_SYNC_PARSER_HEALTH
        metric_rows = int((df["metric_count"].fillna(0) >= 3).sum()) if "metric_count" in df.columns else 0
        st.success(f"Parser Health: {metric_rows}/{len(df)} metric-qualified rows.")
        if metric_rows == 0:
            st.warning("Parser did not recover Blender metric columns. Open Parsed Feed Preview + Feeder Audit.")
