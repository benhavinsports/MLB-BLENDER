...
from parser.csv_reader import read_csv
from parser.xlsx_reader import read_xlsx
from audit.feeder_audit import audit_feed
from engine.machine import engage_blender
from ui.styles import inject_css
from ui.shell import top_shell
from ui.blender_wheel import live_blender
from ui.cards import player_card
from exports.results_export import to_csv_bytes
...
with tabs[0]:
    if "owners" in st.session_state:
        ...
        live_blender(
            owners.head(6).player.tolist(),
            [],
            owners.iloc[0].player if not owners.empty else "OWNER",
            f"{len(st.session_state['df'])} IN → {len(owners)} OWNERS → CORE LOCKED"
        )
    else:
        live_blender()
    ...
    if uploaded:
        ...
        try:
            if name.endswith(".pdf"):
                df, raw_text = unwrap(read_pdf(data))
            elif name.endswith(".csv"):
                df, raw_text = unwrap(read_csv(data))
            else:
                df, raw_text = unwrap(read_xlsx(data))
        ...
        if df.empty:
            ...
            c4.metric("Pitchers", df.pitcher.nunique())

            ok_feed, feed_issues = audit_feed(df)
            ...
            if st.button("ENGAGE BLENDER"):
                if not ok_feed:
                    ...
                    for i, txt in enumerate(["Feeding slate","Mapping columns","Running hard gates","Checking 10.5 transfer","Locking game owners","Building slips"]):
                        ...
                        time.sleep(.12)
                    owners, core, alt, logs, survivors = engage_blender(df)
    ...
    if "core" in st.session_state and not st.session_state["core"].empty:
        ...
        if not alt.empty:
            ...
            for _, r in alt.head(3).iterrows():
                st.markdown(f"<div class='leg'><span>{r['player']}<br><small>{r.get('team','')} · {r.get('role','')}</small></span><span class='odds'>CONF {int(r.get('score',0))}%</span></div>", unsafe_allow_html=True)
            st.markdown("<div class='lock'>LOCK ALT</div></div>")
    else:
        st.info("Run a slate first.")
