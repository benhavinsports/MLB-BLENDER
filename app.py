...
def metrics(block):
    ...
    if pull is None:
        vals=[nfloat(v) for v in re.findall(r"(?<![A-Za-z])([+-]?\d+(?:\.\d+)?)%",block)]
        vals=[v for v in vals if v is not None and 5<=abs(v)<=75]
        if vals: pull=vals[0]
    sweet=grab([r"(?:LINE|Sweet|Sweet Spot|Launch)\s*[↑↓]?\s*([+-]?\d+(?:\.\d+)?)%?"])
    hrpa=grab([r"([0-9]+(?:\.\d+)?)%\s+HR/PA",r"HR/PA\s*[:=]?\s*([0-9]+(?:\.\d+)?)%?"])
    dmg=grab([r"([0-9]+(?:\.\d+)?)\s+DMG",r"DMG\s*[:=]?\s*([0-9]+(?:\.\d+)?)"])
    hpi=grab([r"HPI\s*\+?\s*(\d+)",r"ULT\s*\+?\s*(\d+)",r"ADJ\s*\+?\s*(\d+)"])
    if hpi is None:
        plus=[int(x) for x in re.findall(r"\+\s*(\d+)",block) if 10<=int(x)<=90]
    ...
    return {"lineup_slot":slot,"pull_pct":pull,"sweet_spot_pct":sweet,"hr_pa":hrpa,"dmg":dmg,"hpi":hpi,"pitch_edge":pe,"pitch_type":pt,"hr_alert":"ALERT" in block or "HR ALERT" in block,"cond_up":"COND ↑" in block or "COND UP" in block.upper(),"weak_slot_tag":"Weak Slot" in block,"laser":"Laser" in block,"rakes":"Rakes" in block,"platoon":"Platoon" in block}

def player_blocks(lines):
    out=[]; i=0
    while i<len(lines):
        line=lines[i].strip(); player=""; start=i
        ...
        if not player:
            ...
            if m and is_player_name(m.group(2)): player=m.group(2).strip(); start=i
        if not player and is_player_name(line):
            nxt=" ".join(lines[i:i+14]); prev=lines[i-1].strip() if i>0 else ""
            if re.fullmatch(r"\d+",prev) or ("HR/PA" in nxt and any(k in nxt for k in ["DMG","ALERT","HPI","★★★★★"])):
                player=line; start=i
        if player:
            out.append({"player":player,"raw_block":" ".join(lines[start:start+56])}); i+=8; continue
        i+=1
    return out

def parse_pdf(data):
    if fitz is None: return pd.DataFrame(columns=CANON),""
    doc=fitz.open(stream=data,filetype="pdf")
    rows=[]; raw=[]; team=""; pitcher=""
    for pno,page in enumerate(doc,start=1):
        txt,mode=page_text(page); raw.append(txt)
        lines=[x.strip() for x in txt.splitlines() if x.strip()]
        t,p=detect_header(lines)
        if t: team=t
        if p: pitcher=p
        for b in player_blocks(lines):
            row={c:None for c in CANON}
            row.update({"source":mode,"page":pno,"team":team,"pitcher":pitcher,"game":f"{team} vs {pitcher}" if team and pitcher else "Unknown Game","player":b["player"],"raw_block":b["raw_block"],"notes":f"page={pno}; mode={mode}"})
            row.update(metrics(b["raw_block"]))
            rows.append(row)
    return normalize_df(pd.DataFrame(rows),"pdf"),"\n".join(raw)
...
def archetype(r):
    pull=r.get("pull_pct"); pe=r.get("pitch_edge"); dmg=r.get("dmg"); hrpa=r.get("hr_pa"); hpi=r.get("hpi"); sweet=r.get("sweet_spot_pct")
    if bool(r.get("weak_slot_tag")) or bool(r.get("laser")) or bool(r.get("rakes")): return "Adjacent / Weak-Slot Transfer"
    if (dmg or 0)>=1.8 and (hrpa or 0)>=4: return "Chaos / WHO Finisher"
    if (pe or -99)>=8: return "Pitch-Type Punisher"
    if (pull or 0)>=38 and (sweet or 0)>=28: return "Pull-Air Finisher"
    if (hpi or 0)>=35 and (hrpa or 0)>=3: return "Elite Converter"
    if bool(r.get("platoon")): return "Platoon Leveraged Bat"
...
def score(r):
    ...
    pe=42 if pd.isna(r.get("pitch_edge")) else min(100,max(0,55+r["pitch_edge"]*1.25))
    ...
    s=pull*.18+pe*.14+dmg*.16+hrpa*.16+hpi*.12+sweet*.10+barrel*.06+hh*.04
    for col,pts in [("weak_slot_tag",8),("hr_alert",9),("cond_up",5),("laser",4),("rakes",4),("platoon",3)]:
        if r.get(col): s+=pts
    if str(r.get("team","")).lower() in {"","unknown team"}: s-=10
    if str(r.get("pitcher","")).lower() in {"","unknown pitcher","home","hot","moderate"}: s-=10
...
with tabs[2]:
    ...
    if owners is None or owners.empty: st.info("Run the Blender first.")
        ...
        with st.expander("All Survivors + Gates"):
            cols=[c for c in ["game","player","team","pitcher","archetype","role","score","gate_path"] if c in surv.columns]
