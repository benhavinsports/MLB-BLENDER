...
from ui import top_shell, live_blender, player_card
...
with tabs[0]:
    ...
    if uploaded:
        try:
            df,raw_text=read_file(uploaded.name, uploaded.read())
        ...
        if df.empty:
            ...
            with st.expander("Parser Debug Sample"):
                sample = "\n".join(str(raw_text).splitlines()[:160]) if raw_text else "No raw text captured."
                st.text(sample)
