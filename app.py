...
from ui import inject_css, hero, blender_visual, tickets_view, game_board_grid_view, start_upload_header
...
with tabs[0]:
    # Real START upload area: header + actual Streamlit uploader directly under it.
    start_upload_header()
    uploaded = st.file_uploader(
        "Upload slate file",
        type=["pdf", "csv", "xlsx", "xls", "txt", "md"],
        label_visibility="collapsed",
        key="start_upload_real",
    )
if uploaded is not None:
    ...
    # Public board renders after START/upload controls, not before them.
    blender_visual()
...
with tabs[2]:
    game_board_grid_view(st.session_state.results)
