...
getattr(ui, "inject_css", lambda: None)()
getattr(ui, "hero", lambda: None)()
...
with tabs[0]:
    if hasattr(ui, "start_upload_area"):
    ...
    getattr(ui, "blender_visual", lambda: st.info("UI board unavailable."))()
    ...
    with col2:
        if st.button("RUN OFFICIAL MLB SLATE BLENDER", use_container_width=True):
            run_blender()
    ...
    if df is None or df.empty:
        ...
        c[1].metric(
            "Official Games",
            int(df["game_pk"].dropna().astype(str).replace("", pd.NA).dropna().nunique()) if "game_pk" in df.columns else actual_game_count(df),
        )
...
with tabs[1]:
    getattr(ui, "tickets_view", lambda results: st.info("Tickets view unavailable."))(st.session_state.results)

with tabs[2]:
    getattr(ui, "game_board_grid_view", lambda results: st.info("Game board view unavailable."))(st.session_state.results)
