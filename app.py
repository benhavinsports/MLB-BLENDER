...
APP_VERSION = "v144_TICKET_TIMEFRAME_SGP"
...
def run(df, label):
    ...
    if "meta" in st.session_state.results:
        ...
        if not st.session_state.results["meta"].get("message"):
...
with tabs[0]:
    ...
    c1,c2 = st.columns(2)
    with c1:
        if st.button("LOAD OFFICIAL MLB SLATE", key="live_slate_v109", use_container_width=True):
            ...
            st.info(str(meta))
    with c2:
        if st.button("RUN OFFICIAL MLB SLATE BLENDER", key="live_blender_v109", use_container_width=True):
    ...
    if df is not None and not df.empty:
        ...
        # ENGINE_ONLY_PARSER_HEALTH
        metric_rows = int((df["metric_count"].fillna(0) >= 3).sum()) if "metric_count" in df.columns else 0
        game_ct = int(df["game_key"].dropna().astype(str).str.strip().replace("", pd.NA).dropna().nunique()) if "game_key" in df.columns else 0
        # FINAL_COUNTER_TICKET_FIX_HEALTH
        st.success(f"Parser Health: {metric_rows}/{len(df)} metric rows · {game_ct} true games.")
        # OFFICIAL_MLB_SLATE_STATUS_PANEL
        if "official_slate_attached" in df.columns:
            official_matches = int(df["official_slate_attached"].fillna(False).sum())
            official_games = int(df["game_pk"].dropna().astype(str).replace("", pd.NA).dropna().nunique()) if "game_pk" in df.columns else 0
            st.success(f"Official MLB Slate: {official_games} games attached · {official_matches}/{len(df)} rows matched.")
