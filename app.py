...
from parser_star import parse_pdf, parse_csv, parse_xlsx, feeder_audit
try:
    from parser_star import parse_pdf_with_text
except Exception:
    parse_pdf_with_text = None
...
def unwrap_parse_result(result):
...
with tabs[0]:
    if "owners" in st.session_state:
        ...
        live_blender(owner="READY", status="UPLOAD SLATE → FEEDER LOCK → RUN")
    ...
    if uploaded:
        ...
        try:
            if name.endswith(".pdf"):
                df, raw_text = unwrap_parse_result(parse_pdf_with_text(data) if parse_pdf_with_text else parse_pdf(data))
            elif name.endswith(".csv"):
                df, raw_text = unwrap_parse_result(parse_csv(data))
            else:
                df, raw_text = unwrap_parse_result(parse_xlsx(data))
