...
def inject_css():
    ...
    .stApp {
        background:
          radial-gradient(circle at 0% 0%, rgba(150,255,35,.24) 0%, rgba(13,18,0,.28) 18%, rgba(0,0,0,1) 45%),
          #000;
        color:#f7eedf;
    }
    header, [data-testid="stHeader"] { background: rgba(0,0,0,0); }
    h1,h2,h3 { font-weight:1000!important; letter-spacing:1px; color:#f7eedf!important; }
    .hero {
        border:1px solid #2f2f2f;
        border-radius:38px;
        padding:32px 34px;
        background:linear-gradient(145deg,#0b0b0b,#020202);
        margin-bottom:22px;
        box-shadow: inset 0 0 60px rgba(180,255,40,.05);
    }
    .hero-title {
        font-size:64px;
        line-height:.88;
    ...
    .lime { color:#b8ff22; }
    .feed-card {
        border:2px solid #b8ff22;
        border-radius:34px;
        padding:26px;
        background: radial-gradient(circle at top left, rgba(150,255,35,.18), rgba(6,6,6,1) 50%);
        margin:16px 0 22px 0;
        box-shadow:0 0 28px rgba(184,255,34,.18);
    }
    .feed-inner {
        border-radius:26px;
        padding:22px;
    ...
    div.stButton > button {
        background:linear-gradient(90deg,#a8ff2d,#00f59b,#ffa10a);
        color:#000;
        border:none;
        border-radius:32px;
        min-height:66px;
    ...
    .owner-card {
        ...
        background:linear-gradient(135deg,var(--c1),#050505 70%);
        box-shadow:0 0 24px rgba(184,255,34,.12);
    }
    .owner-name {
    ...
    .owner-meta { color:#e8e8e8; font-weight:800; font-size:13px; opacity:.95; }
    .badge {
    ...
    .elite { background:rgba(184,255,34,.16); color:#dfff8a; border-color:rgba(184,255,34,.55); }
    .recovery { background:rgba(255,161,10,.14); color:#ffd39a; border-color:rgba(255,161,10,.5); }
    .who { background:rgba(255,70,90,.16); color:#ffb0b8; border-color:rgba(255,70,90,.45); }
    .meter {height:10px;border-radius:999px;background:#1a1a1a;overflow:hidden;border:1px solid #303030;margin-top:8px;}
    .meter > span {display:block;height:100%;background:linear-gradient(90deg,#ff5a6a,#ffd166,#b8ff22);}
</style>
...
TEAM_COLORS = {
    ...
    "NYY": ("#0C2340", "#C4CED3"), "ATH": ("#003831", "#EFB21E"), "PHI": ("#E81828", "#002D72"),
    "PIT": ("#FDB827", "#27251F"), "SD": ("#2F241D", "#FFC425"), "SF": ("#FD5A1E", "#27251F"),
    "SEA": ("#0C2C56", "#005C5C"), "STL": ("#C41E3A", "#0C2340"), "TB": ("#092C5C", "#8FBCE6"),
    "TEX": ("#003278", "#C0111F"), "TOR": ("#134A8E", "#E8291C"), "WSH": ("#AB0003", "#14225A")
}

def team_colors(team):
    return TEAM_COLORS.get(str(team).upper().strip(), ("#151515", "#b8ff22"))

def owner_card(row, label="LOCKED OWNER"):
    c1, c2 = team_colors(row.get("team", ""))
    role = str(row.get("official_core_role", "") or "")
    cls = "elite" if role.lower() == "primary" else ("who" if "who" in role.lower() else "recovery" if "recovery" in role.lower() else "elite")
    try:
    ...
    st.markdown(f"""
    <div class="owner-card" style="--c1:{c1};--c2:{c2};">
      <div class="owner-meta">{label} · {row.get("game_key", row.get("game", ""))}</div>
      <div class="owner-name">{row.get("player", "")}</div>
      <div class="owner-meta">{row.get("team", "")} vs {row.get("pitcher", "")}</div>
      <span class="badge {cls}">{role}</span>
      <span class="badge elite">{row.get("archetype", "")}</span>
      <span class="badge recovery">{row.get("data_status", "")}</span>
      <div class="meter"><span style="width:{width}%"></span></div>
      <div class="owner-meta">BLENDER SCORE: {score}</div>
    </div>
    """, unsafe_allow_html=True)
...
with tabs[0]:
    ...
    <div class="feed-card">
      <div class="feed-inner">
        <h2>FEED DATA HERE</h2>
        <p>Upload your slate file, then hit the real Blender run button below.</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    owners_df = res.get("owners", pd.DataFrame()) if isinstance(res, dict) else pd.DataFrame()
    if not owners_df.empty:
        st.subheader("LOCKED GAME OWNERS")
        for _, rr in owners_df.head(12).iterrows():
            owner_card(rr, "LOCKED OWNER")
    ...
    with st.expander("Health Check / Built-In Sample Test"):
        st.caption("Use this only to confirm the app engine runs after deployment.")
        if st.button("RUN BUILT-IN SAMPLE TEST", key="sample_test_restored"):
            sample = pd.DataFrame([
