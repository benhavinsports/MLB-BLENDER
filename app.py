...
from engine import run_true_blender, run_recap_check, csv_bytes, load_locked_results, fetch_live_public_slate, merge_public_context
...
with tabs[0]:
    ...
    if st.button("LOAD LIVE PUBLIC SLATE", key="load_live_public_slate_v72", use_container_width=True):
        ctx, meta = fetch_live_public_slate()
        st.session_state.public_context = ctx
        st.info(f"Live public slate loaded: {meta}")
