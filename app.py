...
def parse_pdf(data):
    ...
    rows=[]; raw=[]; team=""; pitcher=""; weak_slots_by_pitcher={}

    for pno,page in enumerate(doc,start=1):
        txt,mode=page_text(page); raw.append(txt)
        lines=[x.strip() for x in txt.splitlines() if x.strip()]

        # Capture weak slot footer lines before parsing cards.
        for ln in lines:
        ...
        # Detect all headers on page, parse by section spans.
        headers=[]
        for idx,ln in enumerate(lines):
            hm=re.search(r"([A-Z][A-Z .'\-]{2,}?)\s+PROJECTED\s+v(?:s|s\.)\.?\s+([A-Z][A-Za-zÀ-ÿ .'\-]+?)(?:\s+~|\s+\||\s+\d|\s*$)",ln,re.I)
            if hm:
        ...
        if not headers:
            ...
            if t: team=t
            if p: pitcher=p
            sections=[(0,len(lines),team,pitcher)]
        else:
            sections=[]
            if team and pitcher and headers[0][0] > 0:
                sections.append((0,headers[0][0],team,pitcher))
            for h_i,(idx,tm,pit) in enumerate(headers):
                end=headers[h_i+1][0] if h_i+1<len(headers) else len(lines)
                sections.append((idx,end,tm,pit))
                team,pitcher=tm,pit

        for start,end,tm,pit in sections:
            subset=lines[start:end]
            for b in player_blocks(subset):
                row={c:None for c in CANON}
                weak=weak_slots_by_pitcher.get(pit,"")
                row.update({"source":mode,"page":pno,"team":tm,"pitcher":pit,"weak_slots":weak,
                            "game":f"{tm} vs {pit}" if tm and pit else "Unknown Game",
                            "player":b["player"],"raw_block":b["raw_block"],
                            "notes":f"page={pno}; mode={mode}"})
