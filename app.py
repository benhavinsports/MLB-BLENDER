...
def unwrap(x):
    if isinstance(x,tuple): return x[0], x[1] if len(x)>1 else ""
    return x,""
...
with tabs[0]:
    ...
    if uploaded:
        ...
        try:
            if name.endswith(".pdf"): df,raw_text=unwrap(parse_pdf(data))
            elif name.endswith(".csv"): df,raw_text=unwrap(parse_csv(data))
            else: df,raw_text=unwrap(parse_xlsx(data))
        ...
        if df.empty:
            ...
            c1.metric("Players",len(df)); c2.metric("Games",df.game.nunique()); c3.metric("Teams",df.team.nunique()); c4.metric("Pitchers",df.pitcher.nunique())
            ok,issues=feeder_audit(df)
