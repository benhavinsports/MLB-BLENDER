...
.elite-card{animation:elitePulse 1.15s infinite;border-color:#d9ff2f!important;box-shadow:0 0 28px rgba(217,255,47,.35), inset 0 0 18px rgba(217,255,47,.08)}
.fire-tag{float:right;font-family:'Black Ops One';font-size:20px;color:#ff9900;animation:elitePulse 1.15s infinite}
@keyframes elitePulse{50%{filter:brightness(1.28);box-shadow:0 0 42px rgba(217,255,47,.55)}}
...
def is_player_name(x):
    ...
    # hard reject machine/archetype/descriptor phrases that kept leaking into cards
    reject_phrases={"DISENGAGED","ENGAGED","EATS LHP","EATS RHP","PLATOON","FRESH","MODERATE","ELEVATED","HOT","COLD","WARM","HOME","AWAY","TODAY'S GAMES","OWNER","OWNERS","LOCKED","READY"}
    if u in reject_phrases: return False
    parts=s.split()
    if not (2 <= len(parts) <= 4):
        # one-word real MLB names are rare in this feed; one-word labels are common, so block them from picks
        return False
    if any(p.upper() in BAD for p in parts): return False
    if any(len(p)<=1 for p in parts): return False
    if any(p.upper() in {"LHP","RHP","L","R","VS"} for p in parts): return False
...
def strip_player_candidate(cand):
    """Return only the actual hitter name from a Star Tool card line."""
    s=str(cand).strip()
    s=re.sub(r"\s+"," ",s)
    # remove leading ranking if present
    s=re.sub(r"^\d+\s+", "", s)
    # stop at known non-name tags/descriptors
    tag_re=r"\b(?:L|R|⇄|Platoon|Weak|Slot|Laser|Rakes|Eats|RHP|LHP|COND|HR|ALERT|HOT|WARM|COLD|Fresh|Moderate|Elevated|Home|Away|\d+d|\d+d\+|↑|↓)\b"
    m=re.search(tag_re, s, re.I)
    if m:
        s=s[:m.start()].strip()
    # keep max first 3/4 title-case name tokens
    toks=[]
    for p in s.split():
        if re.match(r"^[A-ZÁÉÍÓÚÑ][A-Za-zÀ-ÿ.'’\-]+$", p): toks.append(p)
        else: break
    # Don't allow descriptor as name
    for n in range(min(4,len(toks)),1,-1):
        name=" ".join(toks[:n])
        if is_player_name(name): return name
    return ""

def looks_like_card_context(lines, idx):
    window=" ".join(lines[max(0,idx-2):min(len(lines),idx+18)])
    return any(k in window for k in ["★★★★★","DMG","HR/PA","ULT","HPI","ALERT","COND","LINE"])

def player_blocks(lines):
    """Stage 3/4: identify ONLY true hitter cards. Descriptors never become players."""
    out=[]
    i=0
    n=len(lines)
    while i<n:
        raw=str(lines[i]).strip()
        player=""; start=i

        # Star Tool pattern A: rank on its own line, player line next.
        if re.fullmatch(r"\d+", raw) and i+1<n:
            cand=strip_player_candidate(lines[i+1])
            if cand and looks_like_card_context(lines, i+1):
                player=cand; start=i+1

        # Pattern B: rank and player on one line.
        if not player:
            cand=strip_player_candidate(raw)
            if cand and re.match(r"^\d+\s+", raw) and looks_like_card_context(lines, i):
                player=cand; start=i

        # Pattern C: player line with previous rank and following stats.
        if not player:
            cand=strip_player_candidate(raw)
            prev=lines[i-1].strip() if i>0 else ""
            if cand and re.fullmatch(r"\d+", prev) and looks_like_card_context(lines, i):
                player=cand; start=i

        if player:
            end_idx=min(n,start+70)
            for j in range(start+3,min(n,start+74)):
                # stop at new team/pitcher header
                if "PROJECTED" in lines[j].upper():
                    end_idx=j; break
                # stop at next rank + actual player line
                if re.fullmatch(r"\d+", lines[j].strip()) and j+1<n:
                    nxt=strip_player_candidate(lines[j+1])
                    if nxt and looks_like_card_context(lines,j+1):
                        end_idx=j; break
                # stop at same-line next rank/player
                if re.match(r"^\d+\s+", lines[j].strip()):
                    nxt=strip_player_candidate(lines[j])
                    if nxt and looks_like_card_context(lines,j):
                        end_idx=j; break
            block=" ".join(lines[start:end_idx])
            # card must include at least two real Star Tool signals, not just random text
            sig=sum(k in block for k in ["★★★★★","DMG","HR/PA","ULT","HPI","COND","LINE","ALERT"])
            if sig>=2 and is_player_name(player):
...
def find_page_sections(lines, carry_team="", carry_pitcher=""):
    """
    Section-bound scanner.
    A hitter only inherits the team/pitcher from the section span he appears inside.
    """
    headers=[]
    n=len(lines)
    ...
    for idx, ln in enumerate(lines):
        ...
        # Same-line: TEAM PROJECTED vs. Pitcher
        ...
        # Split header: team above / pitcher after PROJECTED vs.
    ...
    # De-dupe nearby duplicated header reads.
    cleaned=[]
    ...
    for pno,page in enumerate(doc,start=1):
        ...
        for s,e,tm,pit in spans:
            ...
            for b in player_blocks(subset):
                ...
                row.update(mx)
                if (row.get("lineup_slot") is None or pd.isna(row.get("lineup_slot"))) and row.get("raw_block"):
                    sm=re.search(r"\b(1|2|3|4|5|6|7|8|9)(?:st|nd|rd|th)\b", str(row.get("raw_block")))
                    if sm:
                        row["lineup_slot"]=float(sm.group(1))
                rows.append(row)
...
def sanitize_players_df(df):
    if df is None or df.empty:
        return df
    out=df.copy()
    out=out[out["player"].astype(str).apply(is_player_name)].copy()
    bad_words={"DISENGAGED","ENGAGED","READY","LOCKED","COMPLETE","OWNER","OWNERS","OUTPUT","INPUT","STATUS"}
    out=out[~out["player"].astype(str).str.upper().isin(bad_words)].copy()
    return out
...
def run_blender(df):
    ...
    if run is None or run.empty:
        return pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),bad
    # final pre-engine sanitization: no status/descriptor rows may enter gate engine
    run=sanitize_players_df(run)
    if run is None or run.empty:
        return pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),bad

    for game,gdf in run.groupby("game",dropna=False):
        alive=run_game(gdf)
        alive=sanitize_players_df(alive)
    ...
    surv=pd.concat(survivors,ignore_index=True) if survivors else pd.DataFrame()
    if owners.empty:
        return owners,pd.DataFrame(),pd.DataFrame(),pd.DataFrame(),surv,bad
    owners=sanitize_players_df(owners).sort_values("score",ascending=False).reset_index(drop=True)

    def pick_unique(pool,n=3,used_games=None,used_players=None,prefer=None,allow_used_games=False):
        used_games=set() if used_games is None else set(used_games)
        used_players=set() if used_players is None else set(used_players)
        out=[]; p=pool.copy()
        if prefer is not None and not p.empty:
            pref=p[p.apply(prefer,axis=1)].copy(); rest=p[~p.index.isin(pref.index)].copy(); p=pd.concat([pref,rest],ignore_index=True)
        for _,r in p.iterrows():
            game=str(r.get("game","")); player=str(r.get("player",""))
            if player in used_players: continue
            if (not allow_used_games) and game in used_games: continue
            out.append(r.to_dict()); used_players.add(player); used_games.add(game)
            if len(out)>=n: break
        return pd.DataFrame(out),used_games,used_players

    core,core_games,core_players=pick_unique(owners,3,prefer=lambda r: str(r.get("role",""))=="Primary" or any(x in str(r.get("archetype","")) for x in ["Elite","Pitch-Type","Pull-Air","Weak-Slot"]))

    # ALT: use next-best survivors first, excluding exact core players. Different games if possible.
    alt_pool=surv[~surv["player"].astype(str).isin(core_players)].copy() if not surv.empty else owners.copy()
    alt_pool=alt_pool.sort_values("score",ascending=False) if not alt_pool.empty else alt_pool
    alt,alt_games,alt_players=pick_unique(alt_pool,3,used_games=core_games,used_players=core_players,prefer=lambda r: True)
    if len(alt)<3 and not alt_pool.empty:
        extra,alt_games,alt_players=pick_unique(alt_pool,3-len(alt),used_players=core_players.union(alt_players),allow_used_games=True)
        if not extra.empty: alt=pd.concat([alt,extra],ignore_index=True)

    # CHAOS: only chaos/WHO/transfer/weak-slot/alert style candidates, not just copied core.
    used_players=set(core["player"].astype(str).tolist() if not core.empty else []) | set(alt["player"].astype(str).tolist() if not alt.empty else [])
    chaos_pool=surv.copy() if not surv.empty else owners.copy()
    chaos_pool=sanitize_players_df(chaos_pool)
    if chaos_pool is None or chaos_pool.empty:
        chaos=pd.DataFrame()
    else:
        chaos_pool["_chaos"]=(chaos_pool["score"].fillna(0)*.35 + chaos_pool["dmg"].fillna(0)*20 + chaos_pool["hr_pa"].fillna(0)*7 + chaos_pool["weak_slot_tag"].fillna(False).astype(int)*18 + chaos_pool["hr_alert"].fillna(False).astype(int)*14)
        chaos_pool=chaos_pool.sort_values("_chaos",ascending=False).drop(columns=["_chaos"])
        chaos,_,chaos_players=pick_unique(chaos_pool,3,used_players=used_players,prefer=lambda r: any(x in str(r.get("archetype","")) for x in ["Chaos","Weak-Slot","Transfer","Alert"]) or bool(r.get("hr_alert")))
        if len(chaos)<3:
            fill,_,_=pick_unique(chaos_pool,3-len(chaos),used_players=used_players.union(set(chaos["player"].astype(str).tolist() if not chaos.empty else [])),allow_used_games=True)
            if not fill.empty: chaos=pd.concat([chaos,fill],ignore_index=True)
...
def card(r):
    ...
    elite = p >= 75
    fire = "🔥 FIRE" if elite else ""
    elite_class = " elite-card" if elite else ""
    path=str(r.get("gate_path",""))
    short_path=" | ".join(path.split(" | ")[:6])
    st.markdown(f"""<div class="card{elite_class}" style="border-color:{color}99"><span class="role">{r.get('role','')}</span><span class="badge">{r.get('archetype','')}</span><span class="fire-tag">{fire}</span><div class="card-name">{'🔥 ' if elite else ''}{r.get('player','')}</div><div class="meta"><span style="color:{color};font-weight:900">{r.get('team','')}</span> vs {r.get('pitcher','')} · Slot {fmt(r.get('lineup_slot'))}</div><div class="score">{fmt(r.get('score'))}<small style="display:block;font-family:Rajdhani;color:#aaa;font-size:13px">TRUE BLEND SCORE</small></div><div class="bar"><div class="fill" style="width:{p}%"></div></div><div class="stats"><div class="stat"><b>{fmt(r.get('pull_pct'))}</b><span>Pull</span></div><div class="stat"><b>{fmt(r.get('pitch_edge'))}</b><span>Pitch</span></div><div class="stat"><b>{fmt(r.get('dmg'))}</b><span>DMG</span></div><div class="stat"><b>{fmt(r.get('hr_pa'))}</b><span>HR/PA</span></div><div class="stat"><b>{fmt(r.get('hpi'))}</b><span>HPI</span></div><div class="stat"><b>{fmt(r.get('sweet_spot_pct'))}</b><span>Sweet</span></div></div><div class="reason"><b>Blend path:</b> {short_path}</div></div>""",unsafe_allow_html=True)
...
with tabs[0]:
    if "owners" in st.session_state and st.session_state.get("owners") is not None and not st.session_state.get("owners").empty:
        names=st.session_state.get("owners")["player"].dropna().astype(str).head(6).tolist()
        machine_status="LOCKED"
    else:
        names=st.session_state.get("feed_names",None)
        machine_status="READY"
    blender_visual(names,machine_status)
    up=st.file_uploader("FEED PLAYERS",type=["pdf","csv","xlsx"],label_visibility="collapsed")
    if up:
        if st.session_state.get("_last_upload_name") != up.name:
            for _k in ["owners","core","alt","chaos","survivors"]:
        ...
        if st.button("BLEND NOW"):
            if run.empty:
                ...
                st.session_state["feed_names"]=owners["player"].dropna().astype(str).head(6).tolist() if owners is not None and not owners.empty else st.session_state.get("feed_names", [])
                st.success("TRUE BLEND COMPLETE")
                st.rerun()
                if owners is not None and not owners.empty:
                    blender_visual(owners["player"].dropna().astype(str).head(6).tolist(), "LOCKED")

with tabs[1]:
    if "owners" in st.session_state and st.session_state.get("owners") is not None and not st.session_state.get("owners").empty:
        st.session_state["feed_names"]=st.session_state.get("owners")["player"].dropna().astype(str).head(6).tolist()
    ...
    for label,key in [("CORE 3","core"),("ALT 3","alt"),("CHAOS 3","chaos")]:
        ...
        if df is None or df.empty: st.info("Run the Blender first.")
            ...
            for _,r in df.iterrows():
                ...
                st.markdown(f"<div class='leg'><span><b>{fire}{r.get('player','')}</b><br><small>{r.get('archetype','')} · {r.get('team','')} vs {r.get('pitcher','')}</small></span><span class='odds'>{int(r.get('score',0))}%</span></div>",unsafe_allow_html=True)
            st.download_button(f"Download {label}",csv_bytes(df),f"{label.lower().replace(' ','_')}.csv","text/csv")
        st.markdown("</div>",unsafe_allow_html=True)

with tabs[2]:
    st.markdown("<div class='section-title' style='font-size:34px'>GAME BOARD — LOCKED OWNERS BY GAME</div>",unsafe_allow_html=True)
