...
def is_player_name(x):
    ...
    if u in {"DISENGAGED","ENGAGED","READY","LOCKED","COMPLETE","OWNER","OWNERS","OUTPUT","INPUT","STATUS"}: return False
    parts=s.split()
            ...
            for j in range(start+4, min(len(lines), start+70)):
                if re.fullmatch(r"\d+", lines[j].strip()) and j+1<len(lines):
                    ...
                    if m and is_player_name(m.group(1)):
                        end_idx=j
                        break
                if " PROJECTED vs." in lines[j] or " PROJECTED VS." in lines[j].upper() or "PROJECTED" in lines[j].upper():
                    end_idx=j
                    break
...
def run_blender(df):
    ...
    def pick_unique(pool, n=3, used_games=None, used_players=None, prefer=None):
        used_games=set() if used_games is None else set(used_games)
        used_players=set() if used_players is None else set(used_players)
        out=[]
        p=pool.copy()
        if prefer:
            pref=p[p.apply(prefer,axis=1)].copy()
            rest=p[~p.index.isin(pref.index)].copy()
            p=pd.concat([pref,rest],ignore_index=True)
    ...
    def core_pref(r):
        a=str(r.get("archetype",""))
        role=str(r.get("role",""))
        return ("Elite" in a or "Pull-Air" in a or "Pitch-Type" in a or role=="Primary")

    core, core_games, core_players = pick_unique(owners, 3, prefer=core_pref)

    # ALT = next clean survivors from games not used by Core if possible.
    alt_pool=owners.copy()
    alt, alt_games, alt_players = pick_unique(alt_pool, 3, used_games=core_games, used_players=core_players,
                                             prefer=lambda r: "Transfer" in str(r.get("archetype","")) or str(r.get("role","")) in {"Transfer","Primary"})

    # if slate has fewer separate games, allow different players but never exact same player.
    if len(alt)<3 and not surv.empty:
        backup=surv[~surv["player"].astype(str).isin(core_players)].copy()
        backup=backup.sort_values("score",ascending=False)
        extra, alt_games, alt_players = pick_unique(backup, 3-len(alt), used_players=core_players.union(alt_players),
                                                    prefer=lambda r: True)
        if not extra.empty:
            alt=pd.concat([alt,extra],ignore_index=True)

    # CHAOS = chaos-specific. Must not simply repeat Core/Alt unless no other chaos options exist.
    ...
    if chaos_pool is None or chaos_pool.empty:
        ...
        chaos_pool["_chaos_score"]=(
            chaos_pool["score"].fillna(0)*.38
            + chaos_pool["dmg"].fillna(0)*18
            + chaos_pool["hr_pa"].fillna(0)*6
            + chaos_pool["weak_slot_tag"].fillna(False).astype(int)*14
            + chaos_pool["hr_alert"].fillna(False).astype(int)*12
        )
        chaos_pool=chaos_pool.sort_values("_chaos_score",ascending=False).drop(columns=["_chaos_score"])
        used_all=set(core["player"].astype(str).tolist() if not core.empty else []) | set(alt["player"].astype(str).tolist() if not alt.empty else [])
        chaos, _, _ = pick_unique(
            chaos_pool, 3, used_players=used_all,
            prefer=lambda r: ("Chaos" in str(r.get("archetype","")) or "Weak-Slot" in str(r.get("archetype","")) or "Transfer" in str(r.get("archetype","")) or bool(r.get("hr_alert")))
        )
        if len(chaos)<3:
            fill, _, _ = pick_unique(chaos_pool, 3-len(chaos), used_players=set(chaos["player"].astype(str).tolist() if not chaos.empty else []),
                                     prefer=lambda r: True)
            if not fill.empty:
                chaos=pd.concat([chaos,fill],ignore_index=True)
