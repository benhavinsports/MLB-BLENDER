...
APP_VERSION = "v134_RUN_BUTTON_PARSE_UPLOAD_FIX"


# -------------------- v134 RUN BUTTON PARSE UPLOAD FIX --------------------
def ensure_feed_loaded_from_upload(uploaded):
    """Parse selected upload into feed_df right before run if needed."""
    try:
        empty = (
            "feed_df" not in st.session_state
            or st.session_state.feed_df is None
            or getattr(st.session_state.feed_df, "empty", True)
        )
        if empty and uploaded is not None:
            ok = read_uploaded_into_state(uploaded)
            return bool(ok)
        return not empty
    except Exception as e:
        st.error(f"Upload parse failed before run: {e}")
        return False
...
with tabs[0]:
    ...
    if st.button("RUN BLENDER NOW", key="run_v116", use_container_width=True):
        try:
            ...
            ensure_feed_loaded_from_upload(uploaded)
