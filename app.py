...
def attach_official(df):
    ...
    if st.session_state.official_slate is None or st.session_state.official_slate.empty:
        try:
            slate, meta = fetch_live_public_slate()
            st.session_state.official_slate = slate
            st.session_state.official_meta = meta
        except Exception as e:
            st.session_state.last_audit["official_slate_error"] = str(e)
            return df
    return attach_slate_matchup_context(df, st.session_state.official_slate)
...
def read_uploaded(uploaded):
    ...
    try:
        ...
        with st.spinner("Reading uploaded feed..."):
            ...
            st.session_state.results = load_locked_results()  # old-version locks are ignored; prevents stale fake results
        ...
        if st.button("RUN OFFICIAL MLB SLATE BLENDER", use_container_width=True):
            try:
                if st.session_state.official_slate is None or st.session_state.official_slate.empty:
                    slate, meta = fetch_live_public_slate()
                    st.session_state.official_slate = slate; st.session_state.official_meta = meta
                st.session_state.feed_df = st.session_state.official_slate.copy()
                run_blender()
            except Exception:
                st.session_state.last_error = traceback.format_exc(); st.error("Official MLB slate run failed.")
