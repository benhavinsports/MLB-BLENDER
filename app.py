...
BAD_PLAYER_TOKENS = set("VS PROJECTED PITCHER TEAM LINEUP SLOT BATS HAND ALERT DMG HPI PULL SWEET STAR TOOL DATA PAGE HOME AWAY NONE SUMMARY DETAILS WEAK COND LINE FRESH MODERATE ELEVATED HOT COLD WARM PLATOON LASER RAKES TODAY TODAY\'S GAMES".split())
ALIASES = {
...
def blender_visual(names=None, status="READY"):
    ...
    st.markdown(f"""<div class="machine"><div class="blender-wrap"><div class="feed-slot">FEED PLAYERS BELOW</div><div class="chute"></div><div class="machine-overlay"></div><div class="jar"><div class="blade"></div><div class="center">{status}</div>{floats}</div><div class="blender-base"></div>{pulse}<div class="output-slot">CLICK THE BLENDER → CORE / ALT / CHAOS POP OUT</div></div></div>""", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if up:
        ...
        st.markdown("<div class='status'>PLAYERS FED INTO THE BLENDER</div>", unsafe_allow_html=True)
