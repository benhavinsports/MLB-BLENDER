...
def safe_meter(x):
    try:
        if x is None or pd.isna(x):
            return 40
        return max(8, min(100, int(float(x))))
    except Exception:
        return 40
...
def card(r):
    st.markdown(f"""<div class="card"><span class="role">{safe_role(r.get('role'))}</span><div class="player">{r.get('player','')}</div><div class="meta">{r.get('game','')} · Slot {fmt(r.get('lineup_slot'))}</div><div class="score">{fmt(r.get('score'))}<small>BLEND SCORE</small></div><div class="meter"><div class="fill" style="width:{safe_meter(r.get('score'))}%"></div></div><div class="stats"><div class="stat"><b>{fmt(r.get('pull_pct'))}</b><span>Pull</span></div><div class="stat"><b>{fmt(r.get('pitch_edge'))}</b><span>Pitch</span></div><div class="stat"><b>{fmt(r.get('dmg'))}</b><span>DMG</span></div><div class="stat"><b>{fmt(r.get('hr_pa'))}</b><span>HR/PA</span></div><div class="stat"><b>{fmt(r.get('hpi'))}</b><span>HPI</span></div><div class="stat"><b>{fmt(r.get('sweet_spot_pct'))}</b><span>Sweet</span></div></div><span class="badge">LEGAL</span></div>""",unsafe_allow_html=True)
def gate_strip():
