...
def parse_star_lines(lines):
    ...
    while i < len(lines):
        if i in sections:
            team,pitcher=sections[i]; section+=1
        line=lines[i]
        if re.match(r"^\d+$",line) and i+1<len(lines) and is_player_name(lines[i+1]):
            name=lines[i+1].strip()
            block_lines=[]; j=i+1
            while j < len(lines):
                if j>i+1 and re.match(r"^\d+$",lines[j]) and j+1<len(lines) and is_player_name(lines[j+1]):
                    break
                if j in sections:
                    break
                if lines[j].startswith("https://") or re.match(r"Page \d+ of",lines[j]):
                    break
                block_lines.append(lines[j]); j+=1
            block=" ".join(block_lines)
            bats = block_lines[1] if len(block_lines)>1 and block_lines[1] in ["L","R","S"] else ""
            slot=None
            ...
            if hpi is None:
                pm=[int(x) for x in re.findall(r"\+\s*(\d{2})(?=\s|$)",block) if 10<=int(x)<=90]
                if pm: hpi=float(pm[-1])
            ...
            rows.append({"game":f"{tm} vs {pit}","team":tm,"opponent":"","pitcher":pit,"player":name,"bats":bats,
                ...
                "bullpen_dmg":None,"confirmed_lineup":False,"dob":None,"jersey":None,"result_hr":False,"notes":block[:260]})
            i=j; continue
        i+=1
    ...
    for d in attempts:
        try:
            for val in d.astype(str).values.ravel().tolist():
                if str(val).strip() and str(val).strip().lower() != "nan": lines.append(str(val).strip())
        except Exception:
            pass
