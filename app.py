...
st.write("Fix: **pitcher weakness archetype first → matching hitters only → FULL 18-gate Blender elimination → one owner.**")
up=st.file_uploader("Upload Star Tool PDF / CSV / Excel", type=["pdf","csv","xlsx","xls"])
if up:
    with st.spinner("Reading slate, locking pitcher archetypes, then running the FULL 18-gate Blender..."):
        df=load_any(up)
