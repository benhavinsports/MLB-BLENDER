...
.blender-wrap{position:relative;height:430px;border-radius:30px;border:1px solid var(--line);background:radial-gradient(circle at center,rgba(217,255,47,.08),rgba(0,0,0,.96));overflow:hidden;margin-top:12px}
.jar{position:absolute;left:50%;top:44%;width:320px;height:330px;transform:translate(-50%,-50%);border-radius:46px 46px 110px 110px;border:3px solid rgba(245,234,216,.18);background:linear-gradient(180deg,rgba(245,234,216,.06),rgba(0,0,0,.34));box-shadow:inset 0 0 45px rgba(217,255,47,.08)}
.blade{position:absolute;left:50%;top:52%;width:250px;height:250px;margin:-125px;border-radius:50%;background:conic-gradient(transparent 0deg,var(--acid) 34deg,transparent 76deg,var(--green) 150deg,transparent 205deg,var(--orange) 268deg,transparent 320deg);animation:spin .75s linear infinite;filter:blur(.2px)}
.center{position:absolute;left:50%;top:52%;width:116px;height:116px;margin:-58px;border-radius:50%;background:#050505;border:2px solid rgba(245,234,216,.28);display:grid;place-items:center;font-family:'Black Ops One';color:var(--acid);z-index:3}
.float{position:absolute;font-family:Rajdhani;background:rgba(245,234,216,.12);border:1px solid rgba(245,234,216,.20);border-radius:999px;padding:8px 11px;color:var(--cream);max-width:160px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;animation:bob 4s ease-in-out infinite;z-index:4}
.f1{top:15%;left:7%}.f2{top:16%;right:7%}.f3{bottom:20%;left:8%}.f4{bottom:18%;right:8%}.f5{top:5%;left:37%}.f6{bottom:8%;left:36%}
.feed-slot{position:absolute;left:50%;top:12px;transform:translateX(-50%);font-family:'Black Ops One';background:linear-gradient(90deg,var(--acid),var(--green));color:#050505;padding:12px 22px;border-radius:999px;z-index:6}
.output-slot{position:absolute;left:50%;bottom:0;transform:translateX(-50%);font-family:'Black Ops One';letter-spacing:1px;color:#ecffc1;background:rgba(5,5,5,.92);width:100%;text-align:center;padding:15px 10px}
...
def blender_visual(names=None, status="READY"):
    ...
    st.markdown(f"""<div class="machine"><div class="section-title" style="font-size:30px">VISUAL BLENDER MACHINE</div><div class="blender-wrap"><div class="feed-slot">FEED PLAYERS</div><div class="jar"><div class="blade"></div><div class="center">{status}</div>{floats}</div><div class="output-slot">RESULTS POP OUT → CORE / ALT / CHAOS</div></div></div>""", unsafe_allow_html=True)
...
css()
st.markdown("""<div class="hero"><div class="title">THE <span class="hot">BLENDER</span><br/>MACHINE</div><span class="chip">FEED</span><span class="chip">BLEND</span><span class="chip">CORE 3</span><span class="chip">ALT 3</span><span class="chip">CHAOS 3</span></div>""", unsafe_allow_html=True)
