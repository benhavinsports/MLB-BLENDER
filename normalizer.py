...
def map_columns(df):
    norm={c:re.sub(r"[^a-z0-9]+","_",str(c).lower()).strip("_") for c in df.columns}; rename={}
...
def clean_df(df):
    ...
    if df["player"].isna().all() or (df["player"].astype(str).str.strip()=="").all():
        candidates=[]
        for c in df.columns:
            vals=df[c].dropna().astype(str).head(120); score=sum(is_player_name(v) for v in vals)
    ...
    for c in ["team","pitcher","game"]: df[c]=df[c].fillna("").astype(str).replace("None","")
