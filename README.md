...
This build preserves the original README/version-history format above and adds the engine-authority rebuild.

### What changed
- Rebuilt `engine.py` as a true state machine, not a projection/refill sorter.
- Removed generic Core refill behavior.
- Disabled fallback sorting and role recycling.
- Scores now derive from elimination survival depth + gate quality instead of fake normalization.
- One player can occupy only one official path.
- One owner per game is resolved from pass-depth and gate-state memory.
- Core is built only from surviving PRIMARY / ADJACENT / WHO lanes.
- Game Board reads the same `game_board` gate memory used by tickets.
- `load_locked_results()` returns empty results so stale owners do not reappear on app start.

### Current locked runtime files
- `app.py`
- `engine.py`
- `feeder.py`
- `ui.py`
...
## v164 — TRUE EVENT OWNER ENGINE / NO SCORE CLUSTER

- Removed the 98/98/98 score clustering behavior.
- Attack-side lock is context only, not an automatic hard kill.
- Core 3 is the top 3 true event owners.
- Roles display truthfully and are not forced.
- Scores are widened using event ownership, gate quality, player/game fingerprint, and cut penalties.
- No player-name hardcoding is used.


## V0217 Game Board Only Fix
- Does not change CORE / ALT / WHO ticket logic.
- Adds a direct uploaded-feed full slate map to the Game Board tab so the board does not collapse into only ticket owners.
- Shows games present in the feed but not rendered as ticket owners as a board audit, without refilling tickets or changing Blender integrity.
